...
def mutate_plan(plan: CityPlan, exits, rng: random.Random,
    ...
    for _ in range(max_tries):
        ...
        if parent_built and not child.render()[0]:
            continue
        zone = danger
        if zone is not None and (child.width != plan.width
                                 or child.height != plan.height):
            # The canvas operator moved the crop edge, so a zone drawn on the
            # old one can now sit outside the world.
            zone = zone.scaled_to(float(child.width), float(child.height))
        ok, _why = check(child, exits, zone)
        if ok:
