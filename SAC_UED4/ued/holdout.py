...
* hand/JSON maps from `map_infos/` (since removed: every map is now a
  citygen level or an OSM crop);
...
# Crowd size is not fixed here any more. It is a density on the walkable
# ground (CROWD_DENSITY_RANGE), so a 50 m crop and a 180 m one are evaluated
# at the same congestion rather than at the same headcount, which at these
# sizes differ by more than a factor of ten. The density is drawn from the
# level's own seeded generator, so the set stays reproducible.
    ...
    return [
        {
            ...
            "crowd_density": lv.crowd_density,
            # The hazard and the crowd's relation to it are the task. Before
            # this they were dropped on the way into the cache, so a cached
            # holdout level arrived at evaluation with no danger zone at all
            # and measured a different problem from the one trained on.
            "danger": (lv.danger.to_dict() if lv.danger is not None else None),
            "inside_fraction": lv.inside_fraction,
            "perceptibility": lv.perceptibility,
            "prior_informed": lv.prior_informed_fraction,
            "robot_num": lv.robot_num,
            "width": lv.width,
            "height": lv.height,
    ...
    for d in raw:
        ...
        out.append(
            Level(
                ...
                crowd_density=d.get("crowd_density"),
                width=int(d["width"]),
                height=int(d["height"]),
                augmentation="identity",
                danger=None if zone is None else DangerZone.from_dict(zone),
                inside_fraction=float(d.get("inside_fraction", 1.0)),
                perceptibility=float(d.get("perceptibility", 0.5)),
                prior_informed_fraction=float(d.get("prior_informed", 0.0)),
                robot_num=int(d.get("robot_num", 1)),
