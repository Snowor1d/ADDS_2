...
class _EpisodeTrace:
    __slots__ = ("samples", "n_seen", "ret", "level_id", "touched")

    def __init__(self):
        # Opaque value-function inputs, one per sampled decision instant. The
        # trainer passes (static layers, record window) pairs, a few kB each,
        # rather than rendered stacks.
        self.samples: List = []
        self.n_seen = 0
        self.ret = 0.0
        self.level_id = -1
...
class UEDRunner:
    def __init__(
        self,
        value_fn: Optional[Callable[[List], np.ndarray]] = None,
        rng: Optional[random.Random] = None,
        schema: Optional[Dict[str, str]] = None,
    ):
        # The curriculum runs when training draws its maps from it.
        from config import TRAIN_MAP_SOURCE
        self.enabled = TRAIN_MAP_SOURCE == "ued"
        # Success histories mean something only under the reward and
        # observation they were measured with.
    ...
    def on_transition(self, msg):
        ...
        trace.n_seen += 1
        sample = getattr(msg, "value_sample", None)
        if sample is None:
            return
        if len(trace.samples) < MAXMC_SAMPLES:
    ...
    def _maxmc(self, trace: _EpisodeTrace) -> Optional[float]:
        ...
        """
        if trace is None or not trace.samples or self.value_fn is None:
            return None
        ...
        try:
            values = self.value_fn(trace.samples)
        except Exception as e:
    ...
    def load(self, path: str) -> bool:
        ...
        try:
            with open(path, "rb") as f:
                state = pickle.load(f)
            stored = state.get("schema", {})
            if self.schema and stored != self.schema:
                raise RuntimeError(
                    f"curriculum state {path} was recorded under {stored or 'no'} "
                    f"schema versions, not {self.schema}; its success history "
                    "does not mean the same thing. Move it aside or start a "
                    "new LOG_DIR.")
...
def make_value_fn(agent):
    """Moved to learn/sac.py with the new observation; kept importable."""
    from learn.sac import make_value_fn as _mk
    return _mk(agent)
