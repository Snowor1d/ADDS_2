...
from sim.crowd_density import crowd_size_for, realised_density, walkable_area
...
def mutate_level(
    ...
    # The hazard has to reach the validator, or a child is judged on whether
    # its fabric is walkable rather than on whether the crowd can get out of
    # the zone, which is the only question that matters.
    ...
    # A resized canvas moves the crop edge, so the hazard has to be brought
    # back inside it. Its physical size is unchanged: a gas leak does not get
    # bigger because the map did.
    child_danger = parent.danger
    if child_danger is not None:
        child_danger = child_danger.scaled_to(float(child_plan.width),
                                              float(child_plan.height))

    # Congestion is the heritable trait, not the headcount. A mutation that
    # demolishes a block or resizes the canvas changes how much ground the
    # crowd has, so carrying the parent's number across would quietly move the
    # child to a different density; carrying the parent's density and
    # recomputing the number keeps the axis the curriculum is varying separate
    # from the one it is not.
    child_obstacles = [[list(p) for p in ring] for ring in rings]
    child_area = walkable_area(child_plan.width, child_plan.height,
                               child_obstacles)
    child_crowd = crowd_size_for(child_area, rng=rng,
                                 density=parent.crowd_density)
    return parent.child(
        obstacles=child_obstacles,
        exits=exits,
        crowd_size=child_crowd,
        crowd_density=realised_density(child_crowd, child_area),
        ops=ops,
