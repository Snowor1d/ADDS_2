...
class Level:
    ...
    # Kept as a field for the level format, but the task no longer uses
    # exits (the numbered maps that had them were removed): safety is defined by the
...
def level_from_map_data(
    ...
    Kept for random_map callers; the numbered maps that used it were
    removed. The curriculum does not use this path: its
...
def draw_training_seed(rng: Optional[random.Random] = None) -> int:
    """A generation seed outside every range reserved for evaluation.

    Validation and final zero-shot seeds are declared in configs/training/common.py
    and reserved there; a training level drawn on one of them would put an
    evaluation map in the training distribution.
    """
    from config import TRAIN_RESERVED_SEED_RANGES

    rng = rng or random
    while True:
        seed = rng.randrange(0, 2**31 - 1)
        if not any(lo <= seed < hi for lo, hi in TRAIN_RESERVED_SEED_RANGES):
            return seed


def sample_augmentation(rng: Optional[random.Random] = None) -> str:
    rng = rng or random
    from config import UED_AUGMENTATION, UED_AUGMENTATION_TRANSFORMS
    if not UED_AUGMENTATION:
        return "identity"
    return rng.choice(list(UED_AUGMENTATION_TRANSFORMS))


def sample_map_size(rng: Optional[random.Random] = None):
    """Draw a world size in metres for a fresh level.

    From the discrete UED_MAP_SIZES_M when it is set (the shared policy is
    trained on the crop sizes it is evaluated on), otherwise uniformly from
    UED_MAP_SIZE_RANGE.
    """
    from config import UED_MAP_SIZE_WEIGHTS, UED_MAP_SIZES_M

    rng = rng or random
    if UED_MAP_SIZES_M:
        w = int(rng.choices(list(UED_MAP_SIZES_M),
                            weights=list(UED_MAP_SIZE_WEIGHTS))[0])
        return w, w
...
def generate_city_level(rng: random.Random, difficulty: int,
    ...
    from config import (MAX_ROBOTS, UED_DANGER_AREA_RANGE,
                        ...
                        UED_ROBOT_RANGE)
    from sim.danger import sample_along_street, sample_zone
