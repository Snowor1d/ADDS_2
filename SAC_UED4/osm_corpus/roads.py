...
# How far from a mapped street unbuilt ground is still treated as walkable.
# See reachable_open_ground: this is the lever on how much a sparsely mapped
# city gets back, and it is deliberately short, because the further from a
# mapped way it reaches the more it is a guess about somewhere nobody
# surveyed.
OPEN_GROUND_REACH_M = 6.0
...
def reachable_open_ground(traversable, buildings, size_m: float,
                          height_m: Optional[float] = None,
                          reach_m: float = OPEN_GROUND_REACH_M):
    """Unbuilt ground next to the road network, which is walkable too.

    The road network is not a complete description of where people can walk,
    and how incomplete it is varies enormously by city. Chandni Chowk's bazaar
    is a wide pedestrian precinct that OpenStreetMap records as a single
    `highway=pedestrian` centreline; buffered to a carriageway that is eleven
    metres of a precinct many times wider, and the rest becomes wall. Measured
    on the 400 m crops, 83 per cent of what the road network called blocked at
    Chandni Chowk holds no building at all, against 32 per cent at Shibuya.

    So ground that carries no building and touches the street network is
    returned to the walkable space. Both halves of that test matter. Without
    the building check this would hand back building interiors; without the
    reachability check it would hand back private courtyards and the insides
    of blocks, which is exactly the overstatement road-based extraction was
    introduced to remove.

    `reach_m` is how far from a mapped street a patch of open ground is still
    counted. Kept small, because the further out it goes the more it becomes a
    guess about a place nobody mapped.

    Off by default (`OSM_OPEN_GROUND = False`), because measuring it showed it
    fails where it was most needed. At Chandni Chowk the blocked share fell to
    0.128 while buildings genuinely cover 0.168 of the crop: the reach buffer
    threaded between the footprints and hollowed them out. Shibuya and Mitte
    moved the right way, 0.590 to 0.397 and 0.788 to 0.457 against built
    shares of 0.684 and 0.528. So it helps where the survey is good and
    misleads where it is poor, and it makes the definition of walkable depend
    on how well mapped a city happens to be. Sparsely mapped crops are dropped
    by OSM_MAX_UNBUILT_BLOCKED instead.
    """
    from shapely.ops import unary_union

    if traversable is None or traversable.is_empty:
        return traversable

    height_m = size_m if height_m is None else height_m
    world = box(0, 0, size_m, height_m)

    built = unary_union(buildings) if buildings else None
    open_ground = world.difference(traversable)
    if built is not None and not built.is_empty:
        open_ground = open_ground.difference(built)
    if open_ground.is_empty:
        return traversable

    # Only the parts that actually touch the street network, so a sealed
    # courtyard stays sealed.
    near = traversable.buffer(reach_m, join_style=BUFFER_JOIN_STYLE)
    keep = []
    parts = (open_ground.geoms if open_ground.geom_type == "MultiPolygon"
             else [open_ground])
    for part in parts:
        if part.geom_type != "Polygon" or part.area < MIN_BLOCK_AREA_M2:
            continue
        if part.intersects(near):
            keep.append(part)
    if not keep:
        return traversable
    return unary_union([traversable] + keep).intersection(world)
