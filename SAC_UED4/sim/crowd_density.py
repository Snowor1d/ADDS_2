...
def density_range_for(size_m: Optional[float] = None, table=None):
    """The density band for a map of this size.

    `table` is a training mode's per-size bands (UED_DENSITY_BY_SIZE or
    DATASET_DENSITY_BY_SIZE); an entry may lower the band for sizes whose cost
    at the target density is prohibitive; the level still records what it ran at, and a
    size run below target must be reported as a density change, not only a
    size change.
    """
    from config import CROWD_DENSITY_RANGE

    if size_m is not None and table:
        band = table.get(int(round(float(size_m))))
        if band is not None:
...
def sample_density(rng: Optional[random.Random] = None,
                   size_m: Optional[float] = None, table=None) -> float:
    """Draw a target density for one level, persons per walkable square metre."""
    rng = rng or random
    lo, hi = density_range_for(size_m, table)
...
def crowd_size_for(walkable_m2: float,
                   ...
                   size_m: Optional[float] = None, table=None) -> int:
    ...
    if density is None:
        density = sample_density(rng, size_m=size_m, table=table)
...
def crowd_size_for_level(width: float, height: float,
                         ...
                         rng: Optional[random.Random] = None,
                         density: Optional[float] = None, table=None):
    ...
    n = crowd_size_for(area, rng=rng, density=density,
                       size_m=max(float(width), float(height)), table=table)
