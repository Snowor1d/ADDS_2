...
def load_real_level(site: str, size: int):
    ...
    lines.append("  python3 -m cli.ADDS_AS_osm_pipeline collect")
    lines.append("  python3 -m cli.ADDS_AS_osm_pipeline export")
    raise SystemExit("\n".join(lines))
...
def attach_hazard(level, rng=None):
    ...
    # A real crop, so the dataset mode's task parameters, with the area
    # picked by SIM_DIFFICULTY rather than drawn.
    from config import (DATASET_DANGER_AREA_RANGE,
                        DATASET_DANGER_INSIDE_FRACTION, DATASET_DANGER_SHAPES,
                        SIM_DIFFICULTY, SIM_ROBOTS, UED_DIFFICULTY_RANGE)
    ...
    lo_a, hi_a = DATASET_DANGER_AREA_RANGE
    ...
    for _ in range(30):
        shape = DATASET_DANGER_SHAPES[rng.randrange(len(DATASET_DANGER_SHAPES))]
        ...
        if ok:
            level.danger = zone
            level.exits = []
            lo_f, hi_f = DATASET_DANGER_INSIDE_FRACTION
...
def level_from_config(source: str = None, site: str = None,
    ...
    Returns (level, tag). Every source is a city: a generated citygen level
    or a real OSM crop, each with a hazard. The tag names the scenario for
    output folders, so results from a real crop and from a generated level
