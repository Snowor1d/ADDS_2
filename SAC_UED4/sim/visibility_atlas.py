...
class VisibilityAtlas:
    ...
    def has_radius(self, radius: float) -> bool:
        return float(radius) in self._radii

    def exact_polygon(self, x: float, y: float, radius: float,
                      rays: int = 128) -> Polygon:
        """Field of view from this exact point, cast now rather than cached.

        The cached polygons are cast from the centre of a 4 m region, which is
        ...
        if radius not in self._radii:
            raise ValueError(f"radius {radius} m is not registered with the "
                             f"visibility atlas ({self._radii})")
        if self._obs_index is None:
            self._obs_index = _ObstacleIndex([])
        R = float(radius)
        angles = 2.0 * math.pi * (np.arange(rays) / float(rays))
        dir_x = np.cos(angles)
        dir_y = np.sin(angles)
        edges, inside, _cands = self._candidate_edges((x, y), R)
        if inside:
            # Standing inside a building outline sees nothing.
            return Polygon()
        if edges is None:
            t = np.full(rays, R)
        else:
            t = self._cast_rays((x, y), dir_x, dir_y, R, edges)
        pts = list(zip(x + dir_x * t, y + dir_y * t))
        return Polygon(pts)
