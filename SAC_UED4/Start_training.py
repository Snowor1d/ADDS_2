...
# The trainer moved into learn/ with the layout tidy-up, so it is launched
# as a module. The name is also what the stall handler matches on when it
# kills the run, and a pattern that matched a bare filename would match the
# same filename in a sibling project copy.
TARGET_MODULE = "learn.ADDS_AS_reinforcement"
TARGET_SCRIPT = TARGET_MODULE
