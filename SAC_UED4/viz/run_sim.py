...
- Pause/Resume, Step, Reset (a new city level for SIM_SOURCE=curriculum), Speed
- 에피소드 종료 시 자동 재시작
...
# Wide enough to carry the OpenStreetMap thumbnail beneath the controls. It
# used to sit in the corner of the map view, where it covered the part of the
# simulation nearest it; the whole point of showing it is comparing the two,
# and a thumbnail that hides what it is being compared against does not.
PANEL_RIGHT_WIDTH = 420                     # 우측 패널 고정 폭
PADDING = 10                                # 좌/우/상/하 패딩

# 기본 월드 크기(리셋 시 사용). 더 크게 바꿔도 화면 내에 자동으로 맞춰 그려짐
GRID_W, GRID_H = MAP_W, MAP_H

SIM_TIMESTEP = SIM_VIEW_TIMESTEP
# Rendering costs about 23 ms a frame after the renderer was changed to draw
# the crowd as one collection and to clear artists rather than rebuild the
# axes, down from 65 ms. That leaves room for a higher frame rate and for more
# simulation between frames.
TARGET_RENDER_FPS = SIM_RENDER_FPS
# Speed 1.0 is real time: one step is SIM_TIMESTEP seconds of simulated time,
# so 1.0 means four steps a second. The top of the range is what the engine
# can actually sustain rather than an arbitrary round number.
SPEED_MIN, SPEED_MAX = SIM_SPEED_RANGE
MAX_STEPS = 3000
MAX_ACCUM_STEPS = 60
# Steps the viewer may run between two frames.
#
# Six, with a ten-frame target, capped the viewer at sixty steps a second
# whatever the slider said, while the engine itself does about 195. The cap
# exists so a slow frame cannot stall the window, and the adaptive rule below
# still lowers it when rendering starts to miss its budget; it was simply set
# far under what the simulation can deliver.
MAX_SUBSTEPS_PER_FRAME_HARD = 24
ADAPTIVE_SUBSTEP_INIT = 4

ROBOT_CONTROL_MODE = SIM_ROBOT_CONTROL   # "RL", "Human"
ROBOT_VERSION_FOR_MODEL = "Q"
MODEL_NAME = SIM_CHECKPOINT

USE_CONTINUOUS_RENDERER = True
def load_basemap(level):
    """The OpenStreetMap tiles the running level was cut from, as a surface.

    Only a real crop has one. A generated level is not a picture of anywhere,
    so there is nothing to show beside it and this returns None.

    The point is that the extracted layout is an interpretation: road
    centrelines buffered to a carriageway, with everything else called
    obstacle. Whether that interpretation is right is not visible in the
    layout alone, and it is obvious in a second with the real map beside it.
    Tiles are cached on disk, so this costs a download once per site and
    nothing afterwards.
    """
    import numpy as _np
    ...
    lat = getattr(level, "lat", None)
    lon = getattr(level, "lon", None)
    if site is None or lat is None or lon is None:
        return None

    try:
        from osm_corpus.compare import map_image

        img = map_image(float(lat), float(lon), float(level.width))
        arr = _np.asarray(img.convert("RGB"))
        return _pygame.surfarray.make_surface(_np.transpose(arr, (1, 0, 2)))
    except Exception as e:
        print(f"[run_sim] no base map for {site}: {type(e).__name__}: {e}")
        return None
...
# ====== ENV / POLICY ======
# The city level being watched: a real OSM crop or a generated citygen level.
# It carries its own world size and crowd count.
REAL_LEVEL = None
BASEMAP_SURFACE = None


from sim.scenario import (attach_hazard, build_curriculum_level,
                      level_from_config, load_real_level)


def create_env_and_renderer(map_id:int, n_agents:int, world_w:int, world_h:int):
    """map_id, n_agents and the world size are ignored: the level decides."""
    global REAL_LEVEL
    if REAL_LEVEL is None:
        REAL_LEVEL = build_curriculum_level()
    if True:
        env = FightingModel(number_agents=int(REAL_LEVEL.crowd_size),
                            width=int(REAL_LEVEL.width),
                            height=int(REAL_LEVEL.height),
                            robot=ROBOT_VERSION_FOR_MODEL,
                            level=REAL_LEVEL)
        renderer = (make_renderer(int(REAL_LEVEL.width), int(REAL_LEVEL.height))
                    if USE_CONTINUOUS_RENDERER else None)
...
def main():
    ...
    pending_map = 0
    pending_agents = int(REAL_LEVEL.crowd_size) if REAL_LEVEL is not None else 0
    ...
    # The level decides the map and the crowd, so there is no map id or
    # agent count to type in any more.
    widgets=[btn_pause, btn_step, btn_reset, slider]
    ...
    while running:
        ...
        # The OpenStreetMap tiles the crop came from, in the panel rather than
        # over the map. The extracted layout is an interpretation of that map,
        # and reading one against the other is the reason it is on screen at
        # all, so it cannot sit on top of the thing it is being compared with.
        # Only a real crop has one; a generated level is not a picture of
        # anywhere.
        if BASEMAP_SURFACE is not None:
            side = panel_w
            thumb = pygame.transform.smoothscale(BASEMAP_SURFACE, (side, side))
            bx = panel_x
            by = SCREEN_HEIGHT - side - BASEMAP_PANEL_BOTTOM_MARGIN
            screen.blit(font_small.render("OpenStreetMap", True, DARK),
                        (bx, by - 16))
            pygame.draw.rect(screen, (30, 30, 30),
                             (bx - 2, by - 2, side + 4, side + 4))
            screen.blit(thumb, (bx, by))
            where = getattr(REAL_LEVEL, "city", None)
            if where:
                site = getattr(REAL_LEVEL, "site_key", "")
                screen.blit(
                    font_small.render(f"{site}, {where}", True, (90, 90, 90)),
                    (bx, by + side + 3))
...
def _parse_args():
    ...
    ap = argparse.ArgumentParser(
        description="Watch the crowd simulation. Which map it shows comes from "
                    "SIM_REAL_SITE and SIM_REAL_SIZE in config.py, the way the "
                    "rest of this project is configured; the flags below only "
                    "override that for a one-off.")
    ap.add_argument("--real", metavar="SITE", default=None,
                    help="override SIM_REAL_SITE for this run")
    ap.add_argument("--size", type=int, default=None,
                    help="override SIM_REAL_SIZE for this run")
    ap.add_argument("--curriculum", action="store_true",
                    help="a freshly generated city level with a hazard, which "
                         "is what training runs on")
...
if __name__=="__main__":
    ...
    if source == "curriculum":
        REAL_LEVEL = build_curriculum_level()
        print("[run_sim] no base map: a generated level is not a picture of "
              "anywhere. Set SIM_SOURCE = \"real\" to watch a real crop with "
              "its OpenStreetMap tiles beside it.")
    elif source == "real":
        _site = SIM_REAL_SITE if _args.real is None else _args.real
        _size = SIM_REAL_SIZE if _args.size is None else _args.size
        REAL_LEVEL = load_real_level(_site, _size)
        REAL_LEVEL = attach_hazard(REAL_LEVEL)
        print(f"[run_sim] real level {_site} at {_size} m: "
              f"{len(REAL_LEVEL.obstacles)} obstacles, "
              f"{REAL_LEVEL.crowd_size} pedestrians"
              + (f" ({REAL_LEVEL.crowd_density:.3f}/m2 walkable), "
                 if REAL_LEVEL.crowd_density else ", ") +
              f"{REAL_LEVEL.danger.shape} hazard")
    else:
        raise SystemExit(f"unknown SIM_SOURCE {source!r}; expected 'real' "
                         "or 'curriculum'")
    # Loaded from whichever level we ended up with, rather than only on the
    # branch that built it: --real sets the level through a different path,
    # and the map should follow the level, not the branch.
    if REAL_LEVEL is not None:
        BASEMAP_SURFACE = load_basemap(REAL_LEVEL)
    main()
