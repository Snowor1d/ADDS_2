...
class ContinuousRenderer:
    ...
    def __init__(
        ...
        # ===== Sizes =====
        # None means "the body the simulation actually uses". A number
        # overrides it, for a figure that needs to be legible at print size.
        crowd_radius: Optional[float] = None,
        ...
        scale = float(RENDER_BODY_SCALE)
        self.crowd_r = (float(AGENT_BODY_RADIUS) * scale
                        if crowd_radius is None else float(crowd_radius))
        self.robot_r = (float(ROBOT_BODY_RADIUS) * scale
                        if robot_radius is None else float(robot_radius))
        ...
        # The project root, not this file's folder: the renderer lives in
        # viz/ now and images/ is beside it, not under it.
    ...
    def draw(self, model, step: Optional[int] = None) -> np.ndarray:
        ...
        # Remove what was drawn, not the axes themselves.
        #
        # `ax.clear()` tears down the ticks, spines and labels and rebuilds
        # them from scratch on every frame, which cost 0.19 s of clearing plus
        # 0.23 s of axis drawing over twenty frames. The axes are off and
        # their limits are fixed, so none of that work changes anything on
        # screen.
        for artist in (list(self.ax.patches) + list(self.ax.lines)
                       + list(self.ax.collections) + list(self.ax.texts)
                       + list(self.ax.images)):
            artist.remove()
        if not getattr(self, "_axes_ready", False):
            self._setup_axes()
        ...
        # Under the obstacles and the crowd, so a pedestrian standing in the
        # hazard still reads as a pedestrian. Where people are is what has to
        # be legible; the zone is stationary background.
        self._draw_danger_zone(model)
        self._draw_crowds(getattr(model, "crowds", []))
        self._draw_obstacles(getattr(model, "obstacles", []))
        # Every robot, not just the first. The task is a team one and the
        # viewer drew `model.robot` alone, so a two-robot episode looked like
        # a one-robot episode with an invisible helper.
        team = list(getattr(model, "robots", None) or [])
        if not team:
            team = [self._find_robot(model)]
        for _rb in team:
            self._draw_robot(_rb, step=step)
    ...
    def _draw_danger_zone(self, model):
        """The hazard, washed in red over the ground it actually covers.

        Clipped to the free space rather than drawn as a bare circle or
        rectangle. A hazard laid over a city covers streets and courtyards,
        not the insides of blocks, and painting the raw shape puts red over
        masonry and makes it hard to see which of the red is ground somebody
        could be standing on. The outline of the raw shape is kept, because
        that is the line the task is defined by and the clipped fill alone
        would not show where it runs behind a block.

        Translucent so the fabric and the crowd stay readable underneath: the
        question being asked of this picture is usually "who is still inside".
        """
        ...
        from config import (DANGER_EDGE_COLOR, DANGER_FILL_ALPHA,
                            DANGER_FILL_COLOR, DANGER_ZORDER)

        from ued.render import _hazard_patch, _zone_over_free_space

        # Shared with the curriculum snapshots, so the viewer and the contact
        # sheets cannot disagree about where the hazard covers ground.
        for exterior, holes in _zone_over_free_space(model, zone):
            patch = _hazard_patch(exterior, holes,
                                  facecolor=DANGER_FILL_COLOR,
                                  edgecolor="none",
                                  alpha=DANGER_FILL_ALPHA,
                                  zorder=DANGER_ZORDER)
            if patch is not None:
                self.ax.add_patch(patch)

        # From the zone's own polygon, the same object the fill was clipped
        # out of. Rebuilding it from cx/cy and the half-extents drew an
        # unrotated rectangle for a rotated hazard, so the dashed outline sat
        # at a different angle from the red it was supposed to bound.
        edge = mpatches.Polygon(list(zone.polygon().exterior.coords),
                                closed=True, facecolor="none",
                                edgecolor=DANGER_EDGE_COLOR, linewidth=1.8,
                                linestyle="--", zorder=DANGER_ZORDER + 0.1)
        self.ax.add_patch(edge)

    def _body_area_points(self, radius_m: float) -> float:
        """A body's area in points squared, for scatter's `s`.

        scatter sizes markers in points, not in data units, so a fixed `s`
        would make pedestrians the wrong size on every map. Converting through
        the axis transform keeps them the metres across that they are.
        """
        fig = self.ax.figure
        bbox = self.ax.get_window_extent()
        x0, x1 = self.ax.get_xlim()
        span = max(1e-9, abs(x1 - x0))
        px_per_m = bbox.width / span
        pts_per_m = px_per_m * 72.0 / fig.dpi
        return max(1.0, (radius_m * pts_per_m) ** 2 * 3.14159)
    ...
    def _draw_crowds(self, crowds):
        ...
        for ag in crowds:
            ...
            # 몸체: 개별 패치가 아니라 한 번의 산점도로 모아 그린다.
            #
            # `add_patch` was 1.38 s of a 2.26 s profile over twenty frames,
            # against 0.47 s of actual drawing. Every patch makes matplotlib
            # re-derive the axis limits, and for a curved patch that means
            # finding the extrema of its Bezier segments: 12,800 calls to
            # numpy.roots and as many to eigvals, for forty pedestrians, to
            # recompute limits that are pinned by _setup_axes anyway.
            bodies.append((x, y, body_col))
        ...
        if bodies:
            # One collection for the whole crowd. `s` is in points squared, so
            # the radius has to go through the data-to-points transform to keep
            # bodies the physical size they are in metres.
            import numpy as np

            xs = np.fromiter((b[0] for b in bodies), dtype=float,
                             count=len(bodies))
            ys = np.fromiter((b[1] for b in bodies), dtype=float,
                             count=len(bodies))
            cols = [b[2] for b in bodies]
            self.ax.scatter(xs, ys, s=self._body_area_points(self.crowd_r),
                            c=cols, edgecolors=cols, linewidths=0.6,
    ...
    def _draw_robot_signal(self, robot, x: float, y: float,
                           step: Optional[int] = None):
        """What this robot is signalling, drawn on it.

        Half the action is invisible otherwise. A robot in "off" mode and one
        leading a crowd look identical on screen, and that is exactly the
        comparison the modes exist to make, so watching a run tells you
        ...
        if mode == "off":
            self.ax.add_patch(Circle(
                (x, y), radius=self.robot_r * 1.6,
                facecolor="none", edgecolor="#999999", linewidth=0.8,
                linestyle=":", antialiased=True, zorder=3.1))
            return
        if mode == "guide":
            # Visual signal, not a collision or influence radius. Flash on
            # simulation steps so replay and paused frames are deterministic.
            # Keep it behind the default robot image, not over the body.
            if step is None or int(step) % 4 < 2:
                self.ax.add_patch(Circle(
                    (x, y), radius=self.robot_r * 3.0,
                    facecolor=(0.84, 0.10, 0.12, 0.20),
                    edgecolor=(0.84, 0.10, 0.12, 0.75),
                    linewidth=1.6, antialiased=True, zorder=2.9))
            return
        # "direct": an arrow along the signalled heading.
        sx, sy = getattr(robot, "signal_dir", (0.0, 0.0))
        norm = math.hypot(float(sx), float(sy))
        if norm < 1e-6:
            return
        length = max(3.0, self.robot_r * 6.0)
        self.ax.annotate(
            "", xy=(x + sx / norm * length, y + sy / norm * length),
            xytext=(x, y),
            arrowprops=dict(arrowstyle="-|>", color="#1f77b4", linewidth=2.0,
                            shrinkA=0, shrinkB=0),
            zorder=3.2)
