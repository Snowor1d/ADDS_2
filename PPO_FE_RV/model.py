...
class FightingModel(Model):
    ...
    def step(self):
        ...
        if self.robot_version == 'Q':
            if self.using_model and is_boundary:
                ...
                # ---------------------------------------------------------
                action, _ = self.rl_agent.select_action(
                    ego_state,
                    glob_state,
                    robot_state,
    ...
    def use_model(self, file_path):
        ...
        self.ppo_agent = PPOAgent(input_shape=input_shape)
        self.rl_agent = self.ppo_agent
        # Compatibility alias for existing visualization/experiment scripts.
        self.sac_agent = self.ppo_agent
