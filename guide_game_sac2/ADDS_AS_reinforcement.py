...
log_dir = os.path.join(home_dir, "learning_log_guide_game_sac2")
...
class ReplayBuffer:
    ...
    def sample(self, batch_size, device):
        ...
        # 각 상태의 shape가 (70,70,4)라면:
        states = torch.FloatTensor(states).permute(0, 3, 1, 2).to(device)  # (B, 4, H, W)
...
class QNetwork(nn.Module):
    ...
    def forward(self, state, action):
        ...
        # state feature와 action을 연결
        x = torch.cat([x, action], dim=1)  # (B, conv_out_size + action_dim)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        q_val = self.q_out(x)
        return q_val
...
class PolicyNetwork(nn.Module):#행동을 샘플링하고 정책 학습, 주어진 상태 s에 대해 행동 a 결정, 연속적 & 이산적 행동 가능, 혼합 가능 (Actor)
    ...
    def __init__(self, input_shape=(70,70)):
        ...
        # state feature를 받아서 연속적인 방향의 mean, log_std 추론
    ...
    def forward(self, state):
        ...
        # log_std의 범위를 clamp하여 너무 크거나 작지 않도록 함
...
# ------------------------------------------------- #
    ...
    def select_action(self, state_np, deterministic=False):
        ...
        state_t = torch.FloatTensor(state_np).permute(2, 0, 1).unsqueeze(0).to(self.device)  # (1, 4, H, W)
        with torch.no_grad():
