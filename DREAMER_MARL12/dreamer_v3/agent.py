...
class DreamerAgent:
    ...
    def update(self):
        ...
        self._set_requires_grad(self.value, False)
        try:
            with self._amp_context():
            ...
            self.scaler.update()
            self._set_requires_grad(self.value, True)
    ...
    def _actor_loss(
        ...
        for _ in range(self.cfg.horizon): # img step 
            feat = feats[-1]
            action, log_prob = self._sample_path_action(feat)
        ...
        metrics = {
            ...
            "actor_pathwise_loss": float(return_loss.detach().cpu()),
    ...
    def _sample_path_action(self, feat: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
