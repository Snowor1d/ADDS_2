...
from ADDS_AS_reinforcement import TD3Agent, ReplayBuffer, PolicyNetwork, QNetwork, ACTION_SCALE, FrameStack, FrameStack2
from config import *
import json
import ast
...
class FightingModel(Model):
    ...
    def step(self):
        ...
        if self.robot_version == 'Q':
            if self.using_model and is_boundary:
                ...
                # ---------------------------------------------------------
                action, _ = self.td3_agent.select_action(
                    ego_state, glob_state, robot_state, deterministic=True
                )
    ...
    def use_model(self, file_path):
        ...
        self.td3_agent = TD3Agent(input_shape, start_epsilon=0)
        if (USING_TRAINED_MODEL):
            self.td3_agent.load_model(file_path)
        self.using_model = True
        self.td3_agent.policy.eval()
    ...
    #     return: (2,) in [-2,2]
    #     """
    #     device = next(self.td3_agent.policy.parameters()).device  # policy가 올라간 디바이스
    ...
    #     with torch.no_grad():
    #         action = self.td3_agent.policy.action(ego_t, glob_t, robot_t)
