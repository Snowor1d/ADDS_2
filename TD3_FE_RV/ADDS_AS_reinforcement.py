...
def initialize_log_dirs():
    """Create training output directories without import-time side effects."""
            ...
            for step in range(max_steps):
                ...
                if step % ACTION_SCALE == 0:
                    ...
                    if np.random.rand() < eps or policy is None:
                        ...
                        with torch.no_grad():
                            action_t = policy.action(ego_t, glob_t, robot_t)
                        action_np = action_t.cpu().numpy()[0]
                        action_np = action_np + np.random.normal(
                            0.0, TD3_EXPLORATION_NOISE, size=action_np.shape
                        )
                        action_np = np.clip(
                            action_np, TD3_ACTION_LOW, TD3_ACTION_HIGH
                        ).astype(np.float32)
                        dx = action_np[0]
                        dy = action_np[1]
...
class PolicyNetwork(nn.Module):
    ...
    def action(self, ego_state, global_state, robot_state=None):
        """Return the deterministic TD3 action in the environment range."""
        mean = self.forward(ego_state, global_state, robot_state)
        return 4.0 * torch.sigmoid(mean) - 2.0
...
class TD3Agent:
    def __init__(
        ...
        input_shape=(50, 50),
        gamma=GAMMA_START,
        target_polyak=TD3_TARGET_POLYAK,
        lr=1e-4,
        batch_size=64,
        replay_size=int(1e5),
        device="cpu",
        ...
        if self.policy_delay <= 0:
            raise ValueError("TD3 policy_delay must be positive")
        ...
        # Target BatchNorm buffers are copied explicitly by soft_update, so
        # target forward passes must not mutate them.
        self.q1_target.eval()
        self.q2_target.eval()
...
# ------------------------------------------------- #
    ...
    def soft_update(self, net, net_target):
        with torch.no_grad():
            for param, target_param in zip(net.parameters(), net_target.parameters()):
                target_param.mul_(self.target_polyak)
                target_param.add_(param, alpha=1.0 - self.target_polyak)
            # BatchNorm running statistics are buffers rather than parameters.
            # Copy them from the online network instead of letting target
            # forward passes update them from individual replay minibatches.
            for buffer, target_buffer in zip(net.buffers(), net_target.buffers()):
    ...
    def select_action(self, ego_state_np, global_state_np, robot_state_np=None,
        ...
        """Select a deterministic TD3 action, with noise only for exploration."""
        ...
        with torch.no_grad():
            action_t = self.policy.action(ego_t, global_t, robot_t)
        action_np = action_t.cpu().numpy()[0]
        if not deterministic:
            action_np = action_np + np.random.normal(
                0.0, self.exploration_noise, size=action_np.shape
            )
            action_np = np.clip(
                action_np, TD3_ACTION_LOW, TD3_ACTION_HIGH
            ).astype(np.float32)
    ...
    def update(self):
        ...
        with torch.no_grad():
            next_a = self.policy_target.action(next_ego, glob_s2, robot_s2)
            target_noise = torch.randn_like(next_a) * self.target_policy_noise
        ...
        policy_updated = self.total_updates % self.policy_delay == 0
        if policy_updated:
            # TD3 updates the deterministic actor less frequently than the
            # critics, using Q1 only.  Q1 is frozen but remains differentiable
            # with respect to the action produced by the policy.
            q1_was_training = self.q1.training
            self.q1.eval()
            for parameter in self.q1.parameters():
                parameter.requires_grad_(False)
            try:
                new_a = self.policy.action(curr_ego, glob_s, robot_s)
                policy_loss = -self.q1(
                    curr_ego, glob_s, new_a, robot_s
                ).squeeze(-1).mean()
    ...
    def load_model(self, filepath):
        ...
        try:
            if ckpt.get('algorithm') != 'TD3_FE_RV':
                raise ValueError(
                    "Checkpoint is not a TD3_FE_RV checkpoint."
                )
...
def _build_zero_shot_frames(env_model):
    """Build the same ego/global observations used by TD3 worker processes."""
...
def evaluate_zero_shot_once(
    ...
    """Run one TD3 zero-shot episode and return evacuation_time_100."""
    if robot_num != 1:
        raise ValueError(
            "TD3_FE_RV supports one robot; ZSG_ROBOT_NUM must contain only 1."
        )
...
if __name__ == "__main__":
        ...
        with open(epsilon_path, "r") as f:
            try:
                ...
                if lines:
                    ...
                    print("Empty start_epsilon.txt. Resetting value.")
                    start_epsilon = START_EPSILON
            except ValueError:
                print("Invalid value in start_epsilon.txt. Resetting value.")
                start_epsilon = START_EPSILON
    else:
        start_epsilon = START_EPSILON
    ...
    agent = TD3Agent(
        ...
        replay_size=int(BUFFER_SIZE),
        device=DEVICE,
    )
    print(
        f"TD3 agent initialized, lr={LR}, batch_size={BATCH_SIZE}, "
        f"replay_size={BUFFER_SIZE}, policy_delay={TD3_POLICY_DELAY}"
    )
    ...
    if model_load == 1:
        ...
        model_name = "td3_checkpoint_ep_200.pth"
        ...
        model_files = [f for f in os.listdir(log_dir) if f.startswith("td3_checkpoint") and f.endswith(".pth")]
        if model_files:
    ...
    while global_episode < max_episodes:
            ...
            if global_episode >= START_UPDATE_EPISODE:
                ...
                while pending_updates >= 1.0:
                    ...
                    if (
                        ...
                        and agent.total_updates % TD3_TENSORBOARD_LOG_INTERVAL == 0
                    ):
                        for name, value in update_metrics.items():
                            if np.isfinite(value):
                                step_writer.add_scalar(
                                    f"TD3/{name}", value, agent.total_updates
                                )
                        step_writer.add_scalar(
                            "TD3/replay_size",
                        ...
                        step_writer.add_scalar(
                            "TD3/epsilon", agent.epsilon, agent.total_updates
                        )
        ...
        while True:
            ...
            # gamma/epsilon schedules are episode based, matching SAC_FE_RV3.
            ...
            if (global_episode >= START_UPDATE_EPISODE) and (global_episode % 100 == 0):
                model_filename = os.path.join(log_dir, f"td3_checkpoint_ep_{global_episode}.pth")
