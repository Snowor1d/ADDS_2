...
class SpatioContextualAttention(nn.Module):
    ...
    def forward(self, global_2d, context_1d):
        ...
        # 요소별 곱셈 후 채널 차원(dim=1)으로 Sum -> 내적(Dot Product) 연산과 동일
        # score shape: (B, 1, H, W)
        score = torch.sum(k_2d * q_2d_expanded, dim=1, keepdim=True) 
        
        # 차원의 제곱근으로 스케일링 (안정적인 학습을 위해 매우 중요!)
        score = score / (self.embed_dim ** 0.5)
        
        # Sigmoid를 통해 0~1 사이의 Attention Mask 생성
        spatial_mask = torch.sigmoid(score) 
...
class CentralizedAttentionQNetwork(nn.Module):
    ...
    def forward(self, joint_ego_state, global_state, joint_action, joint_robot_state, joint_mask=None):
        ...
        z = torch.cat([agent_embed_updated, glob_embed], dim=-1) # (B, N, embed_dim * 2)
...
class PolicyNetwork(nn.Module):
    def __init__(self, ego_shape=(25,25), global_shape=(50,50), robot_dim = 3, use_robot: bool = True):
        ...
        self.ego_to_glob_film = nn.Sequential(
            nn.Linear(self.context_dim, 128),
            nn.SiLU(),
            nn.Linear(128, 256*2)
        )

        nn.init.zeros_(self.ego_to_glob_film[-1].weight)
        nn.init.zeros_(self.ego_to_glob_film[-1].bias)
    ...
    def backbone(self, ego_state, global_state, robot_state=None):
        ...
        # 4. FiLM 적용
        gamma_beta = self.ego_to_glob_film(ego_context)
        gamma, beta = gamma_beta.chunk(2, dim=-1)
        g_1d_modulated = (1.0 + gamma) * g_1d + beta

        # 5. Final Concat & Feed Forward
        combined = torch.cat([ego_context, g_1d_modulated], dim=1)
