...
def curriculum_level(episode: int, curriculum: Sequence[int] = (500, 1000, 2000)) -> int:
    """
    curriculum = [t1, t2, ...] 일 때
    - episode <= t1  -> 1
    - t1 < episode <= t2 -> 2
    - ...
    - episode > t_last -> len(curriculum)+1
    """
    for i, t in enumerate(curriculum, start=1):
        if episode <= t:
            return i
...
if __name__ == "__main__":
    ...
    def start_workers(ctx, n_workers, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed):
        workers = [None] * n_workers
        param_queues = [None] * n_workers
        for wid in range(n_workers):
            p, pq = start_one_worker(ctx, wid, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed)
            workers[wid] = p
            param_queues[wid] = pq
        return workers, param_queues

    def start_one_worker(ctx, wid, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed):
        pq = ctx.Queue(maxsize=1)
        p = ctx.Process(
            target=worker_process,
            args=(wid, transition_queue, stats_queue, epsilon_shared, pq, global_ep_shared, base_seed),
            daemon=True
        )
        p.start()
    ...
    def restart_worker(ctx, wid, workers, param_queues, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed):
        old_p = workers[wid]
        try:
    ...
    def supervise_workers(ctx, workers, param_queues, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed):
    ...
    global_ep_shared = ctx.Value("i", int(global_episode))
    ...
    workers, param_queues = start_workers(ctx, current_n_workers, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed)
    ...
    while global_episode < max_episodes:
        ...
        if time.time() - last_supervise_t > 10.0 :
            supervise_workers(ctx, workers, param_queues, transition_queue, stats_queue, epsilon_shared, global_ep_shared, base_seed)
            last_supervise_t = time.time()
        ...
        if needs_switch_workers:
            ...
            workers, param_queues = start_workers(
                ...
                epsilon_shared, global_ep_shared, base_seed
            )
