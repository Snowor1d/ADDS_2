...
CURRICULUM_DIR = os.path.join(log_dir, "curriculum")
os.makedirs(CURRICULUM_DIR, exist_ok=True)

CURRICULUM_STATE_PATH = os.path.join(CURRICULUM_DIR, "curriculum_state.pkl")
CURRICULUM_DIFFICULTY_FILE = os.path.join(CURRICULUM_DIR, "difficulty.txt")
CURRICULUM_AVG_FILE = os.path.join(CURRICULUM_DIR, "avg_evacuation_100.txt")


def is_random_map_curriculum_enabled() -> bool:
    """
    MAP_NUM_RANDOM == [0]이면 random map curriculum learning 모드.
    그 외에는 기존처럼 지정 map 학습 모드.
    """
    return (
        isinstance(MAP_NUM_RANDOM, (list, tuple))
        and len(MAP_NUM_RANDOM) == 1
        and int(MAP_NUM_RANDOM[0]) == 0
    )


def clamp_difficulty(level: int) -> int:
    return int(max(MIN_LEVEL, min(MAX_LEVEL, int(level))))


class CurriculumManager:
    """
    Random-map curriculum manager.

    - 최근 CHECK_AVERAGE_FOR_STEPS개 episode의 evacuation_time_100 평균을 계산
    - 평균 evacuation_time_100 <= CURRICULUM_LEARNING[current_level] 이면 다음 difficulty로 상승
    - 실패 episode는 evacuation_time_100 == MAX_STEPS로 들어오므로 평균을 자동으로 악화시킴
    """
    def __init__(self):
        self.enabled = is_random_map_curriculum_enabled()
        self.current_level = clamp_difficulty(MIN_LEVEL)
        self.recent_evac100 = deque(maxlen=int(CHECK_AVERAGE_FOR_STEPS))
        self.last_avg = None

    def update(self, evac_time_100: int, abnormal: int):
        if not self.enabled:
            return False, self.current_level, self.last_avg

        if abnormal == 1:
            return False, self.current_level, self.last_avg

        evac_time_100 = int(evac_time_100)
        self.recent_evac100.append(evac_time_100)

        if len(self.recent_evac100) < int(CHECK_AVERAGE_FOR_STEPS):
            self.last_avg = float(np.mean(self.recent_evac100))
            return False, self.current_level, self.last_avg
        ...
        threshold = CURRICULUM_LEARNING.get(self.current_level, None)
        if threshold is None:
            return False, self.current_level, self.last_avg

        if self.last_avg <= float(threshold):
            old_level = self.current_level
            self.current_level = clamp_difficulty(self.current_level + 1)
            self.recent_evac100.clear()

            print(
                f"[Curriculum] Level up: {old_level} -> {self.current_level} "
                f"(avg_evac100={self.last_avg:.2f}, threshold={threshold})"
            )
            return True, self.current_level, self.last_avg

        return False, self.current_level, self.last_avg

    def save(self, path: str = CURRICULUM_STATE_PATH):
        state = {
            "enabled": self.enabled,
            "current_level": self.current_level,
            "recent_evac100": list(self.recent_evac100),
            "last_avg": self.last_avg,
        }
        with open(path, "wb") as f:
            pickle.dump(state, f)

    def load(self, path: str = CURRICULUM_STATE_PATH):
        if not os.path.exists(path):
            return
        ...
        self.current_level = clamp_difficulty(state.get("current_level", MIN_LEVEL))
        self.recent_evac100.clear()
        for v in state.get("recent_evac100", []):
            self.recent_evac100.append(int(v))
        self.last_avg = state.get("last_avg", None)

        print(
            f"[Curriculum] Loaded: enabled={self.enabled}, "
            f"current_level={self.current_level}, "
            f"recent_count={len(self.recent_evac100)}, "
            f"last_avg={self.last_avg}"
        )
...
def worker_process(
    ...
    while True:
        ...
        random_curriculum = is_random_map_curriculum_enabled()
        if random_curriculum:
            with difficulty_shared.get_lock():
                current_difficulty = int(difficulty_shared.value)
            selected_model_num = -1 
        else:
            current_difficulty = MIN_LEVEL
            selected_model_num = int(random.choice(MAP_NUM_RANDOM)) 
    
        env_model = model.FightingModel(
            number_of_agents,
            MAP_W,
            MAP_H, 
            model_num=selected_model_num,
            robot='Q',
                ...
                for i, rb in enumerate(robots_padded):
                    ...
                    if needs_new or is_finished:
                        ...
                        if np.random.rand() < eps:
                            ...
                            h = np.random.randint(0, int(HEADING_BINS))
                            coarse_theta = 2.0 * np.pi * h / float(HEADING_BINS)

                            offset = np.random.uniform(
                                -HEADING_OFFSET_MAX_RAD,
                                HEADING_OFFSET_MAX_RAD
                            )

                            theta = coarse_theta + offset
                            ...
                            action_i = np.array([dist, dx, dy, spd], dtype=np.float32)
                        else:
        ...
        stat_msg = EpisodeStatMsg(
            ...
            is_random_curriculum=bool(random_curriculum)
        )
        try:
...
class PolicyNetwork(nn.Module):
    """
    Waypoint-style actor.

    Internal action distribution
    ----------------------------
    1) heading_idx ~ Categorical(softmax(heading_logits))
    2) For every heading bin, predict offset, distance, and speed distributions.
    3) Select the components corresponding to heading_idx.

    Returned environment action
    ---------------------------
    action = [distance, dx, dy, speed]

    This keeps ACTION_DIM=4 and avoids changing critic/replay-buffer format.
    The heading index itself is not stored; it is only used to construct
    the final waypoint direction.
    """
    def __init__(self, ego_shape=(25, 25), global_shape=(50, 50), robot_dim=3, use_robot: bool = True):
        ...
        self.offset_max_rad = float(HEADING_OFFSET_MAX_RAD)
        self.gumbel_tau = float(HEADING_GUMBEL_TAU)

        # Coarse heading angles: 0, 30, 60, ... for HEADING_BINS=12.
        heading_angles = torch.linspace(
            0.0,
            2.0 * np.pi,
            steps=self.heading_bins + 1,
            dtype=torch.float32,
        )[:-1]
        self.register_buffer("heading_angles", heading_angles)
        ...
        fusion_dim = self.context_dim + 256
        ...
        # 1) Categorical coarse heading distribution over all headings.
        self.heading_logits = nn.Linear(64, self.heading_bins)

        # 2) Heading-conditioned continuous refinements.
        # Each head outputs one value per heading bin.
        self.offset_mean = nn.Linear(64, self.heading_bins)
        self.offset_log_std = nn.Linear(64, self.heading_bins)

        self.dist_mean = nn.Linear(64, self.heading_bins)
        self.dist_log_std = nn.Linear(64, self.heading_bins)

        self.spd_mean = nn.Linear(64, self.heading_bins)
        self.spd_log_std = nn.Linear(64, self.heading_bins)
    ...
    def backbone(self, ego_state, global_state, robot_state=None):
        ...
        if self.use_robot_state:
            ...
            r = torch.zeros(e.size(0), 0, device=e.device, dtype=e.dtype)

        ego_context = self.ego_context_fc(torch.cat([e, r], dim=1))

        g_2d = self.glob_enc(global_state, return_2d=True)
        attended_g_2d, _ = self.spatial_attention(g_2d, ego_context)

        g_1d_flat = attended_g_2d.reshape(attended_g_2d.size(0), -1)
        g_1d = self.global_1d_fc(g_1d_flat)
    ...
    def forward(self, ego_state, global_state, robot_state=None):
        ...
        heading_logits = self.heading_logits(feat)  # (B, H)

        offset_m = self.offset_mean(feat).unsqueeze(-1)  # (B, H, 1)
        offset_s = torch.clamp(
            self.offset_log_std(feat),
            self.log_std_min,
            self.log_std_max
        ).unsqueeze(-1)

        dist_m = self.dist_mean(feat).unsqueeze(-1)  # (B, H, 1)
        dist_s = torch.clamp(
            self.dist_log_std(feat),
            self.log_std_min,
            self.log_std_max
        ).unsqueeze(-1)

        spd_m = self.spd_mean(feat).unsqueeze(-1)  # (B, H, 1)
        spd_s = torch.clamp(
            self.spd_log_std(feat),
            self.log_std_min,
            self.log_std_max
        ).unsqueeze(-1)

        return heading_logits, offset_m, offset_s, dist_m, dist_s, spd_m, spd_s

    def _sample_with_tanh(self, m, s, low, high):
        """
        Reparameterized tanh-squashed Gaussian sample.

        m/s: (B, H, 1)
        returns:
            act:  (B, H, 1), scaled to [low, high]
            logp: (B, H)
        """
        std = s.exp()
    ...
    def _mode_with_tanh(self, m, low, high):
        z = torch.tanh(m)
        return low + 0.5 * (high - low) * (z + 1.0)

    def _build_action_from_components(self, heading_onehot, offset, dist, spd):
        """
        heading_onehot: (B, H)
        offset/dist/spd: (B, H, 1)

        return:
            action: (B, 4) = [distance, dx, dy, speed]
        """
        B = heading_onehot.size(0)
        H = heading_onehot.size(1)

        coarse = self.heading_angles.view(1, H, 1).to(offset.device)
        theta = coarse + offset

        dx = torch.cos(theta)
        dy = torch.sin(theta)

        candidates = torch.cat([dist, dx, dy, spd], dim=-1)  # (B, H, 4)
        action = (heading_onehot.unsqueeze(-1) * candidates).sum(dim=1)
    ...
    def sample_action(self, ego_state, global_state, robot_state=None):
        (
            ...
            dist_m, dist_s,
            spd_m, spd_s,
        ) = self.forward(ego_state, global_state, robot_state)

        # Categorical heading selection.
        # hard=True이면 실제 실행은 one-hot heading 하나를 사용.
        # gradient는 straight-through 방식으로 흐름.
        heading_onehot = F.gumbel_softmax(
            heading_logits,
            tau=self.gumbel_tau,
            hard=True,
            dim=-1,
        ...
        log_heading_probs = F.log_softmax(heading_logits, dim=-1)
        heading_logp = (heading_onehot * log_heading_probs).sum(dim=-1)  # (B,)

        # 모든 heading에 대해 offset / distance / speed를 예측한 뒤,
        # 선택된 heading의 값만 사용한다.
        offset, offset_logp_all = self._sample_with_tanh(
            offset_m,
            offset_s,
            -self.offset_max_rad,
            self.offset_max_rad,
        ...
        dist, dist_logp_all = self._sample_with_tanh(
            dist_m,
            dist_s,
            R_MIN,
            R_MAX,
        ...
        spd, spd_logp_all = self._sample_with_tanh(
            spd_m,
            spd_s,
            SPD_MIN,
            SPD_MAX,
        ...
        action = self._build_action_from_components(
            heading_onehot,
            offset,
            dist,
            spd,
        ...
        offset_logp = (heading_onehot * offset_logp_all).sum(dim=-1)
        dist_logp = (heading_onehot * dist_logp_all).sum(dim=-1)
        spd_logp = (heading_onehot * spd_logp_all).sum(dim=-1)

        total_logp = heading_logp + offset_logp + dist_logp + spd_logp
    ...
    def deterministic_action(self, ego_state, global_state, robot_state=None):
        ...
        heading_idx = heading_logits.argmax(dim=-1)
        heading_onehot = F.one_hot(
            heading_idx,
            num_classes=self.heading_bins
        ).float()

        offset = self._mode_with_tanh(
            offset_m,
            -self.offset_max_rad,
            self.offset_max_rad,
        )

        dist = self._mode_with_tanh(dist_m, R_MIN, R_MAX)
        spd = self._mode_with_tanh(spd_m, SPD_MIN, SPD_MAX)
...
class SACAgent:
    def __init__(self, input_shape=(50,50), gamma=GAMMA_START, alpha=0.2, tau=0.995, lr=1e-4, batch_size=64, replay_size=int(1e5), device="cpu", start_epsilon = 1.0, start_epsilon_long = 0.1, long_epsilon_min=0):
        ...
        self.target_entropy = -(3.0 + float(np.log(HEADING_BINS)))
    ...
    def select_action(self, ego_state_np, global_state_np, robot_state_np=None, deterministic=False):
        if EXPLORATION_TYPE == 0:
            if np.random.rand() < self.epsilon:
                ...
                return np.array([dist, dx, dy, spd], dtype=np.float32), True
        ...
        with torch.no_grad():
            if deterministic:
                ...
                action_t, _ = self.policy.sample_action(
                    ego_t,
                    global_t,
                    robot_t
                )
        ...
        print(
            f"Action: dist={action_np[0]:.2f}, "
            f"dx={action_np[1]:.2f}, "
            f"dy={action_np[2]:.2f}, "
            f"spd={action_np[3]:.2f}"
        )
...
if __name__ == "__main__":
    ...
    def start_workers(ctx, n_workers, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed):
        workers = [None] * n_workers
        param_queues = [None] * n_workers
        for wid in range(n_workers):
            p, pq = start_one_worker(ctx, wid, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed)
            workers[wid] = p
            param_queues[wid] = pq
        return workers, param_queues

    def start_one_worker(ctx, wid, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed):
        pq = ctx.Queue(maxsize=1)
        p = ctx.Process(
            target=worker_process,
            args=(wid, transition_queue, stats_queue, epsilon_shared, difficulty_shared, pq, base_seed),
            daemon=True
        )
        p.start()
    ...
    def restart_worker(ctx, wid, workers, param_queues, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed):
        old_p = workers[wid]
        try:
    ...
    def supervise_workers(ctx, workers, param_queues, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed):
        # 주기적으로 호출해서 죽은 worker만 재시작
        for wid, p in enumerate(workers):
            if p is None:
                restart_worker(ctx, wid, workers, param_queues, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed)
                continue
    ...
    curriculum_manager.load()
    difficulty_shared = mp.Value('i', int(curriculum_manager.current_level))

    print(
        f"[Curriculum] enabled={curriculum_manager.enabled}, "
        f"initial_difficulty={curriculum_manager.current_level}, "
        f"MIN_LEVEL={MIN_LEVEL}, MAX_LEVEL={MAX_LEVEL}"
    )
    ...
    workers, param_queues = start_workers(ctx, current_n_workers, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed)
    ...
    while global_episode < max_episodes:
        ...
        if time.time() - last_supervise_t > 10.0 :
            supervise_workers(ctx, workers, param_queues, transition_queue, stats_queue, epsilon_shared, difficulty_shared, base_seed)
            last_supervise_t = time.time()
        ...
        while True:
            ...
            step_writer.add_scalar("Curriculum/Difficulty", int(s_msg.difficulty), global_episode)
            step_writer.add_scalar("Curriculum/IsRandomCurriculum", int(s_msg.is_random_curriculum), global_episode)
            ...
            # --------------------------------------------------
            if curriculum_manager.enabled and s_msg.abnormal != 1:
                level_changed, new_level, curr_avg = curriculum_manager.update(
                    evac_time_100=s_msg.evac_time_100,
                    abnormal=s_msg.abnormal
                )

                with difficulty_shared.get_lock():
                    difficulty_shared.value = int(new_level)

                # TensorBoard
                step_writer.add_scalar(
                    "Curriculum/CurrentDifficulty",
                    int(new_level),
                    global_episode
                )
                ...
                if curr_avg is not None:
                    step_writer.add_scalar(
                        "Curriculum/AvgEvacuation100ForLevelCheck",
                        float(curr_avg),
                        global_episode
                    )

                # txt 저장
                ensure_file(CURRICULUM_DIFFICULTY_FILE)
                with open(CURRICULUM_DIFFICULTY_FILE, "a") as f:
                    f.write(f"{global_episode}\t{int(new_level)}\n")

                ensure_file(CURRICULUM_AVG_FILE)
                with open(CURRICULUM_AVG_FILE, "a") as f:
                    avg_to_write = -1.0 if curr_avg is None else float(curr_avg)
                    f.write(f"{global_episode}\t{avg_to_write:.6f}\n")

                if level_changed:
                    print(
                        f"[Curriculum] episode={global_episode}, "
                        f"difficulty -> {new_level}, "
                        f"avg_evac100={curr_avg:.2f}"
                    )
