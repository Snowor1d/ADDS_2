...
def action_to_env(raw_action: torch.Tensor, cfg: DreamerConfig) -> torch.Tensor:
    if cfg.action_dim == 4:
        radius = cfg.r_min + 0.5 * (cfg.r_max - cfg.r_min) * (raw_action[..., 0:1] + 1.0)
        direction = raw_action[..., 1:3].clamp(-1.0, 1.0)
        speed = cfg.spd_min + 0.5 * (cfg.spd_max - cfg.spd_min) * (raw_action[..., 3:4] + 1.0)
        return torch.cat([radius, direction, speed], dim=-1)
...
def env_to_raw_action(action: torch.Tensor, cfg: DreamerConfig) -> torch.Tensor:
    if cfg.action_dim == 4:
        radius_scale = max(cfg.r_max - cfg.r_min, 1e-6)
        speed_scale = max(cfg.spd_max - cfg.spd_min, 1e-6)
        radius = 2.0 * (action[..., 0:1] - cfg.r_min) / radius_scale - 1.0
        direction = action[..., 1:3]
        speed = 2.0 * (action[..., 3:4] - cfg.spd_min) / speed_scale - 1.0
        return torch.cat([radius, direction, speed], dim=-1).clamp(-1.0, 1.0)
...
class WorldModel(nn.Module):
    ...
    def loss(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, float], list[RSSMState]]:
        ...
        reward_loss_per = self.reward_head.loss(flat, batch["reward"].reshape(b * t), reduction="none").reshape(b, t)
        reward_loss = masked_mean(reward_loss_per)
        delta_t = batch.get("delta_t", torch.ones_like(batch["continue"])).clamp_min(1.0)
        continue_target = batch["continue"] * torch.pow(
            torch.as_tensor(self.cfg.discount, device=delta_t.device, dtype=delta_t.dtype),
            delta_t,
        )
        ...
        continue_loss = masked_mean(continue_loss_per)
        ...
        robot_loss_per = ((robot_pred - robot_target).pow(2) * robot_mask).sum(dim=(2, 3))
        robot_loss_per = robot_loss_per / (robot_mask.sum(dim=(2, 3)) * self.cfg.robot_dim).clamp_min(1.0)
        ego_loss = masked_mean(ego_loss_per)
        global_loss = masked_mean(global_loss_per)
        robot_loss = masked_mean(robot_loss_per)
