...
class DreamerAgent:
    ...
    def load_replay_buffer(self, filepath: str) -> None:
        if os.path.exists(filepath):
            ...
            if loaded and not self._replay_action_shape_matches():
                print(
                    f"[DreamerAgent] replay buffer action shape is incompatible with "
                    f"action_dim={self.cfg.action_dim}; starting with an empty replay buffer."
                )
                self.replay.episodes.clear()
                self.replay.current_episode = []
                self.replay.num_steps = 0
                loaded = False
            if loaded:
                print(
    ...
    def _replay_action_shape_matches(self) -> bool:
        expected = (self.cfg.max_robots, self.cfg.action_dim)
    ...
    def _actor_loss(
        ...
        with torch.no_grad():
            ...
            weights = torch.cumprod(
                torch.cat([torch.ones_like(continue_seq[:1]), continue_seq[:-1]], dim=0),
                dim=0,
            )
            weights = weights * (mask.sum(dim=-1) > 0).float().reshape(1, -1)
        denom = weights.sum().clamp_min(1.0)
        return_loss = -((weights * normed_returns).sum() / denom)
        entropy_loss = self.cfg.entropy_scale * ((weights * log_prob_seq).sum() / denom)
    ...
    def _replay_value_loss(
        ...
        with torch.no_grad():
            ...
            delta_bt = batch.get("delta_t", torch.ones_like(batch["continue"])).clamp_min(1.0)
            continue_bt = batch["continue"] * torch.pow(
                torch.as_tensor(self.cfg.discount, device=delta_bt.device, dtype=delta_bt.dtype),
                delta_bt,
            )
            returns = []
            feats = []
