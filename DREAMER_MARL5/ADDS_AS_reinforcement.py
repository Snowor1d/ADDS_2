...
def clip_robot_action(action: np.ndarray) -> np.ndarray:
    action = np.asarray(action, dtype=np.float32).copy()
    if action.shape[-1] != ACTION_DIM:
        raise ValueError(f"Robot action must have {ACTION_DIM} dims, got shape {action.shape}")
    if ACTION_DIM == 4:
        action[0] = np.clip(action[0], R_MIN, R_MAX)
        action[1] = np.clip(action[1], -1.0, 1.0)
        action[2] = np.clip(action[2], -1.0, 1.0)
        action[3] = np.clip(action[3], SPD_MIN, SPD_MAX)
        return action
    return np.clip(action, SPD_MIN, SPD_MAX).astype(np.float32)
    ...
    while True:
        ...
        try:
            ...
            for step in range(MAX_STEPS):
                ...
                # C. 로봇별 이벤트 체크: 최초/도착/충돌/종료 때만 이전 waypoint를 닫고 새 waypoint를 샘플링한다.
                sampled_joint_action = None
                updated_any_action = False
                for i, rb in enumerate(robots_padded):
                    ...
                    if needs_new or is_finished or global_done:
                    ...
                    if global_done or is_finished or not needs_new:
                        continue
                    ...
                    if np.random.rand() < eps:
                        action_i = np.array([
                            ...
                            np.random.uniform(-1.0, 1.0),
                            np.random.uniform(SPD_MIN, SPD_MAX),
                        ], dtype=np.float32)
                    else:
                        if sampled_joint_action is None:
                            sampled_joint_action = worker_agent.select_joint_action(
                                curr_joint_ego,
                                curr_glob_state,
                                joint_robot_state,
                                joint_mask,
                        ...
                        action_i = sampled_joint_action[i].astype(np.float32)

                    action_i = clip_robot_action(action_i)
                    rb.receive_action_from_policy(action_i)
                    rb.new_order_need = False
                    current_executing_actions[i] = action_i
                    updated_any_action = True
...
def evaluate_zero_shot_once(
    ...
    for step in range(MAX_STEPS):
        ...
        for i, rb in enumerate(robots_padded):
            ...
            if sampled_joint_action is None:
                sampled_joint_action = agent.select_joint_action(
                    curr_joint_ego,
                    curr_glob_state,
                    joint_robot_state,
                    joint_mask,
            ...
            action_i = clip_robot_action(sampled_joint_action[i].astype(np.float32))
            rb.receive_action_from_policy(action_i)
            rb.new_order_need = False
            current_executing_actions[i] = action_i
            updated_any_action = True
        ...
        if updated_any_action:
            agent.set_policy_prev_action(current_executing_actions.astype(np.float32))
...
if __name__ == "__main__":
    ...
    while global_episode < max_episodes:
        ...
        try:
            ...
            seq_key = (msg.worker_id, msg.agent_index)
            seq = dreamer_episode_buffers.setdefault(seq_key, [])
            pending_msg = dreamer_pending_transitions.pop(seq_key, None)
            ...
            if msg.done:
                ...
                dreamer_episode_buffers[seq_key] = []
            else:
                dreamer_pending_transitions[seq_key] = msg
        ...
        while True:
            ...
            worker_seq_keys = [
                key for key in list(dreamer_episode_buffers.keys())
                if key[0] == s_msg.worker_id
            ]
            for seq_key in worker_seq_keys:
                seq = dreamer_episode_buffers.get(seq_key)
                if not seq:
                    continue
                pending_msg = dreamer_pending_transitions.pop(seq_key, None)
                if pending_msg is not None:
