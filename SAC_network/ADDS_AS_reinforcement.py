...
##########################################################################
class ResidualBlockP(nn.Module):
    """3×3 ×2 + skip (BatchNorm + LeakyReLU). in = out."""
    def __init__(self, c):
        super().__init__()
    ...
    def forward(self, x):
        return F.leaky_relu(self.net(x) + x, 0.01)

class PolicyNetwork(nn.Module):
    """
    - Conv stem (wider) : 1→64→128 (stride 2)  ➜ 25*25→13*13
    - Residual stage    : 128 channels, 2 blocks
    - Down-sample extra : 128→256 (stride 2)   ➜ 7*7
    - Residual stage    : 256 channels, 2 blocks
    - FC : 784→512→256→64 → (μ, log σ)
    """
    def __init__(self, input_shape=(50, 50),
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(1, 64, 5, 2, 2, bias=False),  # 50×50 → 25×25
            nn.BatchNorm2d(64),
            nn.LeakyReLU(0.01, True),

            nn.Conv2d(64, 128, 3, 2, 1, bias=False),  # 25×25 → 13×13
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.01, True),
        )
        ...
        # ─── Down-sample & residual @256 channels ───────────────────────────
        self.reduce = nn.Sequential(
            nn.Conv2d(128, 256, 3, 2, 1, bias=False),  # 13×13 → 7×7
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.01, True),
        )
        ...
        with torch.no_grad():
            ...
            feat_dim = prod(self._forward_conv(dummy).shape[1:])  # 256×7×7 = 12544
    ...
    def forward(self, s):
        ...
        log_std = torch.clamp(self.log_std_head(feat),
