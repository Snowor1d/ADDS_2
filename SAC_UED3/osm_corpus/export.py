...
def export_row(row: dict, crowd_size: Optional[int] = None) -> Optional[dict]:
    ...
    # Crowd is a density on the walkable ground, the same definition the
    # generated levels use; see crowd_density.py. Scaling with crop area
    # instead, as this did before, ignores built coverage, and coverage runs
    # from 0.33 to 0.72 across this corpus: the same headcount per hectare is
    # a quiet street in Palermo Soho and a crush in Shibuya.
    #
    # Seeded per site and size so re-exporting the corpus does not quietly
    # produce a different set of levels.
    import random as _random

    from crowd_density import crowd_size_for_level

    walkable_rings = [[tuple(p) for p in ring] for ring in rings]
    density_rng = _random.Random(abs(hash((row["site"], row["size_m"], "crowd")))
                                 % (1 << 30))
    derived, realised = crowd_size_for_level(size_m, size_m, walkable_rings,
                                             rng=density_rng)
    if crowd_size is None:
        crowd_size = derived
    else:
        from crowd_density import realised_density, walkable_area
        realised = realised_density(
            int(crowd_size), walkable_area(size_m, size_m, walkable_rings))
    ...
    return {
        ...
        # What the level actually runs at, which is below the target wherever
        # CROWD_SIZE_LIMIT clamps a large crop.
        "crowd_density": round(float(realised), 5),
        "stats": row.get("stats"),
    }
...
def load_levels(path: Optional[str] = None) -> List:
    ...
    for d in payload["levels"]:
        ...
        # Where on earth this crop is. The index has always carried it; the
        # Level did not, so nothing downstream could fetch the map tiles for
        # the place it came from.
        lv.lat = d.get("lat")
        lv.lon = d.get("lon")
        lv.city = d.get("city")
        lv.country = d.get("country")
        out.append(lv)
    return out
