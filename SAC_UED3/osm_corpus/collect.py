...
def collect(keys: Optional[Sequence[str]] = None,
    ...
    for key in keys:
        ...
        for size in crop_sizes:
            ...
            try:
                ...
                row["unbuilt_blocked"] = getattr(crop, "unbuilt_blocked", None)
    ...
    # Survey completeness, judged once per site and applied to all its sizes.
    #
    # A property of the place, not of one crop: how much of it OpenStreetMap
    # records at all. The ratio moves a great deal with crop size and not in a
    # consistent direction, so a per-crop verdict would cut single sizes out of
    # otherwise sound sites and the size axis would then mean something
    # different at each site, which is what the size-generalisation holdout
    # exists to measure.
    _apply_survey_verdicts(rows)
    ...
    payload = {
        ...
        "traversability": mode,
        "simplify_m": simplify_m,
        "unbuilt_blocked_threshold": __import__("config").OSM_MAX_UNBUILT_BLOCKED,
...
def _apply_survey_verdicts(rows: List[dict]) -> None:
    """Reject whole sites OpenStreetMap does not record well enough.

    Measured on each site's widest crop, the one the threshold was fitted on,
    and applied to every size that site has. Rows are edited in place.
    """
    from shapely.geometry import Polygon

    from config import OSM_MAX_UNBUILT_BLOCKED
    from osm_corpus.extract import (UNBUILT_BLOCKED_REFERENCE_SIZE, Crop,
                                    unbuilt_blocked_share)
    ...
    for site, site_rows in sorted(by_site.items()):
        usable = [r for r in site_rows
                  if r.get("rejected") is None and r.get("rings")]
        if not usable:
            continue
        ref = min(usable,
                  key=lambda r: (abs(float(r["size_m"])
                                     - UNBUILT_BLOCKED_REFERENCE_SIZE),
                                 -float(r["size_m"])))
        polys = [Polygon(x) for x in ref["rings"] if len(x) >= 3]
        polys = [p if p.is_valid else p.buffer(0) for p in polys]
        polys = [p for p in polys
                 if p.geom_type == "Polygon" and not p.is_empty]
        crop = Crop(site_key=site, size_m=float(ref["size_m"]),
                    lat=ref["lat"], lon=ref["lon"], polygons=polys,
                    source=ref.get("source", ""))
        share = unbuilt_blocked_share(crop, ref["lat"], ref["lon"],
                                      float(ref["size_m"]))
        for row in site_rows:
            row["unbuilt_blocked"] = share
            row["unbuilt_blocked_ref_size_m"] = float(ref["size_m"])
            if share is not None and share > OSM_MAX_UNBUILT_BLOCKED:
                row["rejected"] = (
                    f"{share:.2f} of the blocked area holds no building at "
                    f"{int(float(ref['size_m']))} m, above "
                    f"{OSM_MAX_UNBUILT_BLOCKED}: OpenStreetMap does not record "
                    f"enough of this place for the road network to describe it")
                row["rings"] = None
                row["stats"] = None
        if share is not None:
            mark = "REJECT" if share > OSM_MAX_UNBUILT_BLOCKED else "keep"
            print(f"  {site:24s} survey {share:.3f} at "
                  f"{int(float(ref['size_m']))} m  {mark}")
