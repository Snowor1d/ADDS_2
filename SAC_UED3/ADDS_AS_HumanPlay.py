...
from scenario import level_from_config
import robot_action
...
############################
# 실험/기록 파라미터  (모두 config.py에서 온다)
############################
# The level comes from SIM_SOURCE and the SIM_* settings, the same way the
# viewer and training get theirs. This script used to pick map 128 by hand and
# set its own MAX_STEPS of 3000, so it was playing a different scenario under a
# different time limit from everything else, and a numbered map has boundary
# exits and no hazard: it was the task this project replaced.
SCENARIO_LEVEL, SCENARIO_TAG = level_from_config()

ROBOT_VERSION_FOR_MODEL = 'Q' # 모델에는 'Q'로 넘기되, 사람이 직접 action 전달
ROBOT_VERSION_FOR_LOG   = 'H' # 결과 폴더명에는 'H'로 기록해 비교군 명확화
EXP_NAME = HUMANPLAY_EXP_NAME
MAX_EPISODES = HUMANPLAY_EPISODES

# The world to play in: the level's own size when there is one, and the
# MAP_W/MAP_H fallback only for numbered maps.
WORLD_W = int(SCENARIO_LEVEL.width) if SCENARIO_LEVEL is not None else int(MAP_W)
WORLD_H = int(SCENARIO_LEVEL.height) if SCENARIO_LEVEL is not None else int(MAP_H)

############################
# 상단 설정 (시뮬/렌더 타이밍)
############################
TARGET_SIM_FPS    = HUMANPLAY_SIM_FPS
TARGET_RENDER_FPS = HUMANPLAY_RENDER_FPS
...
############################
SCREEN_WIDTH, SCREEN_HEIGHT = HUMANPLAY_SCREEN

# The map is fitted to the panel rather than scaled by a fixed cell size: a
# 400 m crop at 7.5 pixels per metre is 3000 pixels wide and did not fit on
# the screen at all.
PANEL_RIGHT_WIDTH = HUMANPLAY_PANEL_WIDTH   # 우측 HUD/조이스틱 패널 폭
PADDING = 10              # 좌측 여백
_MAP_BOX = min(SCREEN_WIDTH - PANEL_RIGHT_WIDTH - 2 * PADDING,
               SCREEN_HEIGHT - 2 * PADDING)
CELL_SIZE = _MAP_BOX / float(max(WORLD_W, WORLD_H))
MAP_CW = WORLD_W * CELL_SIZE
MAP_CH = WORLD_H * CELL_SIZE
...
log_dir = os.path.join(home_dir, f"learning_log_{EXP_NAME}")
...
CONT_VIS_MODE = HUMANPLAY_VIS_MODE
CONT_SAVE_EVERY = HUMANPLAY_SAVE_EVERY
CONT_FPS = HUMANPLAY_MP4_FPS
CONT_OUT_DPI = HUMANPLAY_DPI
CONT_BITRATE = HUMANPLAY_BITRATE
...
def list_test_dirs(map_id, robot_ver: str):
    base = os.path.join(result_root(), f"Result_{map_id}", f"Result_{map_id}_{robot_ver}")
    if not os.path.isdir(base):
        return [], base
    pattern = re.compile(rf"^Result_{re.escape(str(map_id))}_{robot_ver}_(\d+)$")
    subdirs = []
...
def aggregate_over_existing_tests(map_id, robot_ver: str):
...
def get_next_test_index(map_id, robot_ver: str):
    test_dirs, base = list_test_dirs(map_id, robot_ver)
    if not test_dirs:
        return 0
    nums = []
...
# 그리기 유틸
############################
# Which way each key points, in world axes. Screen y is inverted when the
# joystick is read, and these are already in world orientation.
_SIGNAL_KEYS = {
    pygame.K_w: (0.0, 1.0),
    pygame.K_s: (0.0, -1.0),
    pygame.K_a: (-1.0, 0.0),
...
#    replay_buffer = ReplayBuffer(capacity=int(1e5))
    ...
    # 폴더 기본 구조(상위/중간) 준비
    root = result_root()
    map_dir = os.path.join(root, f"Result_{SCENARIO_TAG}")
    ensure_dir(map_dir)
    map_robot_dir = os.path.join(map_dir, f"Result_{SCENARIO_TAG}_{ROBOT_VERSION_FOR_LOG}")
    ensure_dir(map_robot_dir)

    # What the robot is signalling, held across episodes so the operator does
    # not have to set it again after every reset.
    signal_mode = "guide"
    signal_dir = (1.0, 0.0)
    ...
    while running and episode_count < MAX_EPISODES:
        ...
        if SCENARIO_LEVEL is not None:
            # The level carries the crowd size, the world size and the robot
            # count. Taking them from constants is how this script came to run
            # 30 pedestrians on a crop sized for hundreds.
            env_model = FightingModel(
                number_agents=int(SCENARIO_LEVEL.crowd_size),
                width=WORLD_W,
                height=WORLD_H,
        ...
        state = np.array(env_model.return_current_image(H=WORLD_H, W=WORLD_W),
                         dtype=np.float32)
        ...
        while running and not done:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # The signal is the other half of the action, so it needs
                    # its own controls. 1/2/3 pick the mode; WASD point the
                    # signalled heading and switch to "direct", because
                    # pointing is the gesture that means "go that way".
            ...
            while sim_acc >= sim_dt and not done:
                robot_action.apply_to(
                    env_model.robot,
                    robot_action.encode((env_dx, env_dy), signal_mode,
                                        signal_dir))
            ...
            if do_render:
                ...
                if signal_mode == "direct":
                    _arrow = f"  ({signal_dir[0]:+.0f}, {signal_dir[1]:+.0f})"
                draw_text(screen, f"Signal: {signal_mode}{_arrow}",
                          txt_x, 60, color=(0, 0, 160), font_size=22)
                draw_text(screen, "1 off  2 guide  3 direct  WASD point",
                          txt_x, 135, color=(0, 0, 160), font_size=16)
        ...
        if done:
            ...
            # 1) 세부 폴더 결정/생성 (먼저!)
            test_i = get_next_test_index(SCENARIO_TAG, ROBOT_VERSION_FOR_LOG)
            test_dirname = f"Result_{SCENARIO_TAG}_{ROBOT_VERSION_FOR_LOG}_{test_i}"
            ...
            # 4) 집계
            aggregate_over_existing_tests(SCENARIO_TAG, ROBOT_VERSION_FOR_LOG)
