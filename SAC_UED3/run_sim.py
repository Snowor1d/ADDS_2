...
MODEL_NAME = "FE_1000_to_1299_5000ep.pth"
...
# ====== ENV / POLICY ======
# Set by --real to watch a level from the OSM corpus instead of a numbered
# map. A real crop carries its own world size and crowd count, so those
# arguments are taken from the level rather than from the sliders.
REAL_LEVEL = None
BASEMAP_SURFACE = None


from scenario import (attach_hazard, build_curriculum_level,
...
def _parse_args():
    ...
    ap.add_argument("--numbered", action="store_true",
                    help="the hand-made maps in map_infos (the old exit task)")
    ap.add_argument("--generated", action="store_true",
                    help=argparse.SUPPRESS)   # old name for --numbered
    args = ap.parse_args()
    if args.generated:
        args.numbered = True
    return args
...
if __name__=="__main__":
    ...
    if source == "curriculum":
        ...
        print("[run_sim] numbered maps from map_infos. These carry boundary "
              "exits and no hazard, so this is the task that was replaced; "
              "set SIM_SOURCE = \"curriculum\" to watch the current one.")
