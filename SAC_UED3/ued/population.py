...
# The headcount at which a level is running below its target density because
# the simulation-cost clamp bound, logged so a run cannot drift into "every
# level is capped" without that being visible.
_CROWD_CAP = int(CROWD_SIZE_LIMIT[1])
...
class LevelPopulation:
    ...
    def stats(self) -> Dict[str, float]:
        with self._lock:
            ...
            cd = [r.level.crowd_density for r in records
                  if r.level.crowd_density is not None]
            out = {
                ...
                # Realised congestion, which is the quantity held constant
                # across map sizes; mean_crowd_size on its own moves with the
                # size axis and says nothing about how crowded a level is.
                "mean_crowd_density": (sum(cd) / len(cd)) if cd else 0.0,
                "frac_crowd_clamped": (sum(1 for r in records
                                           if r.level.crowd_size >= _CROWD_CAP)
                                       / len(records)),
                "n_bred": self.n_bred,
