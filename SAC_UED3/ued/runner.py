...
class _EpisodeTrace:
    __slots__ = ("ego", "glob", "robot", "mask", "action", "n_seen", "ret",
                 "level_id", "touched")

    def __init__(self):
        # Joint tensors now: the critic values a team's situation, so a
        # trajectory sample has to carry the team, including which slots held
        # real robots.
...
class UEDRunner:
    ...
    def on_transition(self, msg):
        ...
        if len(trace.ego) < MAXMC_SAMPLES:
            trace.ego.append(msg.joint_ego_state)
            trace.glob.append(msg.global_state)
            trace.robot.append(msg.joint_robot_state)
            trace.mask.append(msg.joint_mask)
            trace.action.append(msg.joint_action)
        else:
            ...
            if j < MAXMC_SAMPLES:
                trace.ego[j] = msg.joint_ego_state
                trace.glob[j] = msg.global_state
                trace.robot[j] = msg.joint_robot_state
                trace.mask[j] = msg.joint_mask
                trace.action[j] = msg.joint_action
    ...
    def _maxmc(self, trace: _EpisodeTrace) -> Optional[float]:
        ...
        try:
            values = self.value_fn(
                ...
                np.asarray(trace.mask, dtype=np.float32),
            )
        except Exception as e:
...
def make_value_fn(agent):
    ...
    def value_fn(ego: np.ndarray, glob: np.ndarray, robot: np.ndarray,
                 mask: np.ndarray) -> np.ndarray:
        """One value per sampled instant, averaged over the real robots.

        The critic returns a value per robot, and MaxMC needs one number for
        the state. The team average is the right reduction because the reward
        it is compared against is the team's: a two-robot instant and a
        three-robot one then sit on the same scale, which matters because team
        size is a curriculum axis and levels are ranked against each other.
        """
        device = agent.device
        with torch.no_grad():
            ...
            B, N = mask_t.shape
            flat_ego = ego_t.reshape(B * N, *ego_t.shape[2:])
            flat_glob = glob_t.repeat_interleave(N, dim=0)
            flat_robot = robot_t.reshape(B * N, -1)
            action, log_prob = agent.policy.sample_action(
                flat_ego, flat_glob, flat_robot)
            action = action.reshape(B, N, -1)
            log_prob = log_prob.reshape(B, N)

            q1 = agent.q1(ego_t, glob_t, action, robot_t, mask_t).squeeze(-1)
            q2 = agent.q2(ego_t, glob_t, action, robot_t, mask_t).squeeze(-1)
            alpha = agent.alpha.to(device) if hasattr(agent.alpha, "to") else agent.alpha
            v = (torch.min(q1, q2) - alpha * log_prob) * mask_t
            denom = mask_t.sum(dim=1).clamp(min=1.0)
            return (v.sum(dim=1) / denom).detach().cpu().numpy()
