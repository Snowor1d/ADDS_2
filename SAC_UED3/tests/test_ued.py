...
class _FakeMsg:
    def __init__(self, worker_id, episode_idx, level_id, reward, done=False):
        ...
        # Joint tensors: a transition is the whole team's instant now, so the
        # trace carries one row per robot slot plus the mask saying which of
        # them held a real robot.
        from config import MAX_ROBOTS

        n = MAX_ROBOTS
        self.joint_ego_state = np.full((n, 4, 4, 4), float(worker_id),
                                       dtype=np.float32)
        self.global_state = np.full((4, 4, 4), float(worker_id),
                                    dtype=np.float32)
        self.joint_robot_state = np.full((n, 3), float(worker_id),
                                         dtype=np.float32)
        from robot_action import ACTION_DIM

        self.joint_action = np.zeros((n, ACTION_DIM), dtype=np.float32)
        self.joint_mask = np.zeros((n,), dtype=np.float32)
        self.joint_mask[:2] = 1.0
...
class MaxMCTest(unittest.TestCase):
    ...
    def _runner(self, value=0.0):
        ...
        def value_fn(ego, glob, robot, mask):
            # One value per instant, already reduced over the team, which is
            # what the real critic's wrapper returns.
...
class SizeGeneralisationTest(unittest.TestCase):
    ...
    def test_robot_state_carries_a_symmetric_scale(self):
        ...
        for size in (60, 100, 160):
            ...
            from ADDS_AS_reinforcement import ROBOT_STATE_DIM
            self.assertEqual(len(state), ROBOT_STATE_DIM, f"size {size}")
...
class NetworkRobotDimTest(unittest.TestCase):
    ...
    def test_networks_match_the_configured_robot_state_width(self):
        ...
        # The actor still has a plain robot branch.
        widths = [p.shape[1] for n, p in agent.policy.named_parameters()
                  if n.endswith("robot_fc.0.weight")]
        self.assertTrue(widths, "policy has no robot branch")
        self.assertEqual(widths[0], trainer.ROBOT_STATE_DIM)

        # The critics are centralised and take the robot state together with
        # the action, so their first layer is robot_dim + action_dim wide.
        for name, net in (("q1", agent.q1), ("q2", agent.q2)):
            widths = [p.shape[1] for n, p in net.named_parameters()
                      if n.endswith("robot_action_enc.0.weight")]
            self.assertTrue(widths, f"{name} has no robot branch")
            # The critic's robot branch takes the state and the action
            # together, so it widens with the action too.
            self.assertEqual(widths[0],
                             trainer.ROBOT_STATE_DIM + trainer.ACTION_DIM,
                             name)
    ...
    def test_value_fn_runs_on_a_real_observation_width(self):
        ...
        batch, N = 4, MAX_ROBOTS
        mask = np.zeros((batch, N), dtype=np.float32)
        mask[:, :2] = 1.0
        values = value_fn(
            np.zeros((batch, N, 4, EGO_MAP_SIZE, EGO_MAP_SIZE), dtype=np.float32),
            np.zeros((batch, 4, DOWNSAMPLE_MAP_SIZE, DOWNSAMPLE_MAP_SIZE), dtype=np.float32),
            np.zeros((batch, N, trainer.ROBOT_STATE_DIM), dtype=np.float32),
            mask,
        )
...
class DangerZoneTest(unittest.TestCase):
    ...
    def test_nobody_is_removed_from_the_simulation(self):
        """The crowd count is constant, which is what lets progress reverse."""
        import model as M
    ...
    def test_occupancy_is_a_live_count(self):
        ...
        for step in range(200):
            ...
            # The margin band belongs to neither, so safe + inside <= total.
            self.assertLessEqual(inside + safe, total)
...
class MultiRobotTest(unittest.TestCase):
    ...
    def test_observation_carries_the_hazard(self):
        ...
        for i in range(len(fm.robots)):
            st = fm.return_current_robot_state(i)
            self.assertEqual(len(st), ROBOT_STATE_DIM)
            signed, occupancy = st[5], st[6]
        ...
        # Robots in different places must not report the same observation.
        if len(fm.robots) > 1:
            self.assertNotEqual(fm.return_current_robot_state(0)[:2],
                                fm.return_current_robot_state(1)[:2])
    ...
    def test_critic_is_order_equivariant_and_respects_the_mask(self):
        """What lets one policy handle teams of different sizes."""
        import torch

        import ADDS_AS_reinforcement as R

        q = R.CentralizedAttentionQNetwork(
            ego_shape=(R.EGO_MAP_SIZE, R.EGO_MAP_SIZE),
            global_shape=(R.DOWNSAMPLE_MAP_SIZE, R.DOWNSAMPLE_MAP_SIZE),
            action_dim=2, robot_dim=R.ROBOT_STATE_DIM,
            max_robots=R.MAX_ROBOTS, use_robot=True)
        q.eval()
        B, N = 2, R.MAX_ROBOTS
        torch.manual_seed(0)
        je = torch.randn(B, N, 4, R.EGO_MAP_SIZE, R.EGO_MAP_SIZE)
        gs = torch.randn(B, 4, R.DOWNSAMPLE_MAP_SIZE, R.DOWNSAMPLE_MAP_SIZE)
        ja = torch.randn(B, N, 2)
        jr = torch.randn(B, N, R.ROBOT_STATE_DIM)
        mask = torch.zeros(B, N)
        mask[:, :2] = 1.0

        with torch.no_grad():
            out = q(je, gs, ja, jr, mask)
            perm = torch.tensor([1, 0] + list(range(2, N)))
            out_perm = q(je[:, perm], gs, ja[:, perm], jr[:, perm], mask[:, perm])
            # A padded slot's contents must not reach a real robot's value.
            jr_noise = jr.clone()
            jr_noise[:, 2:] = 1e3
            out_noise = q(je, gs, ja, jr_noise, mask)

        self.assertLess((out[:, 0] - out_perm[:, 1]).abs().max().item(), 1e-4)
        self.assertLess((out[:, :2] - out_noise[:, :2]).abs().max().item(), 1e-4)

    def test_a_ctde_update_runs(self):
        """Joint transitions in, actor weights out."""
        import numpy as np
        import torch

        import ADDS_AS_reinforcement as R

        ag = R.SACAgent(batch_size=4, replay_size=200, device="cpu")
        N = R.MAX_ROBOTS

        def sample(n_real):
            m = np.zeros(N, dtype=np.float32)
            m[:n_real] = 1.0
            return (np.random.rand(N, 4, R.EGO_MAP_SIZE, R.EGO_MAP_SIZE).astype(np.float32),
                    np.random.rand(4, R.DOWNSAMPLE_MAP_SIZE, R.DOWNSAMPLE_MAP_SIZE).astype(np.float32),
                    np.random.rand(N, R.ROBOT_STATE_DIM).astype(np.float32),
                    np.stack([R.robot_action.random_action(random.Random(k))
                              for k in range(N)]), m)

        for i in range(40):
            n_real = 1 + i % N
            je, gs, jr, ja, m = sample(n_real)
            nje, ngs, njr, _, nm = sample(n_real)
            ag.replay_buffer.push(je, gs, jr, ja, m, nje, ngs, njr, nm,
                                  float(np.random.randn()), float(i % 7 == 0),
                                  int(np.random.randint(n_real)), 1.0)

        before = [p.detach().clone() for p in ag.policy.parameters()]
        ag.update()
        ag.update()
        changed = sum(1 for b, a in zip(before, ag.policy.parameters())
                      if not torch.equal(b, a.detach()))
        self.assertEqual(changed, len(before))
        for p in list(ag.q1.parameters()) + list(ag.q2.parameters()):
...
class CrowdFlowTest(unittest.TestCase):
    ...
    def test_inflow_is_off_and_outflow_is_on_by_default(self):
        """They default differently, for different reasons.

        Inflow is off: it adds pedestrians that were not there before, so
        turning it on silently would make every measurement taken on a closed
        box incomparable.

        Outflow is on: without it the crop boundary is a wall, and a
        pedestrian the robots have pushed toward it is pinned between the
        robot and the edge for the rest of the episode. That is an artefact of
        cropping a city, and if the spot is inside the hazard the robots can
        never clear it.
        """
        import config

        self.assertFalse(config.CROWD_ALLOW_INFLOW)
            ...
            for _ in range(CROWD_OUTFLOW_STUCK_STEPS * 2):
                for a in held:
                    if not a.dead:
                        ...
                        a.vel = [0.0, 0.0]
                fm.step()
            self.assertEqual(n0 - self._alive(fm), 3)
        finally:
            M.CROWD_ALLOW_OUTFLOW = original
...
class RotatedZoneTest(unittest.TestCase):
    ...
    def test_a_street_hazard_takes_the_street_s_angle_and_width(self):
        ...
        from citygen.generate import generate_city_plan
        from danger import sample_along_street
    ...
    def test_walking_along_the_edge_is_not_leaving(self):
        ...
        try:
            ...
            for _ in range(CROWD_OUTFLOW_STUCK_STEPS * 3):
                ...
                walker.xy = [2.0, y]
                fm.step()
            self.assertFalse(walker.dead,
                             "a pedestrian walking along the edge was removed")
        finally:
            M.CROWD_ALLOW_OUTFLOW = original
...
class WedgedPedestrianTest(unittest.TestCase):
    ...
    def test_an_exploration_goal_is_never_the_agents_own_position(self):
        ...
        offenders = []
        for a in fm.crowds:
            if a.dead or a.type != 1 or not a.now_goal:
                continue
    ...
    def test_a_wedged_pedestrian_eventually_moves(self):
        ...
        frozen = [uid for uid, d in far.items() if d < 0.2]
        self.assertEqual(frozen, [],
                         f"{len(frozen)} pedestrians never moved at all")
...
class CrowdDensityTest(unittest.TestCase):
    ...
    def test_walkable_area_excludes_blocks_and_clips_overhang(self):
        from crowd_density import walkable_area
    ...
    def test_size_changes_the_crowd_and_not_the_congestion(self):
        from crowd_density import crowd_size_for, realised_density
...
class PhysicsCalibrationTest(unittest.TestCase):
    ...
    def test_the_clearance_grid_never_changes_an_answer(self):
        """The fast path in is_free_point is a filter, not an approximation."""
        import model as M
...
class RobotSignalTest(unittest.TestCase):
    ...
    def test_the_policy_emits_a_mode_and_the_gradient_reaches_it(self):
        ...
        from ADDS_AS_reinforcement import (ACTION_DIM, ROBOT_STATE_DIM,
                                           PolicyNetwork)
        from config import DOWNSAMPLE_MAP_SIZE, EGO_MAP_SIZE, ROBOT_MODES

        pol = PolicyNetwork(ego_shape=(EGO_MAP_SIZE, EGO_MAP_SIZE),
                            global_shape=(DOWNSAMPLE_MAP_SIZE,
                                          DOWNSAMPLE_MAP_SIZE),
                            robot_dim=ROBOT_STATE_DIM)
        b = 4
        ego = torch.randn(b, 4, EGO_MAP_SIZE, EGO_MAP_SIZE)
        glob = torch.randn(b, 4, DOWNSAMPLE_MAP_SIZE, DOWNSAMPLE_MAP_SIZE)
        rs = torch.randn(b, ROBOT_STATE_DIM)
        action, logp = pol.sample_action(ego, glob, rs)
