            ...
            for step in range(max_steps):
                ...
                if step % ACTION_SCALE == 0:
                    ...
                    if np.random.rand() < eps or policy is None:
                        ...
                        with torch.no_grad():
                            action_t, _ = policy.sample_action(ego_t, glob_t, None, temperature=1.0)
...
class QNetwork(nn.Module):
    def __init__(self, ego_shape=(50, 50), global_shape=(50, 50), action_dim=2, robot_dim = 0, use_robot: bool = True):
...
class PolicyNetwork(nn.Module):
    def __init__(self, ego_shape=(50,50), global_shape=(50,50), robot_dim = 0, use_robot: bool = True):
...
# ------------------------------------------------- #
    ...
    # Select action
    # ------------------------------------------------- #
    def select_action(self, ego_state_np, global_state_np, deterministic=False):
        """
        ...
        with torch.no_grad():
            if deterministic:
                ...
                mean, _ = self.policy.forward(ego_t, global_t, None)
                action_t = 4*torch.sigmoid(mean)-2
            else:
                # 비결정적 선택: sample_action에서 샘플링 (자코비안 보정 포함)
                action_t, log_prob = self.policy.sample_action(ego_t, global_t, None)
