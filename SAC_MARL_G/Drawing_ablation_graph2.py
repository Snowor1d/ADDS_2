...
"""
Full-model Map-count Comparison Plotter
- 동일한 Full model(Ours)에 대해 3/30/100/300maps 학습 결과를 비교
- Nested Folder Structure 지원 (Map count -> Ours -> Target -> N개의 txt 파일)
- 다중 시드(Multiple txt files) 병합: 평균(Mean) 및 음영(Shade - Std, Min-Max, SE) 지원
- 네 설정의 eva100s 곡선을 하나의 figure에 생성
...
# 비교할 map 수와 동일한 Full-model 폴더
MAP_SERIES = ["3maps", "30maps", "100maps", "300maps"]
FULL_MODEL_FOLDER = "Ours"
GRAPH_TARGETS = ["eva100s"]
OUTPUT_NAME = "full_model_map_comparison"
...
SERIES_STYLES = {
    "3 maps":   {"color": "#3769f3", "ls": "-",  "lw": 4.5, "marker": "o"},
    "30 maps":  {"color": "#f04a4a", "ls": "--", "lw": 4.5, "marker": "s"},
    "100 maps": {"color": "#e27227", "ls": "-.", "lw": 4.5, "marker": "^"},
    "300 maps": {"color": "#723e16", "ls": ":",  "lw": 4.5, "marker": "D"},
}
...
def map_display_name(map_dir: str) -> str:
    return map_dir.replace("maps", " maps")
...
def load_ablation_data(
    ...
    # txt 파일들 전부 읽기
    txt_files = sorted(f for f in os.listdir(target_dir) if f.endswith(".txt"))
    if not txt_files:
        return None
    ...
    return AggregatedSeries(
        name=display_name or ablation, x=x, raw_mean_y=clip_arr(raw_mean_y),
...
def create_comparison_plot(dict_series: Dict[str, List[AggregatedSeries]]):
    # 만약 타겟들 중 데이터가 하나라도 없으면 결합 그래프 생성 취소
    if not all(t in dict_series and dict_series[t] for t in GRAPH_TARGETS):
        return

    figure_size = FIGSIZE_SINGLE if len(GRAPH_TARGETS) == 1 else FIGSIZE_COMBINED
    fig, axes = plt.subplots(1, len(GRAPH_TARGETS), figsize=figure_size)
    ...
    out_path = os.path.join(OUT_DIR, f"{OUTPUT_NAME}.{SAVE_FORMAT}")
...
def main():
    ...
    print(
        f"[INFO] Comparing {FULL_MODEL_FOLDER}: "
        + ", ".join(map_display_name(map_name) for map_name in MAP_SERIES)
    )
    dict_series = {}

    for target in GRAPH_TARGETS:
        series_list = []
        for map_name in MAP_SERIES:
            data = load_ablation_data(
                map_name,
                FULL_MODEL_FOLDER,
                target,
                display_name=map_display_name(map_name),
            )
            if data is not None:
                series_list.append(data)
            else:
                print(f"[WARN] No data found for {map_name}/{FULL_MODEL_FOLDER}/{target}")

        if series_list:
            dict_series[target] = series_list
        else:
            print(f"[WARN] No Full-model data found for target: {target}")

    # 하나의 축에 map 수별 네 곡선을 그린다.
    create_comparison_plot(dict_series)
