...
def save_debug_triplet(save_dir: str,
                       worker_id: int,
                       episode_idx: int,
                       step: int,
                       agent_idx: int,
    ...
    """
    Save debug images for:
      - current full / ego / global maps
      - each object channel separately
      - stacked states (time x object channel)
    ...
    CHANNEL_NAMES = ["obstacle", "exit", "crowd", "robot"]
    OBS_CHANNELS = len(CHANNEL_NAMES)
    ...
    def save_single_channel_set(prefix: str, arr: np.ndarray):
        """
        arr:
          - (H,W)
          - (C,H,W)
        Save each channel separately and also a horizontal grid overview.
        """
        arr = np.asarray(arr)
        arr_u8 = to_uint8_img(arr)

        if arr_u8.ndim == 2:
            imageio.imwrite(str(d / f"{prefix}.png"), arr_u8)
            return

        if arr_u8.ndim != 3:
            raise ValueError(f"Unsupported shape for save_single_channel_set: {arr_u8.shape}")
        ...
        for ci in range(c):
            ...
            ch_img = arr_u8[ci]
            imageio.imwrite(str(d / f"{prefix}_{ch_name}.png"), ch_img)
            tiles.append(ch_img)

        grid = np.concatenate(tiles, axis=1)
        imageio.imwrite(str(d / f"{prefix}_grid.png"), grid)

    def save_stacked_state(prefix: str, arr: np.ndarray):
        """
        arr:
          - expected (T*OBS_CHANNELS, H, W), e.g. (16,H,W)
          - or (T,H,W) for old style
        Save:
          - each frame/channel separately
          - per-time grid
          - whole stacked grid
        """
        if arr is None:
        ...
        if arr_u8.ndim != 3:
            raise ValueError(f"Unsupported stacked state shape: {arr_u8.shape}")

        c, h, w = arr_u8.shape

        # case 1: new format (T * OBS_CHANNELS, H, W)
        if c % OBS_CHANNELS == 0:
            t_len = c // OBS_CHANNELS
            arr_reshaped = arr_u8.reshape(t_len, OBS_CHANNELS, h, w)

            all_rows = []
            for t in range(t_len):
                row_tiles = []
                for oc in range(OBS_CHANNELS):
                    ch_name = CHANNEL_NAMES[oc]
                    img = arr_reshaped[t, oc]

                    imageio.imwrite(
                        str(d / f"{prefix}_t{t}_{ch_name}.png"),
                        img
                    )
                    row_tiles.append(img)

                row_grid = np.concatenate(row_tiles, axis=1)
                imageio.imwrite(str(d / f"{prefix}_t{t}_grid.png"), row_grid)
                all_rows.append(row_grid)

            big_grid = np.concatenate(all_rows, axis=0)
            imageio.imwrite(str(d / f"{prefix}_all_grid.png"), big_grid)
            return

        # case 2: old format (T,H,W)
        row_tiles = []
        for t in range(c):
            img = arr_u8[t]
            imageio.imwrite(str(d / f"{prefix}_t{t}.png"), img)
            row_tiles.append(img)

        grid = np.concatenate(row_tiles, axis=1)
        imageio.imwrite(str(d / f"{prefix}_grid.png"), grid)

    # -------------------------------------------------
    # 1) current full / global / ego
    # -------------------------------------------------
    save_single_channel_set(f"{step_stem}_full", full_u8)
    save_single_channel_set(f"{step_stem}_glob", glob_f)
    save_single_channel_set(f"{agent_stem}_ego", ego_f)

    # -------------------------------------------------
    # 2) stacked states
    # -------------------------------------------------
    save_stacked_state(f"{step_stem}_globStack", global_state)
    save_stacked_state(f"{agent_stem}_egoStack", ego_state)
...
def random_permute_joint_batch(
    ...
    """
    joint_ego:       (B, N, 16, E, E)
    joint_robot:     (B, N, R)
    joint_action:    (B, N, A)
    joint_mask:      (B, N)
    next_joint_ego:  (B, N, 16, E, E)
...
def ego_crop_from_full_map(full_map: np.ndarray,
    ...
    return:
        (ego_size, ego_size) or (C, ego_size, ego_size)
    """

    if full_map.ndim == 2:
        H, W = full_map.shape
        is_channel_map = False
    elif full_map.ndim == 3:
        C, H, W = full_map.shape
        is_channel_map = True
    else:
        raise ValueError(f"Unexpected full_map shape: {full_map.shape}")
    ...
    crop = np.full((C, ego_size, ego_size), pad_value, dtype=full_map.dtype)
    crop[:, dy0:dy0 + (sy1 - sy0), dx0:dx0 + (sx1 - sx0)] = full_map[:, sy0:sy1, sx0:sx1]
    return crop
...
def downsample_full_map(full_map: np.ndarray, target: int) -> np.ndarray:
    ...
    return:
        (target, target) or (C, target, target)
    """

    x = torch.from_numpy(full_map).float()
...
class FrameStackWithStep:
    ...
    def update(self, frame):
        ...
        return:
            frame이 (H,W)이면      -> (k,H,W)
            frame이 (C,H,W)이면    -> (k*C,H,W)
        """
        self.history.append(np.copy(frame))
        ...
        # channel frame: (k,C,H,W) -> (k*C,H,W)
        if stacked.ndim == 4:
            k, c, h, w = stacked.shape
            return stacked.reshape(k * c, h, w)
...
def worker_process(
    ...
    def _build_joint_frames(env_model, robots_padded):
        ...
        -------
        full_u8:
            (OBS_CHANNELS, MAP_H, MAP_W) uint8

        ego_frames:
            (MAX_ROBOTS, OBS_CHANNELS, EGO_MAP_SIZE, EGO_MAP_SIZE) float32 [0,1]

        glob_f:
            (OBS_CHANNELS, DOWNSAMPLE_MAP_SIZE, DOWNSAMPLE_MAP_SIZE) float32 [0,1]

        joint_robot_state:
            (MAX_ROBOTS, ROBOT_STATE_DIM) float32
        ...
        if full_u8.ndim != 3:
            raise ValueError(
                f"return_current_image() must return (C,H,W), got {full_u8.shape}"
            )

        if full_u8.shape[0] != OBS_CHANNELS:
            raise ValueError(
                f"Expected {OBS_CHANNELS} channels, got {full_u8.shape[0]}"
            )
        ...
        for i, rb in enumerate(robots_padded):
            if rb is None:
                ego_f = np.zeros(
                    (OBS_CHANNELS, EGO_MAP_SIZE, EGO_MAP_SIZE),
                    dtype=np.float32
                )
    ...
    while True:
        ...
        ego_stacks = [FrameStackWithStep(FRAME_STACK, FRAME_STEP) for _ in range(MAX_ROBOTS)]
        glob_stack = FrameStackWithStep(FRAME_STACK, FRAME_STEP)
...
class CentralizedAttentionQNetwork(nn.Module):
    def __init__(
        ...
        # --- image encoding
        self.ego_enc = CNNEncoder(input_shape=ego_shape, in_channels=INPUT_CHANNELS, compress=True)
        self.ego_to_embed = nn.Linear(self.ego_enc.out_dim, embed_dim)
        
        self.glob_enc = CNNEncoder(input_shape=global_shape, in_channels=INPUT_CHANNELS, compress=False)
...
class PolicyNetwork(nn.Module):
    ...
    def sample_action(self, ego_state, global_state, robot_state=None):
        ...
        def _sample_with_sigmoid(m, s, low, high):
            std = s.exp()
            u = m + std * torch.randn_like(m)
            sigma = torch.sigmoid(u)
            act = low + (high - low) * sigma
            # Jacobian correction for log_prob
            logp_u = -0.5 * (((u - m) / (std + 1e-8))**2 + 2*s + np.log(2*np.pi))
            jacobian = torch.log((high - low) * sigma * (1 - sigma) + 1e-8)
            logp = (logp_u - jacobian).sum(dim=-1)
            return act, logp

        # 1) r Sampling (R_MIN ~ R_MAX)
        r_act, r_logp = _sample_with_sigmoid(r_m, r_s, R_MIN, R_MAX)

        # 2) dx, dy Sampling (-1.0 ~ 1.0)
        # dx, dy는 단순히 방향 벡터이므로 범위를 -1 ~ 1로 설정합니다.
        dir_act, dir_logp = _sample_with_sigmoid(d_m, d_s, -1.0, 1.0)

        # 3) speed Sampling (SPD_MIN ~ SPD_MAX)
        spd_act, spd_logp = _sample_with_sigmoid(spd_m, spd_s, SPD_MIN, SPD_MAX)
...
class SACAgent:
    def __init__(self, input_shape=(50,50), gamma=GAMMA_START, alpha=0.2, tau=0.995, lr=1e-4, batch_size=64, replay_size=int(1e5), device="cpu", start_epsilon = 1.0, start_epsilon_long = 0.1, long_epsilon_min=0):
        ...
        self.replay_buffer = ReplayBuffer(
            ...
            max_robots=MAX_ROBOTS,
            ego_state_shape=(INPUT_CHANNELS, EGO_MAP_SIZE, EGO_MAP_SIZE),
            global_state_shape=(INPUT_CHANNELS, DOWNSAMPLE_MAP_SIZE, DOWNSAMPLE_MAP_SIZE),
            action_dim=4,
            robot_dim=self.robot_dim,
            device=self.device,
        )
