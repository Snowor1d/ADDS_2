...
from ADDS_AS_reinforcement import ActorCritic, ACTION_SCALE
...
class FightingModel(Model):
    ...
    def extract_map_50(self, map_num):
        ...
        if map_num == 0:
            ...
            self.obstacles.append([[15 * scale, 20 * scale], [25 * scale, 20 * scale], [25 * scale, 40 * scale], [15 * scale, 40 * scale]])
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def step(self):
        ...
        if(self.using_model and self.step_n%ACTION_SCALE==0):
            ...
            action, _ = self.RL_agent.select_action(state, True)
    ...
    def use_model(self, file_path):
        ...
        self.RL_agent = ActorCritic(input_shape, num_actions, start_epsilon=0)
        if (USING_TRAINED_MODEL):
            self.RL_agent.load_model(file_path)
