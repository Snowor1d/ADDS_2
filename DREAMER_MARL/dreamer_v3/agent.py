...
from .networks import Actor, RSSMState, Value, WorldModel, symlog
...
class DreamerAgent:
    ...
    def _actor_loss(
        ...
        value_seq = self.value(feat_seq)
        returns = self._lambda_returns(reward_seq, value_seq, continue_seq)
        actor_loss = -(returns + self.cfg.entropy_scale * entropy_seq).mean()
        metrics = {
