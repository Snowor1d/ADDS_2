...
log_dir = os.path.join(home_dir, "learning_log_3.1.8")
...
class QNetwork(nn.Module):
    ...
    def _get_conv_out(self, shape):
        dummy = torch.zeros(1, 4, *shape)  # (B, C, H, W) = (1,1,H,W)
        o = self.conv1(dummy)
        o = self.conv2(o)
        o = self.conv3(o)
...
if __name__ == "__main__":
    ...
    for episode in range(max_episodes):
        ...
        frames_deque = deque(maxlen=10)
        stacked_state_indices = [0, 3, 6, 9]
        total_reward = 0
        reward = 0
        ...
        for step in range(max_steps):
            ...
            if(step%3==0) and stacked_state is not None:
                action_np, _ = agent.select_action(stacked_state)
                dx, dy = action_np[0], action_np[1]
                real_action = env_model.robot.receive_action([dx, dy])
                buffered_state = stacked_state
            ...
            frames_deque.append(next_frame)

            if len(frames_deque) >= 10:
                stacked_state = np.stack([frames_deque[-(i+1)] for i in stacked_state_indices], axis=0)
            # 5) Done?
            done = (step >= max_steps-1) or (env_model.alived_agents() <= 2)

            # 6) Store transition
            if(step%3==2) and stacked_state is not None and buffered_state is not None:
        ...
        if episode == 1:
            model_filename = os.path.join(log_dir, f"sac_checkpoint_ep_1.pth")
