...
def save_debug_triplet(save_dir: str, worker_id: int, episode_idx: int, step: int,
    ...
    """
    full_u8: (H,W) uint8
    glob_f: float32 [0,1] single frame
    global_state: (4,H,W) float32 [0,1] stack (optional)
    """
    ...
    # global frame 저장 (float -> uint8 변환)
...
def worker_process(
    ...
    while True:  # 무한히 에피소드 반복
        ...
        # ----- 2) 초기 state 세팅 -----
        full_u8, glob_f = _build_ego_global_frames(env_model)
            ...
            for step in range(max_steps):
                ...
                if step % ACTION_SCALE == 0:
                    ...
                    if np.random.rand() < eps or policy is None:
                        ...
                        with torch.no_grad():
                            action_t, _ = policy.sample_action(glob_t, robot_t, temperature=1.0)
                ...
                _, glob_f2 = _build_ego_global_frames(env_model)
...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int):
        ...
        # 반환 순서: global, robot, action, reward, next_global, next_robot, done
        return (
            batch_global,
            batch_robot,
            batch_actions,
    ...
    def load(self, filepath: Union[str, bytes, os.PathLike]) -> None:
        ...
        required = ["global_states", "global_next_states",
                    ...
                    "state_dtype", "robot_dim", "global_state_shape"]
        for k in required:
...
class QNetwork(nn.Module):
    def __init__(self, global_shape=(50, 50), action_dim=2, robot_dim = 3, use_robot: bool = True):
    ...
    def forward(self,  global_state, action, robot_state=None):
        # 1. Image Features
        g = self.glob_enc(global_state)
        img = torch.cat([g], dim=1)
...
class PolicyNetwork(nn.Module):
    def __init__(self, global_shape=(50,50), robot_dim = 3, use_robot: bool = True):
        ...
        # Fusion Dimension = Global_CNN + Robot
    ...
    def backbone(self, global_state, robot_state=None):
        g = self.glob_enc(global_state)
    ...
    def forward(self, global_state, robot_state=None):
        feat = self.backbone(global_state, robot_state)
        mean = self.mean_head(feat)
        log_std = self.log_std_head(feat)
        log_std = torch.clamp(log_std, self.log_std_min, self.log_std_max)
        return mean, log_std

    def sample_action(self, global_state, robot_state=None, temperature=1.0):
        # 파라미터가 2개 이미지 입력을 받도록 수정됨
        mean, log_std = self.forward(global_state, robot_state)
        std = log_std.exp()
...
# ------------------------------------------------- #
    ...
    # Select action
    # ------------------------------------------------- #
    def select_action(self, global_state_np, robot_state_np=None, deterministic=False):
        """
        ...
        with torch.no_grad():
            if deterministic:
                ...
                mean, _ = self.policy.forward(global_t, robot_t)
                action_t = 4*torch.sigmoid(mean)-2
            else:
                # 비결정적 선택: sample_action에서 샘플링 (자코비안 보정 포함)
                action_t, log_prob = self.policy.sample_action(global_t, robot_t)
    ...
    def update(self):
        ...
        glob_s, robot_s, a, r, glob_s2, robot_s2, d = self.replay_buffer.sample(self.batch_size)

        with torch.no_grad():
            next_a, next_logp = self.policy.sample_action(glob_s2, robot_s2)
            q1_next = self.q1_target(glob_s2, next_a, robot_s2)
            q2_next = self.q2_target(glob_s2, next_a, robot_s2)
            q_next = torch.min(q1_next, q2_next).squeeze(-1)
            q_target = r + self.gamma * (1 - d) * (q_next - self.alpha * next_logp)

        # ----- Update Q1, Q2 -----
        q1_val = self.q1(glob_s, a, robot_s).squeeze(-1)  # (B,) #q value를 scalar 값으로
        q2_val = self.q2(glob_s, a, robot_s).squeeze(-1)
        ...
        new_action, log_prob = self.policy.sample_action(glob_s, robot_s)
        q1_new = self.q1(glob_s, new_action, robot_s)
        q2_new = self.q2(glob_s, new_action, robot_s)
    ...
    while global_episode < max_episodes:
        ...
        try:
            ...
            agent.replay_buffer.push(
                msg.global_state, msg.robot_state,
                msg.action, msg.reward,
                msg.next_global_state, msg.next_robot_state,
                msg.done
            )
