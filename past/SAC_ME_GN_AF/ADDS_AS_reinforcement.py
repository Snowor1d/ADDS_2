...
class QNetwork(nn.Module):
    ...
    def _get_conv_out(self, shape):
        ...
        o = F.leaky_relu(self.gn1(self.conv1(dummy)), negative_slope=0.01)
        o = F.leaky_relu(self.gn2(self.conv2(o)), negative_slope=0.01)
        o = F.leaky_relu(self.gn3(self.conv3(o)), negative_slope=0.01)
        return int(np.prod(o.size()[1:]))

    def forward(self, img, action, robot_state = None):
        x = F.leaky_relu(self.gn1(self.conv1(img)), negative_slope=0.01)
        x = F.leaky_relu(self.gn2(self.conv2(x)), negative_slope=0.01)
        x = F.leaky_relu(self.gn3(self.conv3(x)), negative_slope=0.01)
        x = x.view(x.size(0), -1)
...
class PolicyNetwork(nn.Module):
    ...
    def _get_conv_out(self, shape, robot_state = None):
        ...
        x = F.leaky_relu(self.gn1(self.conv1(dummy)), negative_slope=0.01)
        x = F.leaky_relu(self.gn2(self.conv2(x)), negative_slope=0.01)
        x = F.leaky_relu(self.gn3(self.conv3(x)), negative_slope=0.01)
        x = x.view(x.size(0), -1)
    ...
    def backbone(self, state, robot_state = None):
        x = F.leaky_relu(self.gn1(self.conv1(state)), negative_slope=0.01)
        x = F.leaky_relu(self.gn2(self.conv2(x)), negative_slope=0.01)
        x = F.leaky_relu(self.gn3(self.conv3(x)), negative_slope=0.01)
        x = x.view(x.size(0), -1)
        if self.use_robot_state:
