...
LOG_STD_MAX = 0.5
LOG_STD_MIN = -20
PPO_EPOCHS = 10              # 수집된 데이터로 네트워크를 업데이트하는 횟수
PPO_MINI_BATCH_SIZE = 256    # 한 번 업데이트할 때 사용하는 미니배치 크기
PPO_VALUE_COEF = 0.5         # Critic(Value) Loss의 가중치
PPO_ENTROPY_COEF = 0.01      # 탐험(Exploration)을 유도하는 엔트로피 가중치
PPO_MAX_GRAD_NORM = 0.5      # 그래디언트 클리핑(폭발 방지) 임계값
