...
class QNetwork(nn.Module):
    """
    개선된 QNetwork:
    - 첫 conv layer에서 stride=2를 사용해 입력 해상도를 줄임 (예: 50×50 -> 25×25)
    - ResidualBlock 3개를 사용:
        res1: in=8, out=8
        res2: in=8, out=16  (채널 수 확장)
        res3: in=16, out=16
    - 최종 feature map: (16, 25, 25) → Flatten 차원: 16 * 25 * 25 = 10000
    - action 임베딩: (2 -> 8)
    - FC: (10000+8) → 128 → 64 → 1
    """
    def __init__(self, input_shape=(50,50), action_dim=2):
        super(QNetwork, self).__init__()
        # 첫 Conv: 1→8, stride=2로 다운샘플링
        self.conv1 = nn.Conv2d(1, 8, kernel_size=3, stride=2, padding=1)
        self.bn1   = nn.BatchNorm2d(8)
        
        # Residual blocks
        self.res1 = ResidualBlock(8, 8)      # 채널 유지: 8 → 8
        self.res2 = ResidualBlock(8, 16)     # 채널 확장: 8 → 16
        self.res3 = ResidualBlock(16, 16)    # 채널 유지: 16 → 16
        
        self.bn_final = nn.BatchNorm2d(16)
        
        # conv 출력 크기 계산: (16, 25, 25) → 16*25*25 = 10000
        self.conv_out_size = self._get_conv_out(input_shape)
        
        # action 임베딩: action_dim (2) -> 8
        self.action_fc = nn.Sequential(
            nn.Linear(action_dim, 8),
            nn.ReLU()
        )
        
        # Fully Connected Layers: (10000 + 8) -> 128 -> 64 -> 1
        self.fc1 = nn.Linear(self.conv_out_size + 8, 128)
        self.fc2 = nn.Linear(128, 64)
        self.q_out = nn.Linear(64, 1)
    
    def _get_conv_out(self, shape):
        dummy = torch.zeros(1, 1, *shape)  # (1,1,H,W)
        x = F.relu(self.bn1(self.conv1(dummy)))  # 해상도: H/2 x W/2 (예: 25x25)
        x = self.res1(x)     # (8, 25, 25)
        x = self.res2(x)     # (16, 25, 25)
        x = self.res3(x)     # (16, 25, 25)
        x = F.relu(self.bn_final(x))
        return int(np.prod(x.size()[1:]))  # 16 * 25 * 25 = 10000
    
    def forward(self, state, action):
        x = F.relu(self.bn1(self.conv1(state)))  # (B,8,25,25)
        x = self.res1(x)
        x = self.res2(x)
        x = self.res3(x)
        x = F.relu(self.bn_final(x))
        x = x.view(x.size(0), -1)  # Flatten → (B, 10000)
        a_emb = self.action_fc(action)  # (B, 8)
        x = torch.cat([x, a_emb], dim=1)  # (B, 10008)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        return self.q_out(x)
...
class PolicyNetwork(nn.Module):
    def __init__(self, input_shape=(50,50)):
        ...
        # conv1: stride=2로 다운샘플링
        # => (1 -> 16 채널), (50x50 -> 25x25)
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, stride=2, padding=1)
        self.bn1   = nn.BatchNorm2d(16)
        
        # Residual blocks: stride=1, 채널 확장
    ...
    def _get_conv_out(self, shape):
        ...
        x = F.relu(self.bn1(self.conv1(dummy)))  # (1->16, stride=2) => (16, 25, 25)
        x = self.res1(x)                         # (16, 25, 25)
        x = self.res2(x)                         # (32, 25, 25)
        x = self.res3(x)                         # (32, 25, 25)
        x = F.relu(self.bn_final(x))             # (32, 25, 25)
        return int(np.prod(x.size()[1:]))        # 32 * 25 * 25 = 20000

    def backbone(self, state):
        x = F.relu(self.bn1(self.conv1(state)))  # stride=2 => (B,16,25,25)
        x = self.res1(x)                         # (16,25,25)
        x = self.res2(x)                         # (32,25,25)
        x = self.res3(x)                         # (32,25,25)
        x = F.relu(self.bn_final(x))
        x = x.view(x.size(0), -1)                # (B, 20000)
        feat = self.fc_backbone(x)               # (B, 64)
        return feat

    def forward(self, state):
        feat = self.backbone(state)              # (B,64)
        mean = self.mean_head(feat)             # (B,2)
        log_std = self.log_std_head(feat)       # (B,2)
        # clamp
    ...
    def sample_action(self, state, temperature=1.0):
        B = state.size(0)
        mean, log_std = self.forward(state)      # (B,2), (B,2)
        std = log_std.exp()
