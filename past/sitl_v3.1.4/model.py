...
class FightingModel(Model):
    ...
    def return_current_image(self):
        # 각 객체별로 독립적인 채널 생성
        wall_channel = [[0 for _ in range(self.width)] for _ in range(self.height)]
        exit_channel = [[0 for _ in range(self.width)] for _ in range(self.height)]
        agent_channel = [[0 for _ in range(self.width)] for _ in range(self.height)]
        robot_channel = [[0 for _ in range(self.width)] for _ in range(self.height)]

        for agent in self.agents:
            if agent.type == 9:  # 벽
                wall_channel[agent.pos[0]][agent.pos[1]] = 1
            elif agent.type == 10:  # 출구
                exit_channel[agent.pos[0]][agent.pos[1]] = 1
            elif agent.type in (0, 1, 2):  # 군중
                agent_channel[int(round(agent.xy[0]))][int(round(agent.xy[1]))] = 1
            elif agent.type == 3:  # 로봇
                robot_channel[int(round(agent.xy[0]))][int(round(agent.xy[1]))] = 1

        # Combine into 3D array
        one_hot_image = [
            [  # 각 픽셀의 4채널 값
                [wall_channel[i][j], exit_channel[i][j], agent_channel[i][j], robot_channel[i][j]]
    ...
    def use_model(self, file_path):
        ...
        self.sac_agent = HybridSACAgent(input_shape, num_actions)
