...
class QNetwork(nn.Module):
    def __init__(self, input_shape=(70,70), action_dim=2, agent_feat_dim=16):
        super(QNetwork, self).__init__()

        # ① 이미지(feature) 추출 (Conv Layers)
        self.conv1 = nn.Conv2d(1, 16, kernel_size=5, stride=2)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, stride=2)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, stride=2)
        conv_out_size = self._get_conv_out(input_shape)  # conv layer 출력 차원 계산

        # ② 에이전트 좌표 처리: (x,y) → feature vector  
        self.agent_fc = nn.Sequential(
            nn.Linear(2, 16),
            nn.ReLU(),
            nn.Linear(16, agent_feat_dim),
            nn.ReLU()
        )

        # ③ 이미지 feature, 에이전트 feature, 행동(action)을 결합하여 fully connected layers에 전달  
        self.fc1 = nn.Linear(conv_out_size + agent_feat_dim + action_dim, 256)
        self.fc2 = nn.Linear(256, 128)
        self.q_out = nn.Linear(128, 1)  # 최종 Q-value
    ...
    def forward(self, state, agent_coords, action):
        """
        state: (B, 1, H, W) 텐서 (이미지 입력)
        agent_coords: (B, N, 2) 텐서 – 각 배치마다 N개의 (x,y) 좌표 (N는 에이전트 개수, 최대 100)
        action: (B, action_dim) 텐서
        """
        # 이미지 처리
        x = F.relu(self.conv1(state))
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = x.view(x.size(0), -1)  # (B, conv_out_size)

        # 에이전트 좌표 처리  
        # 에이전트 수가 0보다 크다고 가정(만약 없으면 zeros를 사용)
        if agent_coords is not None and agent_coords.size(1) > 0:
            # 각 (x,y)를 처리 → (B, N, agent_feat_dim)
            agent_features = self.agent_fc(agent_coords)
            # 예: 평균 pooling (B, agent_feat_dim)
            agent_features = agent_features.mean(dim=1)
        else:
            agent_features = torch.zeros(x.size(0), self.fc1.in_features - (x.size(1) + action.size(1))).to(x.device)

        # 이미지 feature, 에이전트 feature, 행동을 결합
        combined = torch.cat([x, agent_features, action], dim=1)
...
class PolicyNetwork(nn.Module):
    """
    에이전트의 방향(action)을 샘플링하기 위한 네트워크  
    이미지와 에이전트 좌표 정보를 모두 사용하여 정책 파라미터(평균, 로그 표준편차)를 추론합니다.
    """
    def __init__(self, input_shape=(70,70), agent_feat_dim=16):
        super(PolicyNetwork, self).__init__()
        self.log_std_min = -10
        self.log_std_max = -0.5

        # ① 이미지 처리: conv layers 및 backbone
        self.conv1 = nn.Conv2d(1, 16, kernel_size=5, stride=2)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, stride=2)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, stride=2)
        ...
        # ③ 이미지 feature와 에이전트 feature를 결합  
        # 이미지 feature: 128, 에이전트 feature: agent_feat_dim → combined feature: 128 + agent_feat_dim  
        self.fc_combined = nn.Sequential(
            nn.Linear(128 + agent_feat_dim, 128),
            nn.ReLU()
        )

        # ④ 최종적으로 정책 파라미터(평균, 로그 표준편차)를 산출
        self.mean_head = nn.Linear(128, 2)      # 2차원 행동 (예: dx, dy)
    ...
    def forward(self, state, agent_coords):
        """
        state: (B, 1, H, W) 이미지 텐서
        agent_coords: (B, N, 2) 에이전트 좌표 텐서
        """
        # 이미지로부터 feature 추출
        feat = self.backbone(state)  # (B, 128)

        # 에이전트 좌표 처리
        if agent_coords is not None and agent_coords.size(1) > 0:
            agent_features = self.agent_fc(agent_coords)  # (B, N, agent_feat_dim)
            agent_features = agent_features.mean(dim=1)   # (B, agent_feat_dim)
        else:
            agent_features = torch.zeros(feat.size(0), self.agent_fc[-1].out_features, device=feat.device)

        # 두 feature 결합
        combined = torch.cat([feat, agent_features], dim=1)  # (B, 128+agent_feat_dim)
        combined = self.fc_combined(combined)  # (B, 128)
    ...
    def sample_action(self, state, agent_coords, temperature=1.0):
        """
        state: (B, 1, H, W)
        agent_coords: (B, N, 2)
        returns:
        ...
        """
        B = state.size(0)
        mean, log_std = self.forward(state, agent_coords)
        std = log_std.exp()

        eps = torch.randn_like(mean)
        action = mean + std * eps
        action = 2 * torch.tanh(action)  # action 범위를 [-2, 2]로 스케일링

        # Gaussian의 log_prob 계산
...
# ------------------------------------------------- #
    ...
    def select_action(self, state_np, agent_coords_list, deterministic=False):
        """
        state_np: (H, W) 또는 (1, H, W) 이미지 (numpy array)
        agent_coords_list: [(x1, y1), (x2, y2), ...] – 에이전트 좌표 리스트
        """
        # 이미지 텐서로 변환: (1, 1, H, W)
        state_t = torch.FloatTensor(state_np).unsqueeze(0).unsqueeze(0).to(self.device)
        # 에이전트 좌표 리스트를 텐서로 변환: (N, 2) → (1, N, 2)
        agent_coords_t = torch.FloatTensor(agent_coords_list).unsqueeze(0).to(self.device)

        with torch.no_grad():
            mean, log_std = self.policy(state_t, agent_coords_t)
            std = log_std.exp()
            if deterministic:
                action_t = mean
            else:
