...
class ImpalaCNN(nn.Module):
    def __init__(self, input_shape, in_channels=4, channels=[32, 64, 64, 64, 64], out_dim=256):
        ...
        for c in channels:
            ...
            cur_channels = c
            # MaxPool2d(kernel=3, stride=2, padding=1) 적용 시의 공간 해상도 계산
            h = (h + 2 * 1 - 3) // 2 + 1
            w = (w + 2 * 1 - 3) // 2 + 1
...
class QNetwork(nn.Module):
    def __init__(self, ego_shape=(25, 25), global_shape=(50, 50), action_dim=2, robot_dim=3, use_robot: bool=True):
        ...
        # --- Ego & Global Encoders (IMPALA CNN) ---
        # 독립적인 가중치를 가짐
        self.ego_enc = ImpalaCNN(input_shape=ego_shape, in_channels=4, channels=[32, 64, 64, 64, 64], out_dim=256)
        self.glob_enc = ImpalaCNN(input_shape=global_shape, in_channels=4, channels=[32, 64, 64, 64, 64], out_dim=256)
        ...
        # FiLM Conditioning (Action + Robot State)
...
class PolicyNetwork(nn.Module):
    def __init__(self, ego_shape=(25, 25), global_shape=(50, 50), robot_dim=3, use_robot: bool=True):
        ...
        # Fusion Dimension = Ego_CNN(256) + Global_CNN(256) + Robot
