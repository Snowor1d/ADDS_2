...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def step(self):
        ...
        if started:
            ...
            max_id = 1
            for agent in self.agents:
                if agent.unique_id > max_id and agent.type in [0, 1]:
                    max_id = agent.unique_id
            for agent in self.agents:
                if agent.unique_id == max_id:
                    agent.dead = True
        ...
        # --- 상태 스택 업데이트 ---
        # self.return_current_image()는 (H, W) 단일 채널 이미지라고 가정
        current_image = self.return_current_image()  # (H, W)
        # 최초 호출 시, state_stack이 없으면 4개의 동일 이미지로 초기화
        if not hasattr(self, 'state_stack'):
            self.state_stack = [current_image] * 4
        else:
            # 가장 오래된 이미지를 제거하고 새 이미지를 추가하여 최신 4개 이미지를 유지
            self.state_stack.pop(0)
            self.state_stack.append(current_image)
        # 4채널 상태 구성: (4, H, W)
        state = np.stack(self.state_stack, axis=0)
