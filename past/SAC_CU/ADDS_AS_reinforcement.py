...
if __name__ == "__main__":
    ...
    level_shared = mp.Value('i', LEVEL_RANGE[0])
    ...
    while global_episode < max_episodes:
        ...
        while True:
            ...
            level = LEVEL_RANGE[0] + ((max(0, global_episode-START_UPDATE_EPISODE))// LEVEL_CHANGE_EP)
            level = min(level, LEVEL_RANGE[1])

            with level_shared.get_lock():
                level_shared.value = int(level)
