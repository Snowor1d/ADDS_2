...
def main():
    ...
    while running:
        ...
        while not done and running:
            ...
            reward = 0.0
            try:
                if REWARD_A: reward += env_model.reward_based_alived() * REWARD_A
                if REWARD_B: reward += env_model.reward_based_all_agents_danger() * REWARD_B
                if REWARD_C: reward += env_model.reward_based_gain() * REWARD_C
                if REWARD_D: reward += env_model.reward_penalty() * REWARD_D
                if REWARD_E: reward += env_model.reward_based_evacuated_with_robot() * REWARD_E
                if REWARD_F: reward += env_model.reward_based_distance_from_near_agents() * REWARD_F
                if REWARD_G: reward += env_model.reward_based_distance_from_near_agent_gain() * REWARD_G
                if REWARD_H: reward += env_model.reward_based_gain_with_time_bonus() * REWARD_H
                if REWARD_I: reward += env_model.reward_based_alived_root() * REWARD_I
                if REWARD_J: reward += env_model.reward_based_all_agents_danger_log() * REWARD_J
                if REWARD_K: reward += env_model.reward_penalty_collision() * REWARD_K
    ...
    save_path = os.path.join(os.path.dirname(__file__), "imitation_dataset.npz")
    replay_buffer.save(save_path)          # ← ReplayBuffer 클래스의 save() 사용
