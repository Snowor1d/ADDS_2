...
class HeatMapLogger:
    ...
    def __init__(
        ...
        self._episode_buf: Dict[int, np.ndarray] = {}
        self._aggregate: Dict[int, np.ndarray] = {}
    ...
    def update(self, map_id: int, x: int, y: int) -> None:
        ...
        if map_id not in self._episode_buf:
        ...
        self._episode_buf[map_id][x, y] += 1
    ...
    def flush_episode(self) -> None:
        ...
        for m, epi_mat in self._episode_buf.items():
            if epi_mat.sum() == 0:
                continue
            self._aggregate[m] += epi_mat
            # atomic save → write temp then rename
            tmp_path = self._file_path(m).with_suffix(".tmp")
