...
class ContinuousRenderer:
    ...
    def __init__(
        ...
        # ===== Robot planner target =====
        show_robot_planner_target: bool = True,
        robot_planner_target_color: str = "red",
        ...
        # ===== Robot planner target =====
        self.show_robot_planner_target = show_robot_planner_target
        self.robot_planner_target_color = robot_planner_target_color

    def _get_velocity(self, obj):
        """
        로봇/에이전트에서 속도 벡터(vx, vy)를 찾아 반환.
        우선순위:
        1) obj.vel
        2) obj.vx, obj.vy
        3) obj.dx, obj.dy
        ...
        v = getattr(obj, "vel", None)
        if v is not None and isinstance(v, (tuple, list)) and len(v) >= 2:
            return (float(v[0]), float(v[1]))

        if hasattr(obj, "vx") and hasattr(obj, "vy"):
            return (float(obj.vx), float(obj.vy))

        if hasattr(obj, "dx") and hasattr(obj, "dy"):
            return (float(obj.dx), float(obj.dy))
    ...
    def _draw_robot(self, robot, step: Optional[int] = None):
        ...
        if self.robot_style == "image" and self.robot_img is not None:
            ...
            # robot.angle 기반 회전
            ang_rad = float(getattr(robot, "angle", 0.0))
            ang_deg = math.degrees(ang_rad) - 90.0

            tr = Affine2D().rotate_deg_around(x, y, ang_deg) + self.ax.transData
            im.set_transform(tr)
        ...
        # 2) planner target 표시
        # ---------------------------
        if self.show_robot_planner_target:
            planner_target = getattr(robot, "planner_target", None)
            planner_active = getattr(robot, "planner_active", False)

            if planner_target is not None:
                tx, ty = float(planner_target[0]), float(planner_target[1])

                # 빨간 점
                self.ax.scatter(
                    [tx], [ty],
                    s=36,
                    c=self.robot_planner_target_color,
                    marker="o",
                    zorder=4.2,
                    edgecolors="white",
                    linewidths=0.8
                ...
                # 보기 편하게 로봇-목표 점선 연결 (선택)
                if planner_active:
                    self.ax.plot(
                        [x, tx], [y, ty],
                        linestyle="--",
                        linewidth=1.0,
                        color=self.robot_planner_target_color,
                        alpha=0.6,
                        zorder=4.0
                    )
        ...
        if self.show_robot_heading:
            ...
            self._arrow(
                x, y, vx, vy,
                scale=self.robot_heading_scale,
                color=rcol,
                alpha=0.95,
                linewidth=1.5
            )
