...
class DreamerAgent:
    ...
    def _actor_loss(
        ...
        for _ in range(self.cfg.horizon): # img step 
            ...
            state = self.world_model.rssm.img_step(state, action.detach(), robot, mask)
    ...
    def _sample_reinforce_action(self, feat: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pre_tanh_logprob = pre_tanh.detach() #log prob을 detached sample 기준으로 계산
        raw_logprob = raw.detach()
        log_prob = dist.log_prob(pre_tanh_logprob) - torch.log(1.0 - raw_logprob.pow(2) + 1e-6)
