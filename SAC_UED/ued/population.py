...
class LevelRecord:
    ...
    # re-scored, instead of being frozen the moment each episode ended.
    ...
    def successes(self, k: float = UED_SUCCESS_K) -> int:
        ...
        return sum(1 for t in self.evac_times if t < limit and t < MAX_STEPS)
...
class LevelPopulation:
    ...
    def _raw_score(self, rec: LevelRecord) -> float:
        if self.score_fn == "learnability":
            return rec.learnability if rec.trials > 0 else rec.inherited_score
        if self.score_fn == "maxmc":
            return rec.maxmc if rec.maxmc is not None else rec.inherited_score
        if self.score_fn == "failure_rate":
            return (1.0 - rec.success_rate) if rec.trials > 0 else rec.inherited_score
        # hybrid
        if rec.trials >= UED_MIN_TRIALS:
            return rec.learnability
    ...
    def should_breed(self, rec: LevelRecord) -> bool:
        if UED_METHOD != "accel":
            return False
        with self._lock:
            records = [r for r in self._records.values() if r.trials > 0]
            if len(records) < 8:
                return False
            scores = sorted(self._raw_score(r) for r in records)
            idx = min(len(scores) - 1, int(UED_MUTATE_THRESHOLD * len(scores)))
            return self._raw_score(rec) >= scores[idx]
