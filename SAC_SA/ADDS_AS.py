...
if (MAP_NUM != -2 and MAP_NUM != -1):
    _, size = map_info(model_num)
if (MAP_NUM == -1):
    map_num_candidates = MAP_NUM_RANDOM
    map_num = random.choice(map_num_candidates)
    _, size = map_info(map_num)    

if visualization_mode == 'on':
    s_model_r = model.FightingModel(number_of_agents, model_num)
    ...
    agent_nums = 100
    SIZE_OF_CANVAS_IN_PIXELS_X = size*20
    SIZE_OF_CANVAS_IN_PIXELS_Y = size*20
