...
parser.add_argument("--log_dir", type=str, default="learning_log_guide_game_sac14_mini")
...
class QNetwork(nn.Module):
    """
    stride=1로만 CNN 구성 → 50×50 해상도 유지
    너무 많은 채널을 사용하지 않도록 조정
    - conv1: 1->16 채널
    - res1(16->16), res2(16->32), res3(32->32)
      => 최종적으로 32채널 ×50×50=80,000
    - Fully Connected: 2단(128->64->1)
    - Action embedding: 2->16
    """
    def __init__(self, input_shape=(50,50), action_dim=2):
        ...
        # 2) Residual 블록 3개: 채널 점진 확대(16->16->32->32)
        self.res1 = ResidualBlock(16, 16)  # in=16, out=16
        self.res2 = ResidualBlock(16, 32)  # in=16, out=32
        self.res3 = ResidualBlock(32, 32)  # in=32, out=32
        ...
        # 4) Action embedding (2->16)
        self.action_fc = nn.Sequential(
            nn.Linear(action_dim, 16),
            nn.ReLU()
        )

        # 5) FC 레이어: 2단(128→64→1)
        self.fc1 = nn.Linear(self.conv_out_size + 16, 128)
    ...
    def _get_conv_out(self, shape):
        ...
        x = self.bn_final(x)
        x = F.relu(x)
        return int(np.prod(x.size()[1:]))  # ex) (32, 50, 50) -> 80000
    ...
    def forward(self, state, action):
        ...
        # concat
        x = torch.cat([x, a_emb], dim=1)  # (B, 80016)
