...
class CentralizedAttentionQNetwork(nn.Module):
    ...
    def forward(
        ...
        vision_feat = torch.cat([ego_embed, glob_embed_expanded], dim = -1)
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            for _ in range(K):
                ...
                alt_joint_action, _ = self.sample_joint_actions(
                    joint_ego, global_state, joint_robot, joint_mask
                )
                
                # 3-2. 남들의 행동은 원래 것(new_joint_action)을 그대로 쓰고,
                # 오직 나(agent_index)의 행동만 방금 뽑은 대체 행동으로 갈아끼움!
                mixed_joint_action = new_joint_action.clone()
                mixed_joint_action[batch_indices, agent_index] = alt_joint_action[batch_indices, agent_index]
                
                # 3-3. 혼합된 행동(Counterfactual action)으로 Q값 재평가
                q1_base_all = self.q1(joint_ego, global_state, mixed_joint_action, joint_robot, joint_mask)
                q2_base_all = self.q2(joint_ego, global_state, mixed_joint_action, joint_robot, joint_mask)
                
                q1_base_i = gather_agent_tensor(q1_base_all, agent_index).squeeze(-1)
                q2_base_i = gather_agent_tensor(q2_base_all, agent_index).squeeze(-1)
                
                q_base = torch.min(q1_base_i, q2_base_i)
                baseline_q_list.append(q_base)
                
            # 3-4. K번 뽑아본 Q값들의 평균 = "내가 평소에 할 법한 행동들의 기댓값" (b)
            baseline_q = torch.stack(baseline_q_list, dim=0).mean(dim=0) # (B,)

        # [Step 4] Advantage 계산: (내가 이번에 한 특별한 행동의 가치) - (내 평소 평균 행동의 가치)
        advantage = q_new - baseline_q

        # [Step 5] 최종 Policy Loss
        # MAAC의 철학을 반영하여 Advantage가 양수면 해당 행동을 강화(Log_prob 증가)하도록 REINFORCE 항 추가
        
        # policy_loss = (self.alpha * logp_i - q_new).mean() # <- (기존 순수 SAC 방식)
        
        # [MAAC Advantage 반영 방식]
        # 순수 Q값 기반의 Reparameterization(SAC)과 Advantage 기반의 REINFORCE를 결합
        policy_loss = (self.alpha * logp_i - q_new - (logp_i * advantage.detach())).mean()
