...
robot_learned_model = os.path.expanduser(
    "~/SOTA_MODELS/dreamer_checkpoint_ep_5000.pth"
)
test_num            =  12
#map_list = [1239]
...
def _robot_world_to_px(env):
    ...
    ix = int(np.clip(rx / max(1, env.width)  * W, 0, W - 1))
    iy = int(np.clip(ry / max(1, env.height) * H, 0, H - 1))

    return ix, iy

def build_ego_global_frames(env):
    # full map은 현재 env의 실제 JSON map 크기 기준으로 생성
    full_map_u8 = env.return_current_image(env.height, env.width)
    ...
    # neural network 입력용 global map은 기존처럼 고정 크기로 downsample
...
def run_one_episode(map_id: int):
    ...
    renderer = ContinuousRenderer(
        world_size=(float(env.width), float(env.height)),
