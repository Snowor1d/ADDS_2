...
##########################################################################
# 3) Critic (Q) Network - Version 2: CNN 4개 + FC layer 2개
##########################################################################
class QNetwork(nn.Module):
    ...
    def _get_conv_out(self, shape):
        ...
        o = F.leaky_relu(self.bn4(self.conv4(o)), negative_slope=0.01)
...
##########################################################################
# 4) Policy (Actor) Network - Version 2: CNN 4개 + FC layer 2개
##########################################################################
...
if __name__ == "__main__":
    import time
    import model  # 환경 코드 (model.FightingModel)
    ...
    agent = SACAgent(input_shape=(50,50), alpha=args.alpha, lr=float(args.lr),
                     ...
                     device=args.device)
    print(f"Agent initialized, lr={args.lr}, alpha={agent.alpha}, batch_size={args.batch_size}, replay_buffer={args.buffer_size}")
        ...
        try:
            for step in range(max_steps):
                ...
                if ((step % ACTION_SCALE == (ACTION_SCALE - 1) and step > ACTION_SCALE) or 
                    (env_model.robot.is_game_finished)):
