...
class FightingModel(Model):
    ...
    def is_valid_space(self, x, y):
        if x < 1.5 or x >= self.width-1.5 or y < 1.5 or y >= self.height-1.5: 
            return False

        if (int(round(x)), int(round(y))) in self.match_grid_to_mesh.keys():
            mesh = self.match_grid_to_mesh[(int(round(x)), int(round(y)))]
            if mesh in self.obstacle_mesh:
                return False
            else:
                return True
        else:
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def reward_based_invalid_order(self):
        if (self.robot.detect_invalid_order == 1):
            print("invalid order, penalty")
            return -2
        return 0
