...
class VisibilityAtlas:
    ...
    def _candidate_edges(self, center, R):
        ...
        if not ax:
            return None, inside
        return (np.asarray(ax), np.asarray(ay), np.asarray(bx), np.asarray(by)), inside
        ...
        for j in range(self.reg_rows):
            for i in range(self.reg_cols):
                ...
                for R in self._radii:
                    ...
                    edges, center_inside = self._candidate_edges(center, R)

                    if center_inside:
                        # A centre buried in an obstacle sees nothing. The old
                        # bisection collapsed onto the centre here too, leaving
                        # a polygon of radius R/2**iters; reproduced exactly so
                        # anything downstream that inspects the degenerate case
                        # sees the same thing it always did.
                        t = np.full(rays_per_poly, R / (2.0 ** max(1, int(bsearch_iters))))
                    elif edges is None:
                        t = np.full(rays_per_poly, float(R))
                    else:
                        t = self._cast_rays(center, dir_x, dir_y, float(R), edges)

                    pts = list(zip(center[0] + dir_x * t, center[1] + dir_y * t))
