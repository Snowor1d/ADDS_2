...
class FightingModel(Model):
    ...
    def free_flow_evacuation_steps(self) -> float:
        """A level-intrinsic lower bound on how long evacuating should take.

        The success criterion the curriculum scores levels by needs a reference,
        and MAX_STEPS is a human-set constant with nothing to do with the level:
        set it generously and almost everything succeeds, tighten it and almost
        nothing does, and either way p*(1-p) collapses to zero and the
        curriculum loses its signal.

        This is the textbook evacuation estimate instead, travel plus queueing:

          travel  = worst-case geodesic distance to an exit, at walking speed
          queue   = crowd size discharged through the total exit width

        Both terms use only geometry and crowd size, which are exactly the
        parameters the curriculum controls, so the reference scales with the
        level rather than against a fixed clock. mesh_danger already holds the
        geodesic distances, computed during construction.
        """
        dt = max(1e-6, float(AGENT_TIME_STEP))
        speed = max(1e-6, float(AGENT_SPEED_MEAN))

        reachable = [d for d in self.mesh_danger.values() if d < 1e5]
        worst_distance = max(reachable) if reachable else 0.0
        travel_steps = (worst_distance / speed) / dt

        # Effective exit width is the side lying along the wall, which is the
        # longer of the two box dimensions the generator produces.
        total_width = 0.0
        for poly in self.exit_list:
            xs = [float(p[0]) for p in poly]
            ys = [float(p[1]) for p in poly]
            total_width += max(max(xs) - min(xs), max(ys) - min(ys))

        queue_steps = 0.0
        if total_width > 0 and EVAC_SPECIFIC_FLOW > 0:
            queue_steps = (self.total_agents / (EVAC_SPECIFIC_FLOW * total_width)) / dt
    ...
    def return_current_robot_state(self):
        ...
        # from a large hall.
        return (
            self.robot.xy[0] / max(1.0, float(self.width)),
            self.robot.xy[1] / max(1.0, float(self.height)),
            agent_num,
