...
# Difficulty table
# -----------------------------
# Lengths in the difficulty table are metres measured on a 100x100 world. On a
# bigger world the same absolute numbers leave only a few huge blocks and a few
# tiny ones, with no mid-scale corridor structure, so the character of a map
# changes with its size. These scale with the world instead.
_SIZE_SCALED_PARAMS = (
    "small_rect_w_min", "small_rect_w_max",
    "small_rect_h_min", "small_rect_h_max",
    "rect_w_min", "rect_w_max",
    "rect_h_min", "rect_h_max",
    "min_obstacle_gap", "wall_clearance", "keep_gap_from_exits",
    "min_exit_distance", "min_exit_center_distance", "corner_avoid_dist",
    "conv_min_width", "large_conv_min_width", "conv_gap_boost",
    "exit_flank_len_min", "exit_flank_len_max",
    # The flanks the generator seeds beside each exit sit a couple of metres
    # away. Left absolute, a level shrunk to 70 m keeps a 2 m flank gap while
    # every other clearance scales down around it, and the gap then falls
    # below what the robot needs to pass between flank and exit.
    "exit_flank_gap_min", "exit_flank_gap_max",
)

# Nothing may scale below these, whatever the world size. A gap narrower than
# the robot's diameter of 2 m makes a level unusable no matter how small the
# map is, and the clearances exist to keep routes walkable rather than to
# express difficulty.
_SIZE_SCALE_FLOORS = {
    "min_obstacle_gap": 4.0,
    "wall_clearance": 4.0,
    "keep_gap_from_exits": 5.0,
    "min_exit_distance": 12.0,
    "min_exit_center_distance": 16.0,
    "corner_avoid_dist": 6.0,
    "conv_min_width": 5.0,
    "large_conv_min_width": 5.0,
    "small_rect_w_min": 3.0,
    "small_rect_h_min": 3.0,
    "small_rect_w_max": 5.0,
    "small_rect_h_max": 5.0,
    # Robot diameter plus a margin; below this the crowd cannot get past.
    "exit_flank_gap_min": 3.0,
    "exit_flank_gap_max": 4.0,
}

# Exit openings are deliberately left absolute. A doorway is a physical width,
# it does not grow because the building did, and the free-flow evacuation
# estimate the curriculum scores against divides the crowd by total exit width.
_PARAMS_REFERENCE_SIZE = 100.0


def _params_from_difficulty(W: int, H: int, difficulty: int) -> Dict[str, float]:
    p = _params_from_difficulty_base(W, H, difficulty)

    scale = float(min(int(W), int(H))) / _PARAMS_REFERENCE_SIZE
    if abs(scale - 1.0) < 1e-9:
        return p

    for key in _SIZE_SCALED_PARAMS:
        if key not in p:
            continue
        value = float(p[key]) * scale
        floor = _SIZE_SCALE_FLOORS.get(key)
        if floor is not None:
            value = max(value, floor)
        p[key] = value

    # Ranges must not invert once the floors bite.
    for lo_key, hi_key in (
        ...
        if lo_key in p and hi_key in p and p[lo_key] > p[hi_key]:
            p[lo_key], p[hi_key] = p[hi_key], p[lo_key]

    return p


def _params_from_difficulty_base(W: int, H: int, difficulty: int) -> Dict[str, float]:
    d = int(difficulty)
    ...
    if d <= 0:
        # Near-empty rooms: two exits and at most one small obstacle. This is
        # the tier ACCEL is designed to start from, so that map complexity has
        # to be earned by editing rather than supplied by the generator. The
        # density floor is zero because a completely empty room is a valid
        # level here, not a generation failure.
        p.update({
            "density_min": 0.0, "density_max": 0.02,
            "large_count_min": 0, "large_count_max": 0,
            "small_count_min": 0, "small_count_max": 0,
            # The flanks the generator normally seeds beside each exit count as
            # obstacles too, so they are off here; otherwise the tier would
            # never actually produce the empty room it exists to produce.
            "exit_flank_bias": 0.0,
        })
        return p
...
def generate_map(spec: RandomMapSpec) -> MapData:
    """Generate a map without disturbing the caller's global RNG stream.

    The generator seeds `random` so that a given seed reproduces a given map.
    That reseeding used to leak: everything the caller drew afterwards (crowd
    spawn positions, per-pedestrian mass/speed, the augmentation choice) became
    a deterministic function of the map seed, which collapses the separation
    between designer-controlled and aleatoric parameters that UED relies on.
    The global state is therefore saved and restored around the generation.
    """
    _require_shapely()
    ...
    try:
        return _generate_map_seeded(spec, seed_used)
    finally:
        random.setstate(saved_state)


def _generate_map_seeded(spec: RandomMapSpec, seed_used: int) -> MapData:
