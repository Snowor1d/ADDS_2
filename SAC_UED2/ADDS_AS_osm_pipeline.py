...
Typical first run:
    ...
    python3 ADDS_AS_osm_pipeline.py resolve
    python3 ADDS_AS_osm_pipeline.py download
    python3 ADDS_AS_osm_pipeline.py collect
    python3 ADDS_AS_osm_pipeline.py report --size 200
    python3 ADDS_AS_osm_pipeline.py export
...
def cmd_status(args):
    ...
    levels = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "osm_corpus", "data", "levels", "real_levels.json")
    if os.path.exists(levels):
        ...
        if corpus_mode and lv_mode != corpus_mode:
            print(f"          STALE: corpus is '{corpus_mode}'. "
                  f"Run: python3 ADDS_AS_osm_pipeline.py export")
    else:
        print("export  : none")
