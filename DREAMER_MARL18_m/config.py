...
#
# Experiment variant: DREAMER_MARL18_m
...
# - Scales only the four model-width dimensions used by the DreamerV3 paper:
#   MLP hidden units, recurrent units, base CNN channels, and codes per latent.
#   Uses the custom intermediate model dimension 224 for a modest reduction
#   while keeping recurrent=8*M and CNN/codes=M/16. All other settings remain
#   identical to DREAMER_MARL17.
# - Removes orchestration and I/O stalls: latest-only policy broadcasts, one
#   TensorBoard writer, asynchronous evaluation/checkpoint serialization, and
#   compact uint8 replay transport with prefetching.
# - Uniformly samples K=16 valid posterior states per sequence for imagination
#   instead of expanding actor/value rollouts from every posterior state.
# ------------- DREAMER V3 ------------------
...
DREAMER_DETER_SIZE = 1792
DREAMER_STOCH_SIZE = 32
DREAMER_DISCRETE_SIZE = 14 # codes per latent
DREAMER_HIDDEN_SIZE = 224
DREAMER_EMBED_SIZE = 256
DREAMER_ACTION_EMBED_SIZE = 256
DREAMER_CONV_DEPTH = 14
DREAMER_VECTOR_ENCODER_LAYERS = 3
DREAMER_RSSM_BLOCKS = 8
# Number of uniformly sampled valid posterior states per replay sequence used
# as imagination starts. Sampling is without replacement.
DREAMER_IMAG_STARTS_PER_SEQUENCE = 16
...
DREAMER_DECODER_CHUNK_SIZE = 512
DREAMER_SKIP_CONTEXT_PREDICTION = True
DREAMER_REPLAY_PREFETCH = True
...
DREAMER_METRIC_LOG_INTERVAL = 50
