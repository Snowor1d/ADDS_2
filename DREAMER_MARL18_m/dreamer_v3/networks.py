...
def image_obs_to_float(image_obs: torch.Tensor) -> torch.Tensor:
    """Map replay uint8 images to [0, 1] while preserving float inputs.

    Inference still supplies already-normalized float observations. Keeping the
    conversion at the network boundary lets replay and host-to-device transfer
    retain the compact uint8 representation.
    """
    if image_obs.dtype == torch.uint8:
        return image_obs.to(dtype=torch.float32).mul_(1.0 / 255.0)
    return image_obs.float()


class ConvEncoder(nn.Module):
    """DreamerV3 image encoder adapted to the current non-multiple-of-16 maps."""

    def __init__(self, cfg: DreamerConfig, in_channels: int, input_shape: tuple[int, int]) -> None:
        super().__init__()
        depth = max(1, int(cfg.conv_depth))
        self.depths = (depth * 2, depth * 3, depth * 4)
        self.pool_stages = len(self.depths)
        modules: list[nn.Module] = []
        current_channels = int(in_channels)
        for out_channels in self.depths:
            conv = nn.Conv2d(current_channels, out_channels, 5, padding=2)
            scale = 1.0 / math.sqrt(max(1, current_channels * 5 * 5))
            nn.init.trunc_normal_(conv.weight, std=scale, a=-2.0 * scale, b=2.0 * scale)
        ...
        height, width = (int(x) for x in input_shape)
        factor = 2 ** self.pool_stages
        self.output_h = int(math.ceil(height / factor))
        self.output_w = int(math.ceil(width / factor))
        self.output_size = self.depths[-1] * self.output_h * self.output_w

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Official DreamerV3 centers uint8 images after mapping them to [0, 1].
        x = image_obs_to_float(x) - 0.5
        x = self.conv(x)
        if x.shape[-2:] != (self.output_h, self.output_w):
            raise RuntimeError(
                f"Unexpected encoder spatial shape {tuple(x.shape[-2:])}; "
                f"expected {(self.output_h, self.output_w)}"
            )
        return x.flatten(start_dim=-3)


class ConvDecoder(nn.Module):
    """Block-spatial image branch adapted to 25x25 and 50x50 maps."""
    ...
    def __init__(self, cfg: DreamerConfig, out_shape: tuple[int, int, int]) -> None:
        ...
        upsample_factor = 2 ** self.upsample_stages
        self.start_h = max(1, int(math.ceil(height / upsample_factor)))
        self.start_w = max(1, int(math.ceil(width / upsample_factor)))
...
class JointObsEncoder(nn.Module):
    def __init__(self, cfg: DreamerConfig) -> None:
        ...
        self.cfg = cfg
        self.ego_encoder = ConvEncoder(cfg, c_ego, (ego_h, ego_w))
        self.global_encoder = ConvEncoder(cfg, c_global, (global_h, global_w))
        self.robot_encoder = (
            nn.Sequential(
                *mlp_blocks(
                    cfg.robot_dim,
                    cfg.hidden_size,
                    cfg.vector_encoder_layers,
                )
            )
            if cfg.robot_dim > 0
            else None
        )
        robot_token_size = cfg.hidden_size if self.robot_encoder is not None else 0
        self.output_size = (
            self.ego_encoder.output_size
            + self.global_encoder.output_size
            + robot_token_size
        )
...
class RSSM(nn.Module):
    def __init__(self, cfg: DreamerConfig, encoder_size: int | None = None) -> None:
        super().__init__()
        self.cfg = cfg
        self.encoder_size = int(cfg.embed_size if encoder_size is None else encoder_size)
        ...
        action_input_dim = joint_action_dim + cfg.max_robots
        ...
        self.posterior = nn.Sequential(
            *linear_block(cfg.deter_size + self.encoder_size, cfg.hidden_size),
            nn.Linear(cfg.hidden_size, self.stoch_flat),
        )
    ...
    def obs_step(
        ...
        prior = self.img_step(prev, prev_action, prev_mask)
    ...
    def img_step(
        ...
        deter = self._core(prev, action, mask)
    ...
    def _core(
        ...
        normalized_action = env_to_raw_action(action, self.cfg)
        action_flat = normalized_action.reshape(batch_size, -1)
        action_input = torch.cat([action_flat, mask], dim=-1)
        ...
        # Split the three GRU gates inside every deterministic block before
        # flattening. Flattening first would interleave block-local R/C/U
        # slices and destroy the block identity used by the spatial decoder.
        reset, cand, update = gates.chunk(3, dim=-1)
        reset = reset.reshape(batch_size, self.cfg.deter_size)
        cand = cand.reshape(batch_size, self.cfg.deter_size)
        update = update.reshape(batch_size, self.cfg.deter_size)
...
class WorldModel(nn.Module):
    def __init__(self, cfg: DreamerConfig) -> None:
        ...
        self.rssm = RSSM(cfg, encoder_size=self.encoder.output_size)
    ...
    def observe(self, batch: dict[str, torch.Tensor]) -> tuple[list[RSSMState], list[RSSMState]]:
        ...
        for i in range(t): # t : 샘플된 실제 관측 sequence 길이, sequence legnth + replay context
            ...
            post, prior = self.rssm.obs_step(prev, prev_action, prev_mask, embed[:, i])
            posts.append(post) # posterior는 이전 latent/action/mask와 현재 관측 embed로 만들어짐
            priors.append(prior)
            prev = post
        return posts, priors
    ...
    def loss(
    ...
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor], list[RSSMState]]:
        ...
        if bool(getattr(self.cfg, "skip_context_prediction", False)):
            # Replay context is a fixed prefix of every sampled sequence.
            # Build a shape-known index grid instead of torch.nonzero(), whose
            # data-dependent output size synchronizes CUDA with the host.
            context = min(max(int(getattr(self.cfg, "replay_context", 0)), 0), t)
            prediction_index = torch.arange(
                b * t,
                device=flat.device,
            ).reshape(b, t)[:, context:].reshape(-1)
        else:
            prediction_index = torch.arange(b * t, device=flat.device)

        for start in range(0, prediction_index.numel(), chunk_size):
            index_chunk = prediction_index[start:start + chunk_size]
            feat_chunk = flat[index_chunk]
            mask_chunk = flat_mask[index_chunk]
            robot_mask_chunk = robot_mask[index_chunk]
            chunk_length = index_chunk.numel()

            reward_loss_per = self.reward_head.loss(
                feat_chunk,
                reward_target[index_chunk],
                reduction="none",
            )
            ...
            ego_slot_mask = robot_mask_chunk.reshape(
                chunk_length,
                self.cfg.max_robots,
                1,
                1,
            ...
            ego_target_chunk = image_obs_to_float(ego_target[index_chunk])
            ego_loss_per = (
                (ego_pred - ego_target_chunk).pow(2) * ego_slot_mask
            ).sum(dim=(1, 2, 3, 4))
            ego_loss_sum = ego_loss_sum + (ego_loss_per * mask_chunk).sum()

            global_pred = self.global_decoder(feat_chunk)
            global_target_chunk = image_obs_to_float(global_target[index_chunk])
            global_loss_per = (
                global_pred - global_target_chunk
            ).pow(2).sum(dim=(1, 2, 3))
            ...
            robot_loss_per = (
                (robot_pred - robot_target[index_chunk]).pow(2) * robot_mask_chunk
            ).sum(dim=(1, 2))
        ...
        metrics = {
            ...
            "recon_loss": (ego_loss + global_loss + robot_loss).detach(),
            "image_recon_loss": (ego_loss + global_loss).detach(),
            "robot_recon_loss": robot_loss.detach(),
