...
if __name__ == "__main__":
    ...
    while global_episode < max_episodes:
        ...
        while True:
            ...
            epsilon_reset_by_curriculum = False
            ...
            if curriculum_manager.enabled and s_msg.abnormal != 1:
                ...
                if level_changed:
                    ...
                    # --------------------------------------------------
                    # Curriculum epsilon reset
                    # --------------------------------------------------

                    if CURRICULUM_EPSILON_USING:
                        if int(new_level) in CURRICULUM_EPSILON:
                            new_epsilon = float(CURRICULUM_EPSILON[int(new_level)])

                            agent.epsilon = new_epsilon
                            epsilon_reset_by_curriculum = True

                            with epsilon_shared.get_lock():
                                epsilon_shared.value = float(agent.epsilon)

                            step_writer.add_scalar(
                                "Curriculum/EpsilonReset",
                                float(agent.epsilon),
                                global_episode
                            )

                            print(
                                f"[Curriculum Epsilon] Reset epsilon: "
                                f"level={new_level}, epsilon={agent.epsilon}"
                            )
                        else:
                            print(
                                f"[Curriculum Epsilon] No epsilon value for level={new_level}. "
                                f"Keep current epsilon={agent.epsilon}"
                            )
            ...
            if not epsilon_reset_by_curriculum:
                agent.epsilon = max(
