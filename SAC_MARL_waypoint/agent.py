...
class RobotAgent(CrowdAgent):
    ...
    def receive_action_from_policy(self, action):
        ...
        # 1. 분해 (Policy 출력 순서 주의: [r, theta_idx, spd])
        r, theta_idx, spd = action[0], action[1], action[2]
        
        # 2. theta_idx를 실제 라디안 각도로 변환
        # DIRECTION_N이 8이라면, 0~7 인덱스를 0 ~ 2*pi 범위로 매핑
        # (예: 0 -> 0도, 1 -> 45도, 2 -> 90도 ...)
        angle_per_idx = (2.0 * math.pi) / DIRECTION_N
        actual_theta = int(theta_idx) * angle_per_idx
        
        # 만약 theta를 [-pi, pi] 범위로 쓰고 싶다면 아래와 같이 조정 가능합니다.
        # actual_theta = (int(theta_idx) * angle_per_idx) - math.pi

        # 3. 로봇 형식에 맞게 재배열 [theta, radius, target_speed]
        # 질문자님의 receive_action 주석에 적힌 순서대로 전달합니다.
        formatted_action = [actual_theta, r, spd]
    ...
    def receive_action(self, action):
        ...
        theta is interpreted RELATIVE to current heading self.angle.
        """
        if action is None:
        ...
        if len(action) != 3:
            raise ValueError(
                "Robot action must be [theta, radius, target_speed]"
            )
        ...
        # interpret theta relative to current heading
        ...
        self.planner_active = radius > 1e-8 and target_speed > 1e-8

        fx, fy = self._safe_normalize(goal_x - self.xy[0], goal_y - self.xy[1])
    ...
    def robot_policy_Q(self):
        ...
        if self.planner_active and self._goal_reached(self.planner_target, tol=0.5):
            self._cancel_planner(stop_velocity=False)
