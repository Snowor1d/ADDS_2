...
log_dir = os.path.join(home_dir, "learning_log_3.1.7")
...
if __name__ == "__main__":
    ...
    for episode in range(max_episodes):
        ...
        frame = env_model.return_current_image()
        frame = np.array(frame, dtype=np.float32)
        frames_deque = deque(maxlen=stack_size)
        for _ in range(stack_size):
            frames_deque.append(frame)
        
        stacked_state= np.stack(frames_deque, axis=0)
        ...
        try:
            for step in range(max_steps):
                ...
                next_stacked_state = np.stack(frames_deque, axis=0)
