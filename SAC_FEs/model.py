...
def _edge_key(a, b, nd=6):
    a = _pt_key(a, nd); b = _pt_key(b, nd)
...
def is_convex_polygon(coords, eps=1e-12):
    """
    coords: ring without duplicated last point
    True if strictly convex OR weakly convex (collinear allowed)
    """
...
def concavity_measure(coords, eps=1e-12):
    """
    오목을 허용하고 싶을 때 사용:
    - reflex(내각>180) 꼭짓점 수를 세어서 반환
    """
    coords = _ensure_ccw(coords)
    n = len(coords)
    if n < 3:
    ...
    for i in range(n):
        ...
        x1, y1 = coords[i]
        x2, y2 = coords[(i+1) % n]
        # cross of edges (x1-x0)->(x2-x1)
        cross = (x1-x0)*(y2-y1) - (y1-y0)*(x2-x1)
        if cross < -eps:  # CCW에서 cross<0 이면 reflex
            reflex += 1
    return reflex

def polygon_edges(coords, nd=6):
    coords = [_pt_key(p, nd) for p in coords]
    out = []
    n = len(coords)
    for i in range(n):
        a = coords[i]
        b = coords[(i+1) % n]
        out.append(_edge_key(a, b, nd))
    return out

def build_edge_to_cells(cell_polys, nd=6):
    """
    cell_polys: list[Polygon]
    return: dict[edge_key] -> list[cell_index]
    """
    edge2cells = {}
    for idx, P in enumerate(cell_polys):
        coords = list(P.exterior.coords)[:-1]
        for e in polygon_edges(coords, nd):
            edge2cells.setdefault(e, []).append(idx)
    return edge2cells

def try_merge_two(Pa: Polygon, Pb: Polygon,
                  obstacle_union: Polygon | None = None,
                  allow_concave_reflex_max: int = 0,
                  max_vertices: int = 12) -> Polygon | None:
    """
    - allow_concave_reflex_max=0  => convex only
    - allow_concave_reflex_max>0  => reflex 꼭짓점 수 제한으로 '조금 오목' 허용
    """
    U = Pa.union(Pb)

    # union 결과가 Polygon이 아니면(멀티/콜렉션) 실패
    if U.is_empty or (U.geom_type != "Polygon"):
        return None

    coords = list(U.exterior.coords)[:-1]
    if len(coords) > max_vertices:
        return None

    # (선택) 장애물과 충돌 방지: free-space끼리 merge면 보통 불필요하지만 안전장치로 가능
    if obstacle_union is not None:
        # touches는 경계가 맞닿는 경우까지 막을지 정책 선택
        if U.intersects(obstacle_union) or U.touches(obstacle_union):
            return None

    coords = _ensure_ccw(coords)

    if allow_concave_reflex_max <= 0:
        if not is_convex_polygon(coords):
            return None
    else:
        if concavity_measure(coords) > allow_concave_reflex_max:
            return None

    return U

def merge_triangles_to_polygons(tri_polys: list[Polygon],
                               obstacle_union: Polygon | None = None,
                               allow_concave_reflex_max: int = 0,
                               max_vertices: int = 12,
                               ndigits: int = 6,
                               max_passes: int = 999999):
    """
    반복적으로 edge 공유 셀을 병합.
    """
    cells = tri_polys[:]  # working list
    alive = [True] * len(cells)

    # 인덱스 재사용 때문에 "실제 객체 리스트" 방식으로 계속 재구성하는 게 안전
    passes = 0
    changed = True
    while changed and passes < max_passes:
        ...
        edge2cells = build_edge_to_cells(cells, nd=ndigits)

        # 공유 edge가 있는 pair들만 검사
        merged_any = False
        for e, idxs in edge2cells.items():
            if len(idxs) != 2:
                continue
            i, j = idxs
            if i == j:
            ...
            Pa, Pb = cells[i], cells[j]
            M = try_merge_two(Pa, Pb,
                              obstacle_union=obstacle_union,
                              allow_concave_reflex_max=allow_concave_reflex_max,
                              max_vertices=max_vertices)
            if M is None:
                continue

            # merge 성공: i 자리에 M 넣고 j는 제거
            cells[i] = M
            cells.pop(j)

            changed = True
            merged_any = True
            break  # edge2cells가 stale이라 한 번 merge하면 rebuild
...
def are_polys_adjacent(coordsA, coordsB, nd=6):
    # 공유 edge가 1개 이상이면 인접
    EA = set(polygon_edges(coordsA, nd))
    EB = set(polygon_edges(coordsB, nd))
    return len(EA & EB) > 0
...
def _poly_key_from_coords(coords, nd=6):
    # Polygon exterior coords -> stable hashable key (rounded)
    pts = [(round(float(x), nd), round(float(y), nd)) for x, y in coords]
    # 닫힘점 제거(Shapely는 닫힘 포함) 대비
    if len(pts) >= 2 and pts[0] == pts[-1]:
        pts = pts[:-1]
    return tuple(pts)

def _is_convex_polygon(poly: Polygon, area_tol_ratio=1e-5) -> bool:
    # convex hull과의 면적 차이가 매우 작으면 convex로 간주
    if poly.is_empty or (not poly.is_valid):
        return False
    hull = poly.convex_hull
    if hull.area <= 1e-12:
        return False
    return abs(hull.area - poly.area) / hull.area <= area_tol_ratio

def _shared_boundary_len(a: Polygon, b: Polygon) -> float:
    inter = a.boundary.intersection(b.boundary)
    if inter.is_empty:
        return 0.0
    # LineString/MultiLineString 길이 합산
    try:
...
class PolyNavMesh:
    """
    - triangle 라이브러리로 constrained triangulation
    - 생성된 삼각형을 "convex 유지" 조건으로 최대한 병합 → 폴리곤 개수 감소
    - adjacency graph 생성
    """
    def __init__(self, boundary_poly: Polygon, obstacle_polys: List[Polygon]):
        self.boundary_poly = boundary_poly
        self.obstacle_union = unary_union(obstacle_polys) if obstacle_polys else None
        self.free_poly = self._build_free_polygon()

        self.polys: List[Polygon] = []       # convex-ish polygons
        self.poly_keys: List[Tuple] = []     # stable keys for dict
        self.adj: Dict[Tuple, List[Tuple]] = {}  # key -> neighbor keys
        self.centroid: Dict[Tuple, Tuple[float,float]] = {}

        self._strtree = None
        self._geom_to_key = {}

    def _build_free_polygon(self) -> Polygon:
        if self.obstacle_union is None or self.obstacle_union.is_empty:
            return self.boundary_poly
        # boundary - obstacles
        free = self.boundary_poly.difference(self.obstacle_union)
    ...
    def build(self,
              D: float = 20.0,
              merge_passes: int = 5,
              min_shared_edge: float = 1e-6,
              convex_area_tol_ratio: float = 1e-5):
        """
        D: PSLG edge subdivision step (너 코드의 기존 D=20 같은 역할)
        """
        tris = self._triangulate_constrained(D=D)
        merged = self._merge_triangles(tris,
                                       merge_passes=merge_passes,
                                       min_shared_edge=min_shared_edge,
                                       convex_area_tol_ratio=convex_area_tol_ratio)
        self.polys = merged
        self.poly_keys = [_poly_key_from_coords(p.exterior.coords) for p in self.polys]
        self._build_adjacency(min_shared_edge=min_shared_edge)
        self._build_spatial_index()

    def _triangulate_constrained(self, D: float) -> List[Polygon]:
        # 1) boundary PSLG
        bx, by = self.boundary_poly.exterior.coords.xy
        boundary_ring = list(zip(bx, by))[:-1]  # 닫힘 제거
        vertices = [list(p) for p in boundary_ring]
        segments = [[i, (i + 1) % len(boundary_ring)] for i in range(len(boundary_ring))]
        offset = len(vertices)

        # 2) obstacles -> outer rings (+ holes) 를 세그먼트로 추가
        # obstacle_union이 MultiPolygon일 수도 있으니 geoms 순회
        obstacle_geoms = []
        if self.obstacle_union is not None and (not self.obstacle_union.is_empty):
            if isinstance(self.obstacle_union, Polygon):
                obstacle_geoms = [self.obstacle_union]
            elif isinstance(self.obstacle_union, MultiPolygon):
                obstacle_geoms = list(self.obstacle_union.geoms)

        for P in obstacle_geoms:
            ring = list(P.exterior.coords)[:-1]
            n = len(ring)
            if n < 3:
                continue
            vertices.extend([list(p) for p in ring])
            segments.extend([[offset + i, offset + ((i + 1) % n)] for i in range(n)])
            offset += n
            # holes
            for hole in P.interiors:
                hring = list(hole.coords)[:-1]
                hn = len(hring)
                if hn < 3:
                    continue
                vertices.extend([list(p) for p in hring])
                segments.extend([[offset + i, offset + ((i + 1) % hn)] for i in range(hn)])
                offset += hn

        # 3) 세그먼트 촘촘하게(메쉬 품질)
        vertices2, segments2 = generate_segments_with_points(vertices, segments, D)
        vertices2, segments2 = _dedup_pslg(vertices2, segments2)

        tri_data = {'vertices': np.array(vertices2), 'segments': np.array(segments2)}
        t = tr.triangulate(tri_data, 'p')
        ...
        for tri in t['triangles']:
            ...
            v1 = t['vertices'][tri[1]]
            v2 = t['vertices'][tri[2]]
            tri_poly = Polygon([(v0[0], v0[1]), (v1[0], v1[1]), (v2[0], v2[1])])
            if tri_poly.is_empty:
                continue
            # free 영역 안의 삼각형만 유지 (centroid 기준)
            c = tri_poly.representative_point()
            if self.free_poly.covers(c):
                out.append(tri_poly)

        return out

    def _merge_triangles(self,
                         tris: List[Polygon],
                         merge_passes: int,
                         min_shared_edge: float,
                         convex_area_tol_ratio: float) -> List[Polygon]:
        """
        인접 삼각형을 가능한 한 병합하여 polygon 수 감소.
        병합 조건:
          - 공유 경계 길이가 충분
          - union 결과가 single Polygon
          - union 결과가 convex-ish (convex hull area 차이 작음)
        """
        polys = tris[:]

        # 빠른 인접 후보 탐색을 위해 STRtree 사용
        for _ in range(merge_passes):
            ...
            for i, p in enumerate(polys):
                ...
                # 한 poly에 대해 여러 번 병합 시도
                while changed:
                    changed = False
                    # 가장 좋은 이웃(공유엣지 가장 큼)부터 병합
                    best_j = None
                    best_len = 0.0

                    for q in candidates:
                        # STRtree가 geometry를 반환(Shapely 1.8/2.x 차이 없이)하므로 index 찾기
                        try:
                            j = polys.index(q)
                        except ValueError:
                            continue
                        if used[j]:
                            continue
                        # 공유 경계 길이
                        L = _shared_boundary_len(merged, q)
                        if L > best_len:
                            best_len = L
                            best_j = j

                    if best_j is not None and best_len > min_shared_edge:
                        q = polys[best_j]
                        u = merged.union(q)
                        if isinstance(u, Polygon) and (not u.is_empty):
                            if not u.is_valid:
                                u = u.buffer(0)
                            if isinstance(u, Polygon) and _is_convex_polygon(u, area_tol_ratio=convex_area_tol_ratio):
                                used[best_j] = True
                                merged = u
                                changed = True
        ...
        # free_poly 내부로 한 번 더 클립/정리
        cleaned = []
        for p in polys:
            u = p.intersection(self.free_poly)
            if u.is_empty:
                continue
            if isinstance(u, Polygon):
                if u.area > 1e-6:
    ...
    def _build_adjacency(self, min_shared_edge: float):
        self.adj = {}
        self.centroid = {}

        for k, p in zip(self.poly_keys, self.polys):
            rp = p.representative_point()
            self.centroid[k] = (rp.x, rp.y)
            self.adj[k] = []

        # O(N^2) 피하려고 STRtree
        tree = STRtree(self.polys)
        for i, (ki, pi) in enumerate(zip(self.poly_keys, self.polys)):
            ...
            for pj in candidates:
                ...
                # key 찾기
                try:
                    j = self.polys.index(pj)
                except ValueError:
                    continue
                kj = self.poly_keys[j]
                # edge 공유 길이 체크
                if _shared_boundary_len(pi, pj) > min_shared_edge:
                    if kj not in self.adj[ki]:
                        self.adj[ki].append(kj)

    def _build_spatial_index(self):
        self._strtree = STRtree(self.polys)
        self._geom_to_key = {id(g): k for g, k in zip(self.polys, self.poly_keys)}

    def locate(self, xy: Tuple[float, float]) -> Optional[Tuple]:
        """
        xy가 속한 polygon key 반환 (covers 사용: 경계 포함)
        """
        if self._strtree is None:
            return None
        p = Point(float(xy[0]), float(xy[1]))
        hits = self._strtree.query(p)
        if not hits:
            return None
        for g in hits:
            if g.covers(p):
                return self._geom_to_key.get(id(g), None)
        return None
...
class FightingModel(Model):
    ...
    def __init__(self, number_agents: int, width: int, height: int, model_num = -1, robot = 'Q'):
        ...
        # loaded = False
        # if self.map_num != 0:
        #     loaded =self._load_prebuilt_env(self.map_num, cache_dir="env_cache")
        
        self.mesh_map_polygon_based()
        if(self.map_num != 0):
    ...
    def _load_prebuilt_env(self, map_num: int, cache_dir="env_cache"):
        path = os.path.join(cache_dir, f"env_map_{map_num}.pkl")
        ...
        self.exit_list = data["exits"]

        self.mesh_list = data["mesh_list"]
        self.pure_mesh = data["pure_mesh"]
        self.obstacle_mesh = data["obstacle_mesh"]

        self.adjacent_mesh = data["adjacent_mesh"]
        self.next_vertex_matrix = data["next_vertex_matrix"]
        self.distance = data["distance"]

        self.match_grid_to_mesh = data["match_grid_to_mesh"]
        self.match_mesh_to_grid = data["match_mesh_to_grid"]
        self.valid_space = data["valid_space"]
    ...
    def visible_exits(self, xy, radius):
        """
        단순 거리 기반 visible exit
        - exit centroid가 radius 안에 있으면 visible
        - vision_atlas / shapely 연산 완전 제거 (초고속)
        """
        ...
        for i, ex in enumerate(self._exit_polys):
            ...
            # centroid 사용 (가장 빠름)
            cx, cy = ex.centroid.x, ex.centroid.y
    ...
    def calculate_mesh_danger(self):
        """
        polygon key -> exit까지의 최단 거리(그래프 기반)
        기존 로직과 동일 목적
        """
        BIG = 1e18
        self.mesh_danger = {}
        if not hasattr(self, "navmesh"):
            return 0
        if not self._exit_polys:
            for k in self.pure_mesh:
                self.mesh_danger[k] = BIG
            return 0

        for k in self.pure_mesh:
            cx, cy = self.navmesh.centroid[k]
            best = BIG
            for exit_idx in range(len(self._exit_polys)):
                q, _ = self.nearest_point_on_exit(exit_idx, (cx, cy))
                d = self.point_to_point_distance_polygraph((cx, cy), q)
                if d < best:
                    best = d
            self.mesh_danger[k] = best
        return 0
    ...
    def mesh_map_polygon_based(self):
        """
        기존 mesh_map()을 대체:
        - 장애물 sanitize
        - boundary - obstacles 로 free polygon 생성
        - constrained triangulation + convex 병합 => polygon navmesh
        - adjacency + floyd-warshall(next_vertex_matrix) 구축
        """
        self._sanitize_obstacles()

        # boundary polygon
        boundary = Polygon([(0,0), (self.width,0), (self.width,self.height), (0,self.height)])
        obstacle_polys = [Polygon(ob) for ob in self.obstacles if len(ob) >= 3]

        # PolyNavMesh build
        self.navmesh = PolyNavMesh(boundary, obstacle_polys)

        # D가 커질수록 삼각형 수/정밀도↓ => 결과 poly 수도 보통 줄어듦
        # merge_passes 키우면 더 많이 합쳐짐(대신 convex 유지 때문에 한계 있음)
        self.navmesh.build(
            D=20.0,               # 너 기존 D=20 유지
            merge_passes=6,        # 병합 패스
            min_shared_edge=1e-6,
            convex_area_tol_ratio=5e-5
        )

        # ---- 기존 필드명으로 “호환” 채우기 ----
        self.mesh_list = self.navmesh.poly_keys[:]   # polygon key(=vertex tuple)
        self.pure_mesh = self.mesh_list[:]           # 모두 free-space polygon
        self.obstacle_mesh = []                      # polygon mesh에서는 obstacle mesh를 별도 리스트로 두지 않음

        # adjacency_mesh: key -> [neighbor_keys]
        self.adjacent_mesh = {k: self.navmesh.adj.get(k, [])[:] for k in self.mesh_list}

        # distance / next_vertex_matrix: floyd-warshall
        self._build_polygon_shortest_paths()
    ...
    def _build_polygon_shortest_paths(self):
        keys = self.mesh_list
        BIG = float("inf")

        self.distance = {a: {} for a in keys}
        self.next_vertex_matrix = {a: {b: None for b in keys} for a in keys}
        ...
        for a in keys:
            for b in keys:
                if a == b:
                    ...
                    self.next_vertex_matrix[a][b] = a
                else:
                    self.distance[a][b] = BIG
                    self.next_vertex_matrix[a][b] = None

        # 이웃 간 거리
        for a in keys:
            ax, ay = self.navmesh.centroid[a]
            for b in self.adjacent_mesh.get(a, []):
                bx, by = self.navmesh.centroid[b]
                d = math.hypot(ax - bx, ay - by)
                self.distance[a][b] = d
                self.next_vertex_matrix[a][b] = b

        # Floyd-Warshall (poly 수가 triangle보다 훨씬 줄어서 여기서 이득 큼)
        for k in keys:
            for i in keys:
    ...
    def mesh_map(self):
        ...
        triangulation_data = {
            'vertices': np.array(vertices_with_points),
            'segments': np.array(segments_with_points),
        }

        # 4) triangulate
        t = tr.triangulate(triangulation_data, 'a')  # 필요하면 'pq30a...' 등으로 튜닝

        # 5) triangles -> shapely polygons (전체를 먼저 만들고)
        tri_polys_all = []
        for tri in t.get('triangles', []):
            v0 = tuple(map(float, t['vertices'][tri[0]]))
            v1 = tuple(map(float, t['vertices'][tri[1]]))
            v2 = tuple(map(float, t['vertices'][tri[2]]))
            P = Polygon([v0, v1, v2])
            if not P.is_valid:
                P = P.buffer(0)
            if P.is_empty or P.area <= 1e-12:
                continue
            tri_polys_all.append(P)

        # 6) free-space triangle만 남김 (한 번만)
        tri_polys_free = []
        if obstacle_union is None:
            tri_polys_free = tri_polys_all
        else:
            for P in tri_polys_all:
                c = P.representative_point()
                if obstacle_union.contains(c) or obstacle_union.touches(c):
                    continue
                tri_polys_free.append(P)

        # 7) merge (한 번만)
        merged_cells = merge_triangles_to_polygons(
            tri_polys_free,
            obstacle_union=obstacle_union,
            allow_concave_reflex_max=0,
            max_vertices=10,
            ndigits=6
        )
        ...
        self.valid_space.clear()

        # 9) 셀 저장 (tuple of coords로 저장하되, 이후엔 Polygon으로 다시 감싸서 씀)
        for cell in merged_cells:
            coords = list(cell.exterior.coords)[:-1]
            coords = [tuple(map(float, xy)) for xy in coords]
            self.mesh_list.append(tuple(coords))

        # 10) obstacle 셀 판정 (centroid가 obstacle 안이면 제외) - free-space만 머지했으면 사실 필요없긴 함
        if obstacle_union is not None:
            for cell_coords in self.mesh_list:
                Pc = Polygon(cell_coords)
                c = Pc.representative_point()
                if obstacle_union.contains(c) or obstacle_union.touches(c):
                    self.obstacle_mesh.append(cell_coords)

        self.pure_mesh = [c for c in self.mesh_list if c not in self.obstacle_mesh]

        # 11) grid -> cell 매칭 (삼각형 말고 polygon contains로)
        #    ⚠️ 매우 비쌈. (W*H*#cells) 구조라서 최적화 필요.
        #    최소한 prepared geometry / bbox로 줄이는 걸 추천.
        prepared = [(cell, Polygon(cell)) for cell in self.pure_mesh]
        prepared = [(cell, prep(P)) for cell, P in prepared]  # shapely.prepared.prep

        for i in range(self.width):
            for j in range(self.height):
                pt = Point(i, j)
                for cell, Pp in prepared:
                    if Pp.covers(pt):  # covers: boundary 포함
                        self.match_grid_to_mesh[(i, j)] = cell
                        self.match_mesh_to_grid.setdefault(cell, []).append([i, j])
                        break

        # 12) adjacency (폴리곤 경계가 "선분"으로 닿는지 기준)
        #    are_polys_adjacent가 삼각형 전용이면 교체 필요.
        self.adjacent_mesh = {a: [] for a in self.pure_mesh}
        for i, A in enumerate(self.pure_mesh):
            PA = Polygon(A)
            for j in range(i + 1, len(self.pure_mesh)):
                B = self.pure_mesh[j]
                PB = Polygon(B)

                # "면적 0이 아닌 선분 접촉"만 adjacency로 보려면:
                inter = PA.boundary.intersection(PB.boundary)
                if inter.is_empty:
                    continue
                # LineString / MultiLineString면 인접으로 간주
                if inter.geom_type in ("LineString", "MultiLineString") and inter.length > 1e-9:
                    self.adjacent_mesh[A].append(B)
                    self.adjacent_mesh[B].append(A)

        # 13) distance / next_vertex_matrix는 Floyd 대신 Dijkstra 추천 (여기선 생략)
    ...
    def construct_map(self):
        # 너 원본대로 obstacles_grid_points 채우고 WallAgent 만들면 됨
        self.obstacles_grid_points.clear()
        for poly in self.obstacles:
            P = Polygon(poly)
            minx, miny, maxx, maxy = P.bounds
            gx0 = int(max(0, math.floor(minx)))
            gx1 = int(min(self.width-1, math.ceil(maxx)))
            gy0 = int(max(0, math.floor(miny)))
            gy1 = int(min(self.height-1, math.ceil(maxy)))
            for x in range(gx0, gx1+1):
                for y in range(gy0, gy1+1):
                    if P.covers(Point(x+0.5, y+0.5)):
                        self.obstacles_grid_points.append([x, y])
                        wa = WallAgent(self.agent_num, self, [x, y], 9)
                        self.agent_num += 1
                        self.schedule.add(wa)
                        self.space.place_agent(wa, (x, y))
                        self.agents.append(wa)
    ...
    def random_agent_distribute_outdoor(self, agent_num: int, ran: int, padding: float = 1.0):
        # polygon key 중 하나를 골라 그 polygon 내부에서 샘플
        for _ in range(agent_num):
            for _t in range(2000):
                k = random.choice(self.pure_mesh)
                # key -> polygon geometry 찾기
                idx = self.mesh_list.index(k)
                poly = self.navmesh.polys[idx]
                minx, miny, maxx, maxy = poly.bounds
                x = random.uniform(minx, maxx)
                y = random.uniform(miny, maxy)
                if poly.covers(Point(x, y)) and self.is_free_point(x, y, padding=padding):
                    break
            else:
                x, y = random.uniform(1, self.width-1), random.uniform(1, self.height-1)
    ...
    def point_to_point_distance_polygraph(self, a_xy, b_xy) -> float:
        """
        polygon navmesh 기반 최단 거리(그래프 중심거리 + 출발/도착 보정)
        - 같은 polygon이면 유클리드
        - 다르면: a->poly_center + graph + poly_center->b
        """
        akey = self.find_mesh(a_xy)
        bkey = self.find_mesh(b_xy)
        ...
        ax, ay = self.navmesh.centroid[akey]
        bx, by = self.navmesh.centroid[bkey]
        base = self.distance.get(akey, {}).get(bkey, float("inf"))
        if not math.isfinite(base):
            # fallback
            return math.hypot(a_xy[0]-b_xy[0], a_xy[1]-b_xy[1])

        return math.hypot(a_xy[0]-ax, a_xy[1]-ay) + base + math.hypot(b_xy[0]-bx, b_xy[1]-by)
