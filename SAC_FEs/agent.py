...
class CrowdAgent(Agent):
    ...
    def point_to_point_distance(self, point1, point2):
        """
        Polygon 기반 coarse mesh 그래프에서 point1 -> point2 최단거리 근사.
        - mesh는 'id'로 표현된다고 가정 (int)
        - model.next_vertex_matrix[from_id][to_id] = next_id
        - model.mesh_centroid[id] = (cx, cy) 사전 준비되어 있다고 가정
        """
        m1 = self.choice_safe_mesh(point1)  # ✅ int mesh id
        m2 = self.choice_safe_mesh(point2)  # ✅ int mesh id

        if m1 is None or m2 is None:
            return 99999999999

        nxt = self.model.next_vertex_matrix.get(m1, {}).get(m2, None)
        if nxt is None:
            return 99999999999
        ...
        # 바로 이웃(mesh hop 1번)이고, 굳이 centroid 경유 안 하고 직선으로 처리하고 싶으면:
        if nxt == m2:
            return math.hypot(point1[0] - point2[0], point1[1] - point2[1])

        # centroid 캐시
        c = self.model.mesh_centroid  # dict[int] -> (cx,cy)

        dist = 0.0

        # 1) point1 -> 첫 hop mesh centroid
        cur = m1
        cur = self.model.next_vertex_matrix[cur][m2]  # 첫 hop
        cx, cy = c[cur]
        dist += math.hypot(cx - point1[0], cy - point1[1])

        # 2) hop centroid -> hop centroid
        while True:
            nxt = self.model.next_vertex_matrix[cur][m2]
            if nxt is None:
                return 99999999999
            if nxt == m2:
                break
            dist += self.model.distance[cur][nxt]  # ✅ mesh 그래프 edge weight
            cur = nxt

        # 3) 마지막 mesh centroid -> point2
        cx, cy = c[cur]
        dist += math.hypot(cx - point2[0], cy - point2[1])

        return dist

    def _neighbors(self, radius):
        # 1) 반경 후보 (이미 radius 조건을 만족한다고 가정)
