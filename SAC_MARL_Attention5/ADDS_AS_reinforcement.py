...
class CentralizedAttentionQNetwork(nn.Module):
    ...
    def forward(
        ...
        if joint_mask is not None:
                mask_expanded = joint_mask.unsqueeze(-1).float() # (B, N, 1)
                # 마스킹된 에이전트의 피처만 더하기
                sum_embed = (agent_embed * mask_expanded).sum(dim=1, keepdim=True)
                # 살아있는 에이전트의 수 계산 (0으로 나누기 방지를 위해 clamp 사용)
                valid_count = mask_expanded.sum(dim=1, keepdim=True).clamp(min=1.0)
                
                global_agent_context = sum_embed / valid_count
        else:
            global_agent_context = agent_embed.mean(dim=1, keepdim=True)


        global_agent_context_expanded = global_agent_context.expand(-1, N, -1)

        film_input_embed = torch.cat([agent_embed, global_agent_context_expanded], dim=-1)
        gamma_beta = self.film_gen(film_input_embed)
