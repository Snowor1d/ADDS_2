...
MAX_ROBOTS = 3
MODE_WAYPOINT = 0
MODE_GO_NEAREST_EXIT = 1
MODE_DIM = 2
CONTINUOUS_ACTION_DIM = 4
ACTION_DIM = 1 + CONTINUOUS_ACTION_DIM
CRITIC_ACTION_DIM = MODE_DIM + CONTINUOUS_ACTION_DIM


def action_to_critic_action(action: torch.Tensor) -> torch.Tensor:
    """
    Convert env/replay action [mode, radius, dx, dy, speed] to critic input
    [mode_waypoint, mode_go_nearest_exit, radius, dx, dy, speed].
    """
    mode = action[..., 0].long().clamp(0, MODE_DIM - 1)
    mode_onehot = F.one_hot(mode, num_classes=MODE_DIM).to(dtype=action.dtype, device=action.device)
    return torch.cat([mode_onehot, action[..., 1:]], dim=-1)
                ...
                for i, rb in enumerate(robots_padded):
                    ...
                    if needs_new or is_finished:
                        ...
                        if np.random.rand() < eps:
                            # Hybrid action: [mode, radius, dx, dy, speed]
                            action_i = np.array([
                                0,
                        ...
                        has_active_order = (
                            rb.has_active_motion_order()
                            if hasattr(rb, "has_active_motion_order")
                            else bool(getattr(rb, "planner_active", False))
                        )
                        rb.new_order_need = not has_active_order
...
class ReplayBuffer:
    ...
    def load(self, filepath: Union[str, bytes, os.PathLike]) -> None:
        ...
        convert_legacy_action = prev_action_dim == CONTINUOUS_ACTION_DIM and self.action_dim == ACTION_DIM

        if prev_action_dim != self.action_dim and not convert_legacy_action:
            raise ValueError(
                f"[ReplayBuffer.load] incompatible action_dim: file={prev_action_dim}, "
        ...
        if convert_legacy_action:
            self.joint_actions[:self.size] = 0.0
            self.joint_actions[:self.size, :, 0] = MODE_WAYPOINT
            self.joint_actions[:self.size, :, 1:] = data["joint_actions"]
        else:
...
class PolicyNetwork(nn.Module):
    def __init__(self, ego_shape=(25,25), global_shape=(50,50), robot_dim = 3, use_robot: bool = True):
        ...
        self.mode_logits = nn.Linear(64, MODE_DIM)
    ...
    def forward(self, ego_state, global_state, robot_state=None):
        ...
        mode_logits = self.mode_logits(feat)
        ...
        return mode_logits, r_m, r_s, d_m, d_s, spd_m, spd_s
    
    def sample_action(self, ego_state, global_state, robot_state=None, return_critic_action=False):
        mode_logits, r_m, r_s, d_m, d_s, spd_m, spd_s = self.forward(ego_state, global_state, robot_state)
        ...
        mode_logp = mode_dist.log_prob(mode_idx)
        mode_action = mode_idx.to(dtype=r_m.dtype).unsqueeze(-1)

        mode_onehot_st = F.gumbel_softmax(mode_logits, tau=1.0, hard=True, dim=-1)
        ...
        # --- Final Combine ---
        # Env/replay action: [batch, 5] -> [mode, r, dx, dy, speed]
        if r_act.dim() == 1:
        ...
        action = torch.cat([mode_action, r_act, dir_act, spd_act], dim=-1)
        critic_action = torch.cat([mode_onehot_st, r_act, dir_act, spd_act], dim=-1)
        
        # logp들도 (B,) 형태인지 확인 (이미 sum(dim=-1)을 해서 (B,)일 것입니다)
        total_logp = mode_logp + r_logp + dir_logp + spd_logp

        if return_critic_action:
            return action, total_logp, critic_action
...
class SACAgent:
    def __init__(self, input_shape=(50,50), gamma=GAMMA_START, alpha=0.2, tau=0.995, lr=1e-4, batch_size=64, replay_size=int(1e5), device="cpu", start_epsilon = 1.0, start_epsilon_long = 0.1, long_epsilon_min=0):
        ...
        self.target_entropy = -(CONTINUOUS_ACTION_DIM + 1)
    ...
    def select_action(self, ego_state_np, global_state_np, robot_state_np=None, deterministic=False):
        ...
        if (EXPLORATION_TYPE == 0):
            if np.random.rand() < self.epsilon:
                ...
                return np.array([mode, r, dx, dy, spd], dtype=np.float32), True
        ...
        with torch.no_grad():
            if deterministic:
                mode_logits, r_m, _, d_m, _, spd_m, _ = self.policy.forward(ego_t, global_t, robot_t)
                
                # Sigmoid 스케일링 (PolicyNetwork.sample_action 내부 로직과 일치시켜야 함)

                mode = torch.argmax(mode_logits, dim=-1, keepdim=True).to(dtype=r_m.dtype)
                ...
                action_t = torch.cat([mode, r, dir_vec, spd], dim=-1) 
            else:
        ...
        #확인용 출력: [mode, r, dx, dy, speed] 순서임
        print(f"DEBUG: Policy Output Shape = {action_np.shape}")
        print(
            f"Action: mode={int(round(action_np[0]))}, r={action_np[1]:.2f}, "
            f"dx={action_np[2]:.2f}, dy={action_np[3]:.2f}, spd={action_np[4]:.2f}"
        )
...
# ------------------------------------------------- #
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            next_joint_critic_action = action_to_critic_action(next_joint_action)
            ...
            q1_next_all = self.q1_target(
                next_joint_ego, next_global_state, next_joint_critic_action, next_joint_robot, next_joint_mask
            )
        ...
        # 2) Critic update
        # -----------------------
        joint_critic_action = action_to_critic_action(joint_action)
        q1_val_all = self.q1(joint_ego, global_state, joint_critic_action, joint_robot, joint_mask)
        q2_val_all = self.q2(joint_ego, global_state, joint_critic_action, joint_robot, joint_mask)
        ...
        # 3) Actor update
        # -----------------------
        new_joint_action, joint_logp, new_joint_critic_action = self.sample_joint_actions(
            joint_ego, global_state, joint_robot, joint_mask, return_critic_actions=True
        )
        logp_i = gather_agent_tensor(joint_logp.unsqueeze(-1), agent_index).squeeze(-1)

        q1_new_all = self.q1(joint_ego, global_state, new_joint_critic_action, joint_robot, joint_mask)
        q2_new_all = self.q2(joint_ego, global_state, new_joint_critic_action, joint_robot, joint_mask)
    ...
    def sample_joint_actions(self, joint_ego, global_state, joint_robot_state, joint_mask, return_critic_actions=False):
        """
        joint_ego: (B, N, 4, E, E)
        ...
        for i in range(N):
            ...
            if return_critic_actions:
                act_i, logp_i, critic_act_i = self.policy.sample_action(
                    ego_i, global_state, robot_i, return_critic_action=True
                )
            else:
            ...
            if return_critic_actions:
                critic_actions.append(critic_act_i * m_i)
            log_probs.append(logp_i)

        joint_action = torch.stack(actions, dim=1)   # (B,N,A)
        joint_logp = torch.stack(log_probs, dim=1)   # (B,N)
        if return_critic_actions:
            joint_critic_action = torch.stack(critic_actions, dim=1)
            return joint_action, joint_logp, joint_critic_action
