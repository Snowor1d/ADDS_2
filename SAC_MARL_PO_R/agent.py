...
class RobotAgent(CrowdAgent):
    ...
    def _nearest_exit_by_path_distance(self):
        if not getattr(self.model, "_exit_polys", None):
        ...
        for exit_idx in range(len(self.model._exit_polys)):
            q, _ = self.model.nearest_point_on_exit(exit_idx, self.xy)
            try:
                path_dist = self.point_to_point_distance(self.xy, q)
            except Exception:
                path_dist = float("inf")
            ...
            if path_dist < best_dist:
                best_dist = path_dist
                best_idx = exit_idx
                best_goal = self.model.goal_point_into_exit(exit_idx, self.xy, eps=0.6)

        if best_idx is None:
            exit_idx, _, euclid_dist = self.model.nearest_exit(self.xy)
            if exit_idx is None:
                return None, None, float("inf")
            return exit_idx, self.model.goal_point_into_exit(exit_idx, self.xy, eps=0.6), euclid_dist

        return best_idx, best_goal, best_dist

    def _set_planner_target(self, goal, speed, escape_mode=False):
        self.velocity_command_active = False
        self.velocity_command = [0.0, 0.0]

        if goal is None:
            return self.receive_action([0.0, 0.0, 0.0])

        target_speed = max(0.0, min(float(speed), ROBOT_SPEED_MAX))
        dx = float(goal[0]) - self.xy[0]
        dy = float(goal[1]) - self.xy[1]
        dist = math.hypot(dx, dy)

        if dist < 0.5 or target_speed <= 1e-5:
            return self.receive_action([0.0, 0.0, 0.0])

        self.action[0] = (math.atan2(dy, dx) - self.angle + math.pi) % (2.0 * math.pi) - math.pi
        self.action[1] = dist
        self.action[2] = target_speed

        self.planner_target = [float(goal[0]), float(goal[1])]
        self.planner_speed = target_speed
        self.planner_active = True
        self.escape_mode_active = bool(escape_mode)
        self.planner_fallback_vec = self._safe_normalize(dx, dy)

        return np.array(self.action, dtype=float)

    def robot_policy_going_exit(self):
        _, goal, _ = self._nearest_exit_by_path_distance()
        return self._set_planner_target(goal, ROBOT_SPEED_MAX, escape_mode=True)

    def receive_escape_action(self, speed=None):
        """
        High-level primitive: set a planner target inside the path-nearest exit.
        """
        target_speed = ROBOT_SPEED_MAX if speed is None else float(speed)
        _, goal, _ = self._nearest_exit_by_path_distance()
        return self._set_planner_target(goal, target_speed, escape_mode=True)
    
    def receive_action_from_policy(self, action):
        """
        action = [mode, radius, dx, dy, speed] for hybrid policy.
        Backward-compatible legacy action = [radius, dx, dy, speed].
        dx, dy: World 좌표계 기준의 방향 벡터 (Normalized 권장)
        """
        if len(action) == 5:
            mode = int(round(float(action[0])))
            if mode == 1:
                if not (hasattr(self.model, "in_exit") and self.model.in_exit(self.xy, tol=0.5)):
                    return self.receive_escape_action(action[4])
            action = action[1:]
    ...
    def has_active_motion_order(self):
        return bool(
            getattr(self, "planner_active", False)
            or getattr(self, "velocity_command_active", False)
        )
    ...
    def _planner_direction_to_goal(self, goal):
        ...
        try:
            ...
            if len(shared_vertices) >= 2:
                ...
                # 공유 변 자체가 아니라 다음 mesh 안쪽의 portal point를 향하게 한다.
                # 경계선 위 점만 목표로 잡으면 mesh 판정이 앞뒤로 흔들릴 때 진동할 수 있다.
                portal_pt = self._closest_point_on_segment(v1, v2, self.xy)
                next_center = (
                    (next_mesh[0][0] + next_mesh[1][0] + next_mesh[2][0]) / 3.0,
                    (next_mesh[0][1] + next_mesh[1][1] + next_mesh[2][1]) / 3.0,
                )
                inward_x = next_center[0] - portal_pt[0]
                inward_y = next_center[1] - portal_pt[1]
                inward_norm = math.hypot(inward_x, inward_y)
                if inward_norm > 1e-9:
                    inward_step = min(max(self.body_radius * 2.0, 0.8), inward_norm)
                    target_pt = (
                        portal_pt[0] + inward_step * inward_x / inward_norm,
                        portal_pt[1] + inward_step * inward_y / inward_norm,
                    )
                else:
                    target_pt = next_center
    ...
    def robot_policy_Q(self):
        ...
        if self.collision_check == 1:
            if not getattr(self, "escape_mode_active", False):
                #print("충돌함!")
                self.new_order_need = True
        ...
        if self.planner_active:
            if getattr(self, "escape_mode_active", False):
                reached_escape_goal = self._goal_reached(self.planner_target, tol=0.3)
                in_exit = self.model.in_exit(self.xy, tol=0.5) if hasattr(self.model, "in_exit") else False
                if reached_escape_goal or in_exit:
                    self.new_order_need = True
                    self._cancel_planner(stop_velocity=False)
            else:
                self._new_goal_trigger(self.planner_target, tol=2)
                if self._goal_reached(self.planner_target, tol=0.3):
                    self.new_order_need = True
                    self._cancel_planner(stop_velocity=False)
