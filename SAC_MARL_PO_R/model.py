...
from ADDS_AS_reinforcement import SACAgent, ReplayBuffer, PolicyNetwork, ACTION_SCALE, FrameStack, FrameStack2, ego_crop_from_full_map, FrameStackWithStep 
from config import *
import json
import ast
            ...
            if self.robot_version == 'Q':
                if self.using_model:
                    ...
                    if (robot.new_order_need):
                        ...
                        has_active_order = (
                            robot.has_active_motion_order()
                            if hasattr(robot, "has_active_motion_order")
                            else bool(getattr(robot, "planner_active", False))
                        )
                        robot.new_order_need = not has_active_order
                    # else:
