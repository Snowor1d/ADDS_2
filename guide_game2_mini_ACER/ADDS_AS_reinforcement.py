...
log_dir = os.path.join(home_dir, "learning_log_guide_game2_mini_ACER")
...
if __name__ == "__main__":
 ...
 # TensorBoard 로그 경로 및 total_reward.txt 경로 설정
    ...
    if model_load == 1:
        ...
        model_name = "acer_mini_checkpoint_ep_590.pth"
        ...
        model_files = [f for f in os.listdir(log_dir) if f.startswith("acer_mini_checkpoint") and f.endswith(".pth")]
        if model_files:
    ...
    for episode in range(max_episodes):
        ...
        if (episode+1) % 10 == 0:
            model_filename = os.path.join(log_dir, f"acer_mini_checkpoint_ep_{start_episode + episode + 1}.pth")
